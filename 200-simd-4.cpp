...
public:
    ...
    void filter_tuples_with_key_5_with_simd() {
        std::cout << "Counting tuples with key 5 \n";
        uint32_t tuple_count = 0;
        ...
        for (size_t page_itr = 0; page_itr < num_pages; page_itr++) {
            ...
            for (size_t slot_itr = 0; slot_itr < MAX_SLOTS; slot_itr += 64) { 
                uint8_t keys[64];
                ...
                // Load the keys into 4 NEON 8-bit integer vectors
                uint8x16_t keys_vec[4];
                for (int i = 0; i < 4; ++i) {
                    keys_vec[i] = vld1q_u8(&keys[i * 16]);
                }

                // Compare the key vectors with key_to_match_vec (value 5)
                for (int i = 0; i < 4; ++i) {
                    uint8x16_t res_vec = vceqq_u8(keys_vec[i], key_to_match_vec);

                    // Extract results
                    uint8_t results[16];
                    vst1q_u8(results, res_vec);
...
int main() {
    ...
    std::cout << "Filter time (Without SIMD): " << duration.count() << " microseconds" << std::endl;

    auto start_filter_with_simd_time = std::chrono::high_resolution_clock::now();
    db.filter_tuples_with_key_5_with_simd();
    auto end_filter_with_simd_time = std::chrono::high_resolution_clock::now();
    auto duration_with_simd = std::chrono::duration_cast<std::chrono::microseconds>(end_filter_with_simd_time - start_filter_with_simd_time);
    std::cout << "Filter time (With SIMD): " << duration_with_simd.count() << " microseconds" << std::endl;
