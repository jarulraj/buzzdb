...
public:
    ...
    void acquireAllTupleAccess(const TxnPtr& txn,
                               AccessType type,
                               TableMetadata& metadata) {
        TableHeap tableHeap(metadata, page_manager);
    ...
    void acquireStatementLocks(const TxnPtr& txn,
        ...
        if (std::regex_match(statement, matches, selectTableRegex)) {
            const std::string tableName = matches[1];
            auto& metadata = catalog.getTable(tableName);
            acquireAllTupleAccess(txn, AccessType::Read, metadata);
        }
    }

    void acquireTxnAccess(
            const TxnPtr& txn,
            AccessType type,
            const std::vector<ConcurrencyControlResource>& resources) {
        if (!txn || resources.empty()) {
...
void runRecoveryAndConcurrencyControlWorkload(
        const std::string& workloadName,
        const std::function<void(BuzzDB&)>& configurePolicy) {
    std::cout << "\nWorkload: " << workloadName << std::endl;
    {
        BuzzDB db;
        configurePolicy(db);
        DatabaseImporter::importFile(db, "booking.txt");
        std::cout << "Concurrency-control policy: "
                  << db.concurrencyControlPolicyName() << std::endl;
        std::cout << "The execution layer asks the policy for READ or WRITE "
                  << "access before touching tuples.\n";
        std::cout << "The 2PL policy implements those requests using S and X "
                  << "locks held until COMMIT or ABORT.\n";

        std::cout << "\n1. Runtime abort: strict 2PL prevents cascading aborts.\n";
        auto writer = db.beginTransaction();
        db.execute(writer,
        ...
        std::thread readerThread([&]() {
            try {
                ...
                std::cout << "\nReader tries to read 1A while writer is uncommitted.\n";
                readerStarted = true;
                db.execute(reader, "SELECT {*} FROM seats WHERE seat_no = 1A");
                db.commit(reader);
            } catch (...) {
        ...
        while (!readerStarted && !readerFinished) {
        ...
        if (readerFinished) {
            ...
            throw std::runtime_error("Expected reader to wait for writer's X lock.");
        }

        std::cout << "\nConcurrency control outcome: reader waits on the tuple lock, "
                  << "so it never depends on the writer's dirty value.\n";
        std::cout << "Recovery outcome: abort writes CLRs, restores the page, "
                  << "and only then releases locks.\n";
        db.abort(writer);
        ...
        std::cout << "\nNo cascading abort: only the writer was undone. "
                  << "The reader saw the restored committed value:\n";
        db.executeStatementsAndQueries({
            "SELECT {*} FROM seats WHERE seat_no = 1A",
        });

        std::cout << "\n2. Crash restart: locks disappear, so recovery must clean disk.\n";
        db.crashAt(CrashPoint::AFTER_STEAL_PAGE_FLUSH);
        auto crashTxn = db.beginTransaction();
        bool sawCrash = false;
        try {
            db.execute(crashTxn,
            ...
            sawCrash = true;
            std::cout << "Crash: " << error.what() << std::endl;
        }
        if (!sawCrash) {
            throw std::runtime_error("Expected simulated STEAL crash.");
        }
        std::cout << "The process stops here. Locks were in memory, so they "
                  << "cannot clean the disk page after restart.\n";
    }
    ...
    std::cout << "\nRestart after crash.\n";
    {
        BuzzDB db;
        configurePolicy(db);
        std::cout << "\nAfter ARIES recovery, a new transaction reads 1B.\n";
        db.executeStatementsAndQueries({
            "SELECT {*} FROM seats WHERE seat_no = 1B",
        });
    }

    std::cout << "\nResult: 2PL is a swappable concurrency-control policy.\n";
}
...
int main() {
    ...
    std::cout << "Concurrency-control policy interface\n";
    std::cout << "Two-Phase Locking is one possible policy\n";

    runRecoveryAndConcurrencyControlWorkload(
        "Two-Phase Locking",
        [](BuzzDB& db) {
