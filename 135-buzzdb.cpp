...
void printV104BootstrapTrace(const std::string& data_file) {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v135-bootstrap-trace-" + std::to_string(::getpid()));
...
void printPartitionedSQLTrace(const std::string& data_file) {
    ...
    if (runtime.runUntilIdle() != 3) {
        throw std::runtime_error("trace index range runtime failed");
    }
    size_t copied = activeEntries(target).rows.size();
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v135; "
                  << "this version focuses on secondary indexes."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v135: secondary indexes as ranges" << std::endl;
    ...
    auto createTitleYearIndexCommand = [&]() {
        return CreateSecondaryIndexCommand{
            title_year_index, "title", "production_year", "group-1"};
    };

    auto expectedPrimaryKeysFor =
        [](const std::vector<TitleRowFixture>& rows,
           const std::string& year) {
        std::vector<std::string> primary_keys;
        for (const auto& row : rows) {
            if (row.values.size() >= 4 && row.values[3] == year) {
                primary_keys.push_back(row.values[0]);
            }
        }
    ...
    auto distinctYearCount =
        [](const std::vector<TitleRowFixture>& rows) {
        std::set<std::string> years;
        for (const auto& row : rows) {
            if (row.values.size() >= 4) years.insert(row.values[3]);
        }
        return years.size();
    };
    ...
    auto activeIndexEntryCount = [&](const SelectAllResult& rows,
                                     const std::string& index_name) {
        size_t count = 0;
        auto index_column = catalogColumnIndex(rows, "index_name");
        auto status_column = catalogColumnIndex(rows, "status");
        if (!index_column.has_value() || !status_column.has_value()) {
            return count;
        }
        for (const auto& row : rows.rows) {
            if (*index_column < row.size() &&
                *status_column < row.size() &&
                row[*index_column] == index_name &&
                row[*status_column] == "active") {
                ++count;
            }
        }
    ...
    auto primaryKeysContain = [](const IndexLookupResult& lookup,
                                 const std::string& primary_key) {
        return std::find(lookup.primary_keys.begin(),
    ...
    auto controlPlaneRuntime = [&](IndexScenario& scenario) {
        ...
        runtime.registerReplicaGroup(
            ...
            [&](const Command& command) {
                return commitCommand(scenario.index_source_group, command);
            }
        );
        runtime.registerReplicaGroup(
            scenario.index_target_group.group_id,
            [&](const Command& command) {
                return commitCommand(scenario.index_target_group, command);
            }
        );
        return runtime;
    };

    auto moveIndexRangeThroughRuntime =
        [&](IndexScenario& scenario,
            ...
            tests.check(runtime.runUntilIdle() == 3,
                        "control plane runtime should prepare, catch up, and commit");
            size_t copied = physicalIndexEntryCount(target, range_id);
    ...
    tests.test("HashIndex stores duplicate secondary-key postings", [&] {
        ...
        tests.check(index.insert(2008, 10) &&
                        index.insert(2008, 20) &&
                        index.insert(2011, 30),
                    "hash index should accept duplicate index keys");
        tests.check(index.lookup(2008) == std::vector<int>({10, 20}),
                    "lookup should return all primary keys for a duplicate key");
        tests.check(index.getValue(2011) == 30,
                    "v104 getValue API should still work");
        tests.check(index.rangeQuery(2000, 2010) ==
    ...
    tests.test("Secondary index lookup uses routed physical HashIndex", [&] {
        ...
        tests.check(lookup.primary_keys ==
                    ...
                    "lookup should return the real title ids for the year");
        tests.check(lookup.hash_slot ==
                        HashIndex::hashSlotFor(std::stoi(year)) &&
                        lookup.hash_entry_count ==
                            scenario.fixture.initial_rows.size() &&
    ...
    tests.test("Insert update and delete maintain physical index entries", [&] {
        ...
        std::string new_year =
            old_year == scenario.fixture.duplicate_year
    ...
    tests.test("Secondary index metadata and lookup replicate to followers", [&] {
        auto scenario = buildIndexScenario();
        std::string year = scenario.fixture.duplicate_year;
    ...
    tests.test("Index range transfers independently from table range", [&] {
        ...
        int transfer_epoch = moveIndexRangeThroughRuntime(
        ...
        Result table_route_result =
            leaderNode(scenario.catalog)->executeReadOnlyForTest(
                ExplainRouteCommand{
                    "title",
                    scenario.fixture.initial_rows.front().values[0],
                    false});
