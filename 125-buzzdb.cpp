...
std::vector<std::string> resultWireFields(const Result& result) {
    return std::visit(
        [](const auto& value) {
            ...
            } else if constexpr (std::is_same_v<T, RouteResult>) {
                fields = {"RouteResult", value.start_key, value.end_key,
                          std::to_string(value.range_ids.size())};
                fields.insert(fields.end(), value.range_ids.begin(),
                              value.range_ids.end());
                fields.push_back(std::to_string(value.replica_group_ids.size()));
                fields.insert(fields.end(), value.replica_group_ids.begin(),
                              value.replica_group_ids.end());
                fields.push_back(std::to_string(value.descriptor_versions.size()));
                for (int version : value.descriptor_versions) {
...
Result deserializeResult(const std::string& encoded) {
    ...
    if (kind == "RouteResult" && fields.size() >= 6) {
        size_t cursor = 1;
        RouteResult result;
        result.start_key = fields.at(cursor++);
        result.end_key = fields.at(cursor++);
        size_t range_count =
            static_cast<size_t>(std::stoull(fields.at(cursor++)));
        for (size_t i = 0; i < range_count; ++i) {
            result.range_ids.push_back(fields.at(cursor++));
        }
        size_t group_count =
            static_cast<size_t>(std::stoull(fields.at(cursor++)));
        for (size_t i = 0; i < group_count; ++i) {
            result.replica_group_ids.push_back(fields.at(cursor++));
        }
        size_t version_count =
            static_cast<size_t>(std::stoull(fields.at(cursor++)));
        for (size_t i = 0; i < version_count; ++i) {
            result.descriptor_versions.push_back(std::stoi(fields.at(cursor++)));
        }
        if (cursor != fields.size() ||
            result.range_ids.size() != result.replica_group_ids.size() ||
            result.range_ids.size() != result.descriptor_versions.size()) {
            throw std::runtime_error("bad RouteResult fields");
        }
        return result;
    }
...
private:
    ...
    Result executeForReplicatedApply(const ExplainRouteCommand& command,
                                     const TxnPtr&) {
        return executeOne(command);
    }
...
bool isReadOnlyCommand(const Command& command) {
    return std::holds_alternative<SelectAllCommand>(command) ||
           ...
           std::holds_alternative<ExplainRouteCommand>(command) ||
...
void printPartitionedSQLTrace(const std::string& data_file) {
    ...
    SearchState state = buildInitialReplicaGroupState(
        settings, replicas, leader, client, 1);
    state = deliverConsensusCommandToQuorum(
...
int main(int argc, char* argv[]) {
    ...
    tests.test("Range descriptor changes are versioned and latest wins", [&] {
        ...
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 505, 4,
    ...
    tests.test("Router cache refreshes after descriptor change", [&] {
        ...
        const std::string new_end = tableRowKey("title", primary_key) + "~";
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 506, 4,
            RegisterRangeCommand{defaultRangeIdForTable("title"),
                                 tableRowPrefix("title"),
                                 new_end,
                                 "group-1",
