...
struct QueryComponents {
    ...
    std::vector<bool> select_min_columns;
    ...
    bool hasMinProjection() const {
        return std::any_of(
            select_min_columns.begin(),
            select_min_columns.end(),
            [](bool is_min) { return is_min; }
        );
    }
};
...
bool joinProjectionUsesMin(const std::string& projection) {
    std::regex minProjectionRegex(
        "^\\s*MIN\\s*\\{([A-Za-z_][A-Za-z0-9_]*)\\.([A-Za-z_][A-Za-z0-9_]*)\\}\\s*$",
        std::regex_constants::icase
    );
    return std::regex_match(projection, minProjectionRegex);
}
...
QueryComponents parseJoinQuery(const std::string& query, Catalog& catalog) {
    ...
    for (std::sregex_iterator it(projection_list.begin(),
        ...
        components.select_min_columns.push_back(joinProjectionUsesMin((*it)[0]));
    ...
    if (components.hasMinProjection() &&
        !std::all_of(
            components.select_min_columns.begin(),
            components.select_min_columns.end(),
            [](bool is_min) { return is_min; })) {
        throw std::runtime_error("Join query cannot mix MIN projections with raw projections.");
    }
...
QueryResult executeJoinQuery(const QueryComponents& components,
    ...
    QueryResult result;
    const bool min_projection = components.hasMinProjection();
    bool saw_min_input = false;
    std::vector<std::unique_ptr<Field>> min_values;
    ...
    while (rootOp->next()) {
        ...
        if (min_projection) {
            if (!saw_min_input) {
                min_values = cloneFields(output);
                saw_min_input = true;
                continue;
            }

            for (size_t column = 0; column < output.size(); column++) {
                if (compareFields(*output[column], *min_values[column]) < 0) {
                    min_values[column] = output[column]->clone();
                }
            }
            continue;
        }
    ...
    if (min_projection && saw_min_input) {
        result.sample_rows.push_back(std::move(min_values));
        result.row_count = 1;
    }
    return result;
}
...
void runImdbAnalyzeAndHashJoin() {
    ...
    std::cout << "\nResult: ANALYZE reports table and column stats.\n";
}

int main() {
    try {

    std::cout << "IMDB ANALYZE statistics\n";
