...
// BuzzDB v109: BuzzDB state machine inside the search simulator.
//
// This version keeps the v108 DSLabs-style simulator but brings BuzzDB back as
// the application being tested. The database here is intentionally an
// in-memory, deterministic state machine; later versions can replicate this
// state machine before reintroducing the richer single-node storage engine.
...
int main() {
    std::cout << "BuzzDB v109: BuzzDB state machine inside the search simulator"
              << std::endl;
    TestRunner tests;
