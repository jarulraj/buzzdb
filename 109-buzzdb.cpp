...
// -----------------------------------------------------------------------------
// v109 simulator-facing command API
// -----------------------------------------------------------------------------
...
std::string makeScratchBuzzDBFile() {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v109-core-" + std::to_string(::getpid()) + "-" + std::to_string(++counter));
...
void printRequestExecution(const SearchState& state,
    ...
    std::cout << "  duplicate-suppression table: key "
              << "(client=" << client_id << ", request=" << request_id << ")"
...
void printBuzzDBBootstrapTrace(const std::string& data_file) {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v109-bootstrap-trace-" + std::to_string(::getpid()));
...
int main(int argc, char* argv[]) {
    std::cout << "BuzzDB v109: real BuzzDB core inside the search simulator" << std::endl;
    ...
    tests.test("BuzzDBCore uses real ARIES log and master files", [&] {
        ...
        std::filesystem::path dir = std::filesystem::temp_directory_path() / "buzzdb-v109-real-log-test";
        std::filesystem::create_directories(dir);
    ...
    tests.test("BuzzDBCore bootstraps JOB tables from an IMDB tuple file", [&] {
        runWithSuppressedStdout([&] {
        std::filesystem::path dir = std::filesystem::temp_directory_path() / "buzzdb-v109-bootstrap-test";
        std::filesystem::remove_all(dir);
        std::filesystem::create_directories(dir);
        {
            BuzzDBCore db((dir / "buzzdb.dat").string());
            ...
            Result join_result = db.execute(QuerySQLCommand{jobDistributorJoinQuery()});
