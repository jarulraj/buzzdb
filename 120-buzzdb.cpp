...
void printLogRepairTrace(const std::string& data_file) {
    ...
    std::cout << "  initial leader: " << leader
    ...
    switched_partition.partition({
        {replicas[0], replicas[1]},
        {replicas[2], replicas[3], replicas[4], client}
    });
    state = electConsensusLeader(
        state, switched_partition, new_leader, {replicas[3], replicas[4]});
    const auto* new_node = state.nodeAs<ConsensusReplica>(new_leader);
    std::cout << "  new majority leader: " << new_leader
              << " term=" << (new_node ? new_node->currentTerm() : 0)
              << ", preserved rows="
              << describeResult(new_node
                                    ? new_node->executeReadOnlyForTest(
                                          CountRowsCommand{"title"})
                                    : Result{TableNotFoundResult{}})
              << "\n" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    tests.test("Progress with no partition preserves sequential BuzzDB results", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address leader = replicas[0];
    ...
    tests.test("Majority partition commits while minority remains empty", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings majority_partition;
        ...
        for (const auto& replica : std::vector<Address>{replicas[3], replicas[4]}) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(node != nullptr && node->lastNonEmpty() == 0,
                        replica.str() + " should not learn entries across partition");
        }
        CheckResult logs = consensusLogsConsistentAllSlots(state, replicas);
        tests.check(logs.ok, logs.message);
    });

    tests.test("Minority partition cannot decide a BuzzDB command", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address candidate = replicas[3];
        Address helper = replicas[4];
        Address client = ScenarioAddress::client1();
        SearchState initial = consensusClusterState(replicas);
        initial.send(client, candidate, ClientRequest{1, 1, createTitleTableCommand()});
    ...
    tests.test("Switching majorities preserves committed BuzzDB log", [&] {
        ...
        first_partition.partition({
            {replicas[0], replicas[1], replicas[2], client1},
        ...
        second_partition.partition({
            {replicas[0], replicas[1]},
            {replicas[2], replicas[3], replicas[4], client2}
        });
        ...
        state = catchUpConsensusFollowerThrough(state, second_partition,
                                                second_leader, replicas[3], 2);
        state = catchUpConsensusFollowerThrough(state, second_partition,
                                                second_leader, replicas[4], 2);
        state = deliverConsensusCommandToQuorum(
            state, second_partition, second_leader, client2, 2, 1,
    ...
    tests.test("Bounded search finds one command with consistent logs", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchState initial = consensusClusterState(replicas);
        SearchSettings setup;
        initial = electConsensusLeader(initial, setup, leader,
                                       {replicas[1], replicas[2]});
        initial.send(client, leader, ClientRequest{1, 1, createTitleTableCommand()});
        ...
        search.goals.push_back({
            "CLIENT_HAS_RESULT",
            [](const SearchState& state) {
                return CheckResult{clientReplyInNetworkFor(state, 1, 1),
                                   "client has no result"};
            }
        });

        SearchResults result = bfs(initial, search);
        tests.check(result.end_condition == SearchResults::EndCondition::GoalFound,
                    "search should find a committed command, got " +
                        endConditionName(result.end_condition) +
                        ": " + result.message);
    });

    tests.test("Consensus client retries from follower to leader", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        ...
        state.addNode(std::make_unique<ConsensusClientWorker>(
            client,
            std::vector<Address>{follower, leader, replicas[2]},
            10,
            Workload(
        ...
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                "first client request to follower without known leader"),
            settings);
        tests.check(!clientReplyInNetworkFor(state, 10, 1),
                    "follower without leader knowledge should not reply");
        ...
        state = stepRequired(
            ...
            requireTimerEvent(
                ...
                "consensus client retry timer"),
            settings);
        state = deliverConsensusRequestAtLeaderToQuorum(
            state,
            settings,
            leader,
            client,
        ...
        const auto* leader_node =
            state.nodeAs<ConsensusReplica>(leader);
        tests.check(client_node != nullptr && client_node->done(),
                    "retrying consensus client should finish");
        tests.check(client_node != nullptr &&
                        client_node->resultsOk() &&
                        client_node->retryCount() == 1,
                    "client should complete after exactly one retry");
        tests.check(leader_node != nullptr &&
                        leader_node->executionCount(10, 1) == 1 &&
                        leader_node->lastNonEmpty() == 1,
                    "leader should execute the retried request once in one slot");
    });

    tests.test("Concurrent consensus clients observe one SQL order", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client1 = ScenarioAddress::client1();
        Address client2 = ScenarioAddress::client2();
        ...
        for (const auto& item :
             std::vector<std::pair<Address, int>>{
                 {client1, 21}, {client2, 22}, {client3, 23}}) {
            state = stepRequired(
                state,
                requireMessageEvent(
                    state,
                    ...
                    [&](const MessageEnvelope& envelope) {
                        const auto* request =
                            std::get_if<ClientRequest>(&envelope.message);
                        return request != nullptr &&
                               request->client_id == item.second &&
                               request->request_id == 1 &&
                               envelope.from.rootAddress() == item.first &&
                               envelope.to.rootAddress() == leader;
                    },
                    "concurrent client request at leader"),
                settings);
        }

        state = deliverConsensusAppendsForRequestToQuorum(
            state,
            settings,
            leader,
            client3,
                ...
                requireMessageEvent(
                    ...
                    [&](const MessageEnvelope& envelope) {
                        const auto* reply =
                            std::get_if<ClientReply>(&envelope.message);
                        return reply != nullptr &&
                               reply->client_id == item.second &&
                               reply->request_id == 1 &&
                               envelope.to.rootAddress() == item.first;
                    },
                    "concurrent client reply"),
                settings);
        }

        for (const auto& client :
             std::vector<Address>{client1, client2, client3}) {
            const auto* node = state.nodeAs<ConsensusClientWorker>(client);
            tests.check(node != nullptr &&
                            node->done() &&
                            node->resultsOk(),
                        client.str() +
                            " should finish with its expected SQL result");
        }
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(node != nullptr &&
    ...
    tests.test("Duplicate client request does not create a second slot", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state.send(client, leader, ClientRequest{11, 1, command});
        state.send(client, leader, ClientRequest{11, 1, command});
        ...
        tests.check(leader_node != nullptr &&
                    ...
                    "duplicate pending request should share the first slot");
        state = deliverConsensusAppendsForRequestToQuorum(
            state,
            settings,
            leader,
        ...
        leader_node = state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->executionCount(11, 1) == 1 &&
                        leader_node->lastNonEmpty() == 1,
                    "duplicate request should execute once after commit");
    });

    tests.test("Append retry completes after undelivered append wave", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                "client request before append retry"),
            settings);
        const auto* leader_node =
            state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->commitIndex() == 0 &&
                        !clientReplyInNetworkFor(state, 12, 1),
                    "leader should wait because no append reply was delivered");

        uint64_t last_message_before_retry = 0;
        for (const auto& envelope : state.network()) {
            last_message_before_retry =
                std::max(last_message_before_retry, envelope.id);
        }
        int current_retry_generation = 0;
        auto timers_for_leader = state.timers().find(leader.rootAddress());
        if (timers_for_leader != state.timers().end()) {
            for (const auto& timer : timers_for_leader->second) {
                if (const auto* retry =
                    ...
                    current_retry_generation =
                        std::max(current_retry_generation,
                                 retry->generation);
                }
            }
        }
        tests.check(current_retry_generation > 0,
                    "leader should have a current append retry timer");
        state = stepRequired(
            state,
            requireTimerEvent(
                state,
                ...
                [&](const TimerEnvelope& timer) {
                    const auto* retry =
                        std::get_if<AppendRetryTimer>(&timer.timer);
                    return timer.to.rootAddress() == leader &&
                           retry != nullptr &&
                           retry->generation == current_retry_generation;
                },
                "leader append retry timer"),
            settings);
        state = deliverConsensusAppendsForRequestToQuorum(
            state,
            settings,
        ...
        tests.check(leader_node != nullptr &&
                        ...
                        leader_node->executionCount(12, 1) == 1,
                    "retry append should replicate, commit, and execute once");
        tests.check(clientReplyInNetworkFor(state, 12, 1),
                    "client reply should be produced after append retry");
    });

    tests.test("Old leader isolated from majority cannot commit", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        ...
        isolated.partition({
            {old_leader, client},
            {replicas[1], replicas[2], replicas[3], replicas[4]}
        });
            ...
            requireMessageEvent(
                ...
                [&](const MessageEnvelope& envelope) {
                    const auto* request =
                        std::get_if<ClientRequest>(&envelope.message);
                    return request != nullptr &&
                           request->client_id == 31 &&
                           request->request_id == 1 &&
                           envelope.from.rootAddress() == client &&
                           envelope.to.rootAddress() == old_leader;
                },
                "isolated old-leader client request"),
            isolated);
        ...
        tests.check(old_node != nullptr &&
                        ...
                        old_node->executionCount(31, 1) == 0,
                    "isolated old leader should only have an uncommitted entry");
        tests.check(!clientReplyInNetworkFor(state, 31, 1),
                    "isolated old leader should not reply before majority");

        SearchSettings search = isolated;
        search.max_depth = 4;
        search.max_states = 500;
        search.deliverTimers(false);
        search.invariants.push_back({
            "NO_OLD_LEADER_COMMIT",
            [replicas](const SearchState& candidate) {
                return noConsensusProgressFor(candidate, replicas, 31, 1);
            }
        });
        search.invariants.push_back({
            "LOGS_CONSISTENT",
            [replicas](const SearchState& candidate) {
                return consensusLogsConsistentAllSlots(candidate, replicas);
            }
        });
        SearchResults result = bfs(state, search);
        tests.check(result.end_condition ==
                        SearchResults::EndCondition::SpaceExhausted,
                    "isolated old leader search should exhaust safely, got " +
                        endConditionName(result.end_condition) +
                        ": " + result.message);
    });

    tests.test("New majority elects leader and old leader steps down", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address old_leader = replicas[0];
        ...
        majority_partition.partition({
            {old_leader},
            {new_leader, replicas[2], replicas[3], replicas[4]}
        });
        state = electConsensusLeader(state, majority_partition, new_leader,
                                     {replicas[2], replicas[3]});

        const auto* old_node = state.nodeAs<ConsensusReplica>(old_leader);
        const auto* new_node = state.nodeAs<ConsensusReplica>(new_leader);
        tests.check(old_node != nullptr &&
                        old_node->role() == ConsensusRole::Leader &&
                        old_node->currentTerm() == 1,
                    "isolated old leader should still think it leads term 1");
        tests.check(new_node != nullptr &&
                        new_node->role() == ConsensusRole::Leader &&
                        new_node->currentTerm() == 2,
                    "majority side should elect a term-2 leader");
        ...
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                [&](const MessageEnvelope& envelope) {
                    ...
                    return append != nullptr &&
                           append->term == 2 &&
                           append->leader == new_leader &&
                           envelope.from.rootAddress() == new_leader &&
                           envelope.to.rootAddress() == old_leader;
                },
                "higher-term append to old leader"),
            healed);
        old_node = state.nodeAs<ConsensusReplica>(old_leader);
        tests.check(old_node != nullptr &&
                        old_node->role() == ConsensusRole::Follower &&
                        old_node->currentTerm() == 2 &&
                        old_node->leader() == new_leader,
                    "old leader should step down after seeing higher term");
    });

    tests.test("Client retries across leader change and completes once", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address old_leader = replicas[0];
        Address new_leader = replicas[1];
        Address client = ScenarioAddress::client1();
        ...
        state.addNode(std::make_unique<ConsensusClientWorker>(
            client,
            std::vector<Address>{old_leader, new_leader, replicas[2]},
            41,
            Workload(
                std::vector<Command>{createTitleTableCommand()},
                std::vector<Result>{Result{CreateTableOkResult{}}})));
            ...
            requireMessageEvent(
                ...
                [&](const MessageEnvelope& envelope) {
                    const auto* request =
                        std::get_if<ClientRequest>(&envelope.message);
                    return request != nullptr &&
                           request->client_id == 41 &&
                           request->request_id == 1 &&
                           envelope.from.rootAddress() == client &&
                           envelope.to.rootAddress() == old_leader;
                },
                "client request to isolated old leader"),
            first_partition);
        tests.check(!clientReplyInNetworkFor(state, 41, 1),
                    "client should not complete on isolated old leader");

        SearchSettings second_partition;
        second_partition.partition({
            {old_leader},
            {new_leader, replicas[2], replicas[3], replicas[4], client}
        });
        ...
        state = stepRequired(
            ...
            requireTimerEvent(
                ...
                "client retry after leader change"),
            second_partition);
        state = deliverConsensusRequestAtLeaderToQuorum(
            state,
            second_partition,
        ...
        const auto* client_node = state.nodeAs<ConsensusClientWorker>(client);
        const auto* new_node = state.nodeAs<ConsensusReplica>(new_leader);
        const auto* old_node = state.nodeAs<ConsensusReplica>(old_leader);
        tests.check(client_node != nullptr &&
                        client_node->done() &&
                        client_node->resultsOk() &&
                        client_node->retryCount() == 1,
                    "client should complete after one retry to the new leader");
        tests.check(new_node != nullptr &&
                        new_node->executionCount(41, 1) == 1 &&
                        new_node->commitIndex() == 1,
                    "new leader should execute the retried request once");
        tests.check(old_node != nullptr &&
                        old_node->executionCount(41, 1) == 0,
                    "old isolated leader should not apply the stale request");
    });

    tests.test("Old leader catches up after rejoining new majority", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address old_leader = replicas[0];
        Address new_leader = replicas[2];
        Address client1 = ScenarioAddress::client1();
        ...
        SearchSettings first_partition;
        first_partition.partition({
            {old_leader, replicas[1], new_leader, client1},
            {replicas[3], replicas[4], client2}
        });
        SearchState state = consensusClusterState(replicas);
        ...
        SearchSettings second_partition;
        second_partition.partition({
            {old_leader, replicas[1]},
            {new_leader, replicas[3], replicas[4], client2}
        });
        state = electConsensusLeader(state, second_partition, new_leader,
                                     {replicas[3], replicas[4]});
        state = catchUpConsensusFollowerThrough(state, second_partition,
        ...
        SearchSettings healed;
        state = fireConsensusAppendRetry(state, healed, new_leader);
        state = stepRequired(
            state,
            requireMessageEvent(
                state,
    ...
    tests.test("Concurrent clients during repartition remain linearizable", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address old_leader = replicas[0];
        Address new_leader = replicas[2];
        Address client1 = ScenarioAddress::client1();
        ...
        first_partition.partition({
            {old_leader, replicas[1], new_leader, client1},
            {replicas[3], replicas[4], client2, client3}
        });
        SearchState state = consensusClusterState(replicas);
        state = electConsensusLeader(state, first_partition, old_leader,
                                     {replicas[1], new_leader});
        state = deliverConsensusCommandToQuorum(
            state, first_partition, old_leader, client1, 61, 1,
            commands[0], {replicas[1], new_leader});

        SearchSettings second_partition;
        second_partition.partition({
            {old_leader, replicas[1]},
            {new_leader, replicas[3], replicas[4], client2, client3}
        });
        state = electConsensusLeader(state, second_partition, new_leader,
                                     {replicas[3], replicas[4]});
        state = catchUpConsensusFollowerThrough(state, second_partition,
                                                new_leader, replicas[3], 1);
        state = catchUpConsensusFollowerThrough(state, second_partition,
                                                new_leader, replicas[4], 1);
        state.addNode(std::make_unique<ConsensusClientWorker>(
            client2,
        ...
        for (const auto& item :
            ...
            state = stepRequired(
                ...
                requireMessageEvent(
                    ...
                    "repartitioned concurrent client request"),
                second_partition);
        }
        state = deliverConsensusAppendsForRequestToQuorum(
            state,
        ...
        for (const auto& item :
            ...
            state = stepRequired(
                ...
                requireMessageEvent(
                    ...
                    "repartitioned concurrent client reply"),
                second_partition);
        }
    ...
    tests.test("Bounded trace with two clients and changing partitions", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(5);
        Address first_leader = replicas[0];
        Address second_leader = replicas[2];
        Address client1 = ScenarioAddress::client1();
        ...
        SearchSettings first_partition;
        first_partition.partition({
            {first_leader, replicas[1], second_leader, client1},
            {replicas[3], replicas[4], client2}
        });
        initial = electConsensusLeader(initial, first_partition,
                                       first_leader,
                                       {replicas[1], second_leader});
        initial = deliverConsensusCommandToQuorum(
            initial,
            first_partition,
            first_leader,
            client1,
        ...
        SearchState switched = initial;
        SearchSettings second_partition;
        second_partition.partition({
            {first_leader, replicas[1]},
            {second_leader, replicas[3], replicas[4], client2}
        });
        switched = electConsensusLeader(switched, second_partition,
                                        second_leader,
                                        {replicas[3], replicas[4]});
        switched = catchUpConsensusFollowerThrough(switched,
                                                   second_partition,
                                                   second_leader,
                                                   replicas[3],
                                                   1);
        switched = catchUpConsensusFollowerThrough(switched,
                                                   ...
                                                   1);
        SearchState compact;
        std::vector<Address> compact_replicas{
            second_leader, replicas[3], replicas[4]};
        for (const auto& replica : compact_replicas) {
            const auto* node = switched.nodeAs<ConsensusReplica>(replica);
            if (node == nullptr) {
                throw std::runtime_error("missing compact replica " +
                                         replica.str());
            }
            compact.addNode(node->clone());
        }
        compact.addNode(std::make_unique<ConsensusClientWorker>(
            client2,
            std::vector<Address>{second_leader, replicas[3], replicas[4]},
            72,
            Workload(
                std::vector<Command>{CountRowsCommand{"title"}},
                std::vector<Result>{Result{CountRowsResult{0}}})));

        SearchSettings second_search;
        second_search.partition({
            {second_leader, replicas[3], replicas[4], client2}
        });
        second_search.max_depth = 20;
        second_search.max_states = 1200;
        second_search.deliverTimers(false);
        second_search.invariants.push_back({
            "AT_MOST_ONE_LEADER_PER_TERM",
            [compact_replicas](const SearchState& candidate) {
                return atMostOneConsensusLeaderPerTerm(candidate,
                                                       compact_replicas);
            }
        });
        second_search.invariants.push_back({
            "LOGS_CONSISTENT",
            [compact_replicas](const SearchState& candidate) {
                return consensusLogsConsistentAllSlots(candidate,
                                                       compact_replicas);
            }
        });
        second_search.goals.push_back({
            "SECOND_CLIENT_REPLY_IN_NETWORK",
            [](const SearchState& candidate) {
                return CheckResult{clientReplyInNetworkFor(candidate, 72, 1),
                                   "second client reply is not in the network"};
            }
        });

        SearchState direct = deliverConsensusRequestAtLeaderToQuorum(
            compact,
            second_search,
            second_leader,
            client2,
            ...
            1,
            {replicas[3], replicas[4]});
        CheckResult direct_done = consensusClientDone(direct, client2);
        tests.check(direct_done.ok,
                    "direct schedule should complete before bounded search: " +
                        direct_done.message);
        tests.check(direct.depth() <= second_search.max_depth,
                    "direct schedule should fit inside the bounded depth");
        tests.check(direct.depth() > 5,
                    "direct schedule should cover a real replication path");

        SearchResults second_result =
            replayTrace(compact, second_search, direct.history());
        tests.check(second_result.end_condition ==
                        SearchResults::EndCondition::GoalFound,
                    "second partition trace should finish count, got " +
                        endConditionName(second_result.end_condition) +
                        ": " + second_result.message);
    });

    tests.test("Random search preserves Raft safety invariants", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address client1 = ScenarioAddress::client1();
        Address client2 = ScenarioAddress::client2();
        SearchState state = consensusClusterState(replicas);
        ...
        search.max_depth = 40;
        search.max_states = 500;
        search.random_dfs_probes = 25;
        search.invariants.push_back({
            "AT_MOST_ONE_LEADER_PER_TERM",
            [replicas](const SearchState& candidate) {
                return atMostOneConsensusLeaderPerTerm(candidate, replicas);
            }
        });
        search.invariants.push_back({
            "LOGS_CONSISTENT",
        ...
        search.invariants.push_back({
            ...
            [client1, client2](const SearchState& candidate) {
                for (const auto& client : std::vector<Address>{client1, client2}) {
                    const auto* worker =
                        candidate.nodeAs<ConsensusClientWorker>(client);
                    if (worker == nullptr || !worker->resultsOk()) {
                        return CheckResult{
                            false,
                            client.str() + " has invalid result prefix"};
                    }
                }
        ...
        search.invariants.push_back({
            ...
            [replicas](const SearchState& candidate) {
                for (const auto& replica : replicas) {
                    const auto* node =
                        candidate.nodeAs<ConsensusReplica>(replica);
                    if (node == nullptr) {
                        return CheckResult{false,
                                           replica.str() + " is missing"};
                    }
                    if (node->executionCount(91, 1) > 1 ||
                        node->executionCount(92, 1) > 1) {
                        return CheckResult{
                            false,
                            replica.str() + " applied a request twice"};
                    }
                }
                return CheckResult{true, ""};
            }
        ...
        tests.check(result.end_condition !=
                    ...
                    "random search should preserve safety, got " +
                        endConditionName(result.end_condition) +
                        ": " + result.message);
