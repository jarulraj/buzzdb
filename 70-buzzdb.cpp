...
public:
        ...
        switch (statement.kind) {
            ...
            case ParsedStatement::Kind::Insert: {
                ...
                acquireLock(txn,
                            ...
                            tableResourceLabel(statement.tableName));
                return;
            }
            case ParsedStatement::Kind::Select: {
                auto& metadata = catalog.getTable(statement.query.tableName);
                ...
                acquireLock(txn,
                            LockMode::S,
                            tableResource(metadata),
                            tableResourceLabel(statement.query.tableName));
                return;
            }
        }
    }
    ...
    void acquireLock(const TxnPtr& txn,
        ...
        std::string requestLabel = upgrade
            ? "S->X upgrade"
...
int main() {
    ...
    std::cout << "S-to-X lock upgrade deadlock during read-then-write booking\n";

    BuzzDB db;
    DatabaseImporter::importFile(db, "booking.txt");

    std::cout << "\nInitial committed state of seat 1B:\n";
    ...
    std::cout << "\nBoth transactions read seat 1B and hold compatible S locks.\n";
    db.execute(txn1, "SELECT {*} FROM seats WHERE seat_no = 1B");
    db.execute(txn2, "SELECT {*} FROM seats WHERE seat_no = 1B");
    ...
    std::cout << "\nTxn " << txn2->id
              << " also decides to book 1B and requests its own S->X upgrade.\n";
    try {
        ...
        db.execute(txn2, "INSERT INTO holds VALUES (2, 2, patel, open)");
        db.commit(txn2);
        throw std::runtime_error("Expected a deadlock when txn 2 requested its upgrade.");
    ...
    std::cout << "\nResult: the waits-for graph handled an S-to-X upgrade deadlock, "
              << "aborted one upgrader, and let the survivor commit the seat plus hold row.\n";
