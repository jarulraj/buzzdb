...
constexpr uint16_t BUZZDB_VERSION = 70;
...
void RecoveryManager::recover() {
    ...
    auto collectWalRecord = [&](const std::string& record) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v70."
            );
        }
        if (txn_id > 0) {
...
public:
    ...
    bool hasSharedLock(int txn_id, const std::string& resource) {
        std::lock_guard<std::mutex> guard(latch);
        return hasSharedHolder(locks[resource], txn_id);
    }
    ...
    bool releaseAll(int txn_id) {
        ...
        for (auto& entry : locks) {
            ...
            auto old_size = state.shared_holders.size();
            removeSharedHolder(state, txn_id);
            released = released || old_size != state.shared_holders.size();
        }
        if (released) {
            lock_cv.notify_all();
        }
...
private:
    ...
    static bool hasOtherSharedHolder(const LockState& state, int txn_id) {
        return std::any_of(
            state.shared_holders.begin(),
            state.shared_holders.end(),
            [txn_id](int holder) { return holder != txn_id; }
        );
    }

    static bool hasSharedHolder(const LockState& state, int txn_id) {
        return std::find(state.shared_holders.begin(),
                         state.shared_holders.end(),
                         txn_id) != state.shared_holders.end();
    }

    static void removeSharedHolder(LockState& state, int txn_id) {
        state.shared_holders.erase(
            std::remove(state.shared_holders.begin(),
                        state.shared_holders.end(),
                        txn_id),
            state.shared_holders.end()
        );
    ...
    static std::string sharedHolderLabel(const LockState& state, int txn_id) {
        std::string label;
        for (int holder : state.shared_holders) {
            if (holder == txn_id) {
                continue;
            ...
            label += "T" + std::to_string(holder);
        }
        return label;
    }

    static std::string incompatibleHolderLabel(const LockState& state, int txn_id) {
        if (state.exclusive_holder != 0 && state.exclusive_holder != txn_id) {
            return "X lock is held by T" + std::to_string(state.exclusive_holder);
        }
        if (hasOtherSharedHolder(state, txn_id)) {
            return "S lock is held by " + sharedHolderLabel(state, txn_id);
        }
...
public:
    ...
    bool acquireTxnLock(const TxnPtr& txn,
        ...
        logCC(txnLabel(txn) + " inferred " + mode + " lock on " +
              resource + " for " + reason);
        logCC(txnLabel(txn) + " requests " + mode + " lock on " + resource);

        auto result = mode == "S" ?
            cc_lock_manager.waitForShared(
                txn->id, resource, std::chrono::milliseconds(200)
            ) :
            cc_lock_manager.waitForExclusive(
                txn->id, resource, std::chrono::milliseconds(200)
            );

        if (result.granted) {
            logCC(txnLabel(txn) + " " + mode +
        ...
        logCC(txnLabel(txn) + " " + mode + " lock on " + resource +
    ...
    bool acquireQueryLocks(const TxnPtr& txn,
        ...
        if (!acquireTxnLock(txn, "S", resource, reason)) {
            return false;
        }
    ...
    void executeStatement(const std::string& statement,
        ...
        if (components.type == StatementType::INSERT) {
            ...
            if (recovery_manager.isActive()) {
                throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v70.");
            }
...
void runTransactionLocking() {
    ...
    std::cout << "\nTransaction execution with automatic CC locks" << std::endl;
    std::cout << "  BuzzDB infers and acquires locks." << std::endl;
    std::cout << "  T1 and T2 run in separate threads and both try seat 1B." << std::endl;
    std::cout << "  The storage calls are serialized, but lock requests happen concurrently." << std::endl;
    ...
    auto printFinalState = [&]() {
        auto seat_status = readValue(nullptr, "seats", "seat_no", "1B", "status");
        auto seat_customer = readValue(nullptr, "seats", "seat_no", "1B", "customer");
        auto hold_1_customer = readValue(nullptr, "holds", "id", "1", "customer");
        auto hold_2_customer = readValue(nullptr, "holds", "id", "2", "customer");

        std::cout << "  Final booking tuples:" << std::endl;
        std::cout << "    seats[seat_no=1B].status=" << seat_status.first
                  << ", customer=" << seat_customer.first
                  << " at " << tupleIdLabel(seat_status.second) << std::endl;
        std::cout << "    holds[id=1].customer=" << hold_1_customer.first
                  << " at " << tupleIdLabel(hold_1_customer.second) << std::endl;
        std::cout << "    holds[id=2].customer=" << hold_2_customer.first
                  << " at " << tupleIdLabel(hold_2_customer.second) << std::endl;
    };

    auto bookSeat = [&](const std::string& txn_label,
                        int hold_id,
                        const std::string& customer,
                        bool wait_before_upgrade) {
        ...
        waitForBothReads();
        if (txn_label == "T1") {
            print("  Both transactions read seat 1B before either takes X.");
        }
        if (wait_before_upgrade) {
            std::this_thread::sleep_for(std::chrono::milliseconds(20));
        }

        if (seat_status.first != "available") {
            print("  " + txn_label + " sees seat 1B is not available; no hold is created");
            db.commit(txn);
            return;
        }
    ...
    std::thread txn1_worker([&]() {
        bookSeat("T1", 1, "garcia", false);
    });

    std::this_thread::sleep_for(std::chrono::milliseconds(10));

    std::thread txn2_worker([&]() {
        bookSeat("T2", 2, "patel", true);
    });
