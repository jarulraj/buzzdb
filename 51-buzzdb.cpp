        ...
        std::stable_sort(tuples.begin(), tuples.end(),
                         [&](const auto& left, const auto& right) {
                             for (auto attr_index : sort_attrs) {
                                 if (attr_index >= left.size() || attr_index >= right.size()) {
...
enum class JoinAlgorithm { HASH_JOIN, SORT_MERGE_JOIN, NESTED_LOOP_JOIN };

std::string joinOperatorName(JoinAlgorithm join_algorithm) {
...
std::string operatorTreeString(const QueryComponents& components,
                               JoinAlgorithm join_algorithm = JoinAlgorithm::HASH_JOIN) {
    std::string tree = "Scan(" + components.tableName + ")";
    for (const auto& join : components.joins) {
        tree = joinOperatorName(join_algorithm) + "(" + tree + ", Scan(" + join.tableName + "))";
    }
...
void executeQuery(const QueryComponents& components,
    ...
    for (const auto& join : components.joins) {
        ...
        if (join_algorithm == JoinAlgorithm::HASH_JOIN) {
        ...
        } else if (join_algorithm == JoinAlgorithm::SORT_MERGE_JOIN) {
    ...
    while (rootOp->next()) {
        output_tuple_count++;
        if (print_tuples || output_tuple_count <= sample_tuple_count) {
            if (!print_tuples && output_tuple_count == 1) {
                std::cout << "Sample rows (company_type_id, company_id, title_id first):"
                          << std::endl;
            }
...
public:
    ...
    bool next() override {
        ...
        for (PageID pageId : tableHeap.getPageIds()) {
            auto& page = tableHeap.getPage(pageId);
            char* pageBuffer = page->page_data.get();
            Slot* slotArray = reinterpret_cast<Slot*>(pageBuffer);
            bool pageUpdated = false;

            for (size_t slotId = 0; slotId < MAX_SLOTS; slotId++) {
                if (slotArray[slotId].empty) {
                    continue;
                }

                assert(slotArray[slotId].offset != INVALID_VALUE);
                const char* tupleData = pageBuffer + slotArray[slotId].offset;
                std::istringstream iss(
                    std::string(tupleData, slotArray[slotId].length)
                );
...
public:
    ...
    void loadImdbFile(const std::string& filename,
        ...
        while (std::getline(inputFile, line)) {
            ...
            if (separator == std::string::npos) {
                throw std::runtime_error("Malformed IMDb row" + lineContext(line_number));
            }
    ...
    void executeQuery(const std::string& query,
        ...
        std::cout << "Operator Tree: " << operatorTreeString(components, join_algorithm) << std::endl;
    ...
    void executeTimedJoinQuery(const std::string& label,
                               JoinAlgorithm join_algorithm,
                               const TxnPtr& txn = nullptr,
                               size_t sample_tuple_count = 0) {
        const std::string join_query =
            "PROJECT {movie_companies.company_type_id}, "
            "{movie_companies.company_id}, {title.id}, "
            "{title.title}, {company_name.name}, {company_type.kind}, "
        ...
        executeQuery(join_query, txn, join_algorithm, false, sample_tuple_count);
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed = end - start;
        std::cout << label << " join elapsed time: "
                  << elapsed.count() << " seconds" << std::endl;
    }

    void executeQueries(const TxnPtr& txn = nullptr) {
        executeTimedJoinQuery("HashJoin", JoinAlgorithm::HASH_JOIN, txn);
        executeTimedJoinQuery("SortMergeJoin", JoinAlgorithm::SORT_MERGE_JOIN, txn, 8);
        executeTimedJoinQuery("NestedLoopJoin", JoinAlgorithm::NESTED_LOOP_JOIN, txn);
    }
...
int main(int argc, char* argv[]) {
    ...
    BuzzDB db;
    std::string imdb_filename = argc > 1 ? argv[1] : "imdb_medium.txt";
    ...
    if (seed_database) {
        db.loadImdbFile(imdb_filename, txn1);
    }
