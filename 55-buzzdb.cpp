...
// Recovery policy for v55: copy-on-write shadow pages.
...
private:
    ...
    size_t committed_data_pages_copied = 0;
...
public:
    ...
    void simulateCrashBeforeCatalogRootSwitch() { crash_before_catalog_root_switch = true; }
    void begin();
    void commit();
    void abort();
    void collectGarbage();
...
void RecoveryManager::commit() {
    ...
    for (const auto& entry : shadow_page_table) {
        ...
        updated_metadata[table_id] = metadata;
    }

    std::vector<TableMetadata> updates;
    for (const auto& entry : updated_metadata) {
        updates.push_back(entry.second);
    }
    size_t catalog_pages_copied = catalog.persistTableMetadataAtomically(
        updates, crash_before_catalog_root_switch
    );
    if (shadow_pages_copied != 0 || catalog_pages_copied != 0) {
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside a shadow transaction is not supported in v55.");
        }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v55 booking demo cannot open a buzzdb.dat containing "
