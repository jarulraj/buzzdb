...
void printLogicalExplanation(const QueryComponents& components) {
    ...
    std::cout << "  Written join order: "
              ...
              << std::endl;
    std::cout << "\nCurrent physical plan:" << std::endl;
    std::cout << "  " << operatorTreeString(components) << std::endl;
}
...
QueryTable executeQuery(const QueryComponents& components,
    ...
    if (!components.filters.empty() || !components.columnEqualities.empty()) {
        auto filterPredicate = std::make_unique<ComplexPredicate>(ComplexPredicate::LogicOperator::AND);
        for (const auto& filter : components.filters) {
            auto offset_it = table_offsets.find(filter.column.tableName);
            if (offset_it == table_offsets.end()) {
                throw std::runtime_error("WHERE filter table is not in the query output.");
            }
            auto& metadata = catalog.getTable(
                actualTableName(components, filter.column.tableName)
            );
            ...
            filterPredicate->addPredicate(std::make_unique<SimplePredicate>(
                SimplePredicate::Operand(
                    offset_it->second + static_cast<size_t>(filter.column.attributeIndex)
                ),
...
int main(int argc, char* argv[]) {
    ...
    auto rows = db.executeQuery(jobJoinQuery(), query_txn, false);
    ...
    std::cout << "  Query: " << elapsedMs(query_start, query_end)
              << " ms" << std::endl;
