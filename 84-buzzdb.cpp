...
const std::string imdb_join_comparison_query =
...
void runImdbJoinQuery(BuzzDB& db, PhysicalJoinKind join_kind) {
    ...
    db.execute(join_reader, imdb_join_comparison_query);
...
void runImdbJoinComparison() {
    ...
    runImdbJoinQuery(db, PhysicalJoinKind::NestedLoopJoin);
    runImdbJoinQuery(db, PhysicalJoinKind::HashJoin);

    std::cout << "\nResult: Same SQL and same rows; query time changes "
              << "with the physical join algorithm.\n";
}

int main() {
    try {

    std::cout << "IMDB nested-loop join vs hash join\n";

    runImdbJoinComparison();
