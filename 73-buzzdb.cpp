...
void RecoveryManager::recover() {
    ...
    auto collectWalRecord = [&](const std::string& record) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v73."
            );
        }
        if (txn_id > 0) {
...
public:
    ...
    bool usesPredicateLocksForPhantoms() const {
        // v73 models locking isolation levels by lock duration.
    ...
    std::vector<std::string> rowLockResourcesForEquality(
    ...
    bool acquireQueryLocks(const TxnPtr& txn,
        ...
        if (components.equalityWhereCondition) {
            ...
            auto resources = rowLockResourcesForEquality(components, column_name);
            logCC(txnLabel(txn) + " tuple scan locks " +
                  std::to_string(resources.size()) +
                  " existing row(s) matching " + column_name + "=" +
                  components.equalityWhereValue);
            for (const auto& row_resource : resources) {
                if (!acquireTxnLock(txn, LockMode::S, row_resource, reason)) {
                    return false;
                }
            }
            return true;
    ...
    void releaseReadLocksAfterStatement(const TxnPtr& txn) {
        ...
        if (txn && isolation_level == IsolationLevel::ReadCommitted &&
            cc_lock_manager.releaseByMode(txn->id, LockMode::S)) {
            logCC(txnLabel(txn) +
    ...
    void executeStatement(const std::string& statement,
        ...
        if (components.type == StatementType::INSERT) {
            ...
            if (recovery_manager.isActive()) {
                throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v73.");
            }
...
void resetBookingRows(BuzzDB& db) {
    ...
    resetSeat(db, 3, "1C");
    resetSeat(db, 4, "1D");
    if (rowExists(db, "seats", "seat_no", "3A")) {
        executeQuiet(
            db,
            "UPDATE seats SET status=held, customer=old WHERE seat_no=3A"
        );
    }
}
...
void runTransactionLocking() {
    ...
    std::cout << "\nIsolation levels with inferred locks" << std::endl;
    std::cout << "  READ UNCOMMITTED takes no read locks." << std::endl;
    std::cout << "  READ COMMITTED releases read locks after each statement." << std::endl;
    std::cout << "  REPEATABLE READ keeps row read locks until commit." << std::endl;
    std::cout << "  Lower levels use only row S/X locks." << std::endl;
    std::cout << "  SERIALIZABLE adds predicate locks only for phantoms." << std::endl;

    auto print = [&](const std::string& line) {
        db.logCC(line);
    };

    auto printIsolation = [&](IsolationLevel level,
                              const std::string& purpose) {
        std::cout << "\nIsolation level: "
                  << BuzzDB::isolationLevelName(level) << std::endl;
        std::cout << "  Purpose: " << purpose << std::endl;
        db.setIsolationLevel(level);
        db.resetLockStats();
    };

    auto querySeat = [&](const TxnPtr& txn,
    ...
    auto joinSeats = [](const std::vector<std::string>& seats) {
        std::string joined;
    ...
    auto containsSeat = [](const std::vector<std::string>& seats,
                           const std::string& seat_no) {
        return std::find(seats.begin(), seats.end(), seat_no) != seats.end();
    };

    auto scanAvailable = [&](const TxnPtr& txn,
        ...
        for (const auto& row : rows) {
            if (!row.fields.empty()) {
                seats.push_back(fieldToString(*row.fields[0]));
            }
        }
        std::sort(seats.begin(), seats.end());
        print("  " + label + " sees " + std::to_string(seats.size()) +
              " available seat(s): " + joinSeats(seats));
        return seats;
    };

    auto runPhantomCase = [&](const std::string& label_prefix,
        ...
        std::thread scanner([&]() {
            ...
            print("  " + label_prefix +
                  "1 first scan: PROJECT available seats");
            auto first_scan = scanAvailable(txn, label_prefix + "1 first scan");
            std::this_thread::sleep_for(std::chrono::milliseconds(150));
            print("  " + label_prefix +
                  "1 second scan repeats the same predicate");
            auto second_scan = scanAvailable(txn, label_prefix + "1 second scan");
            bool phantom_seen =
                !containsSeat(first_scan, "3A") &&
                containsSeat(second_scan, "3A");
            print("  " + label_prefix +
                  "1 phantom observed: " +
                  (phantom_seen ? "yes" : "no"));
            db.commit(txn);
        ...
        std::thread inserter([&]() {
            ...
            print("  " + label_prefix +
                  "2 EXECUTE INSERT seats|9|1|3A|available|unassigned");
            db.execute(
                "INSERT seats|9|1|3A|available|unassigned",
                txn,
                false
            );
        ...
        scanner.join();
        inserter.join();
        db.printLockStats(stats_label);
    };

    resetBookingRows(db);
    printIsolation(
        IsolationLevel::ReadUncommitted,
        "dirty reads are possible because readers do not take S locks."
    );
    bool dirty_read = false;
    std::thread dirty_writer([&]() {
        auto txn = db.begin("RU1");
        print("  RU1 EXECUTE UPDATE seats SET status=held WHERE seat_no=1A");
        db.execute(
            "UPDATE seats SET status=held, customer=garcia WHERE seat_no=1A",
            txn,
            false
        );
        std::this_thread::sleep_for(std::chrono::milliseconds(150));
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
    std::thread dirty_reader([&]() {
        auto txn = db.begin("RU2");
        auto read = querySeat(txn, "1A");
        dirty_read = read.first == "held";
        print("  RU2 read 1A=" + read.first + "/" + read.second +
              " while RU1 was still active");
        db.commit(txn);
    });
    dirty_writer.join();
    dirty_reader.join();
    print("  Dirty read observed: " + std::string(dirty_read ? "yes" : "no"));
    db.printLockStats("read uncommitted");

    resetBookingRows(db);
    printIsolation(
        IsolationLevel::ReadCommitted,
        "non-repeatable reads are possible because S locks are short."
    );
    bool non_repeatable = false;
    std::thread rc_reader([&]() {
        auto txn = db.begin("RC1");
        auto first = querySeat(txn, "1A");
        print("  RC1 first read 1A=" + first.first + "/" + first.second);
        std::this_thread::sleep_for(std::chrono::milliseconds(150));
        auto second = querySeat(txn, "1A");
        print("  RC1 second read 1A=" + second.first + "/" + second.second);
        non_repeatable = first != second;
        db.commit(txn);
    });
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
    std::thread rc_writer([&]() {
        auto txn = db.begin("RC2");
        print("  RC2 EXECUTE UPDATE seats SET status=held WHERE seat_no=1A");
        db.execute(
    ...
    rc_reader.join();
    rc_writer.join();
    print("  Non-repeatable read observed: " +
          std::string(non_repeatable ? "yes" : "no"));
    db.printLockStats("read committed");

    resetBookingRows(db);
    printIsolation(
        IsolationLevel::RepeatableRead,
        "row S locks are held until commit, so the same row is stable."
    );
    bool repeatable_changed = false;
    std::thread rr_reader([&]() {
        auto txn = db.begin("RR1");
        auto first = querySeat(txn, "1A");
        print("  RR1 first read 1A=" + first.first + "/" + first.second);
        std::this_thread::sleep_for(std::chrono::milliseconds(150));
        auto second = querySeat(txn, "1A");
        print("  RR1 second read 1A=" + second.first + "/" + second.second);
        repeatable_changed = first != second;
        db.commit(txn);
    });
    std::this_thread::sleep_for(std::chrono::milliseconds(30));
    std::thread rr_writer([&]() {
        auto txn = db.begin("RR2");
        print("  RR2 EXECUTE UPDATE seats SET status=held WHERE seat_no=1A");
        db.execute(
    ...
    rr_writer.join();
    print("  Non-repeatable read observed: " +
          std::string(repeatable_changed ? "yes" : "no"));
    db.printLockStats("repeatable read row check");

    std::cout << "\nIsolation level: REPEATABLE READ" << std::endl;
    std::cout << "  Purpose: tuple locks still allow phantoms." << std::endl;
    std::cout << "  Watch for: RR2 inserts 3A while RR1 is still active." << std::endl;
    runPhantomCase(
        "RR",
        IsolationLevel::RepeatableRead,
        "repeatable read phantom check"
    );

    std::cout << "\nIsolation level: SERIALIZABLE" << std::endl;
    std::cout << "  Purpose: predicate locks prevent phantoms." << std::endl;
    std::cout << "  Watch for: S2 waits on X(predicate seats.status=available)." << std::endl;
    runPhantomCase(
        "S",
