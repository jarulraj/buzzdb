...
void runImdbJoinOrdering() {
    ...
    auto greedy_join_kinds =
        db.optimizer().physicalJoinKindsForPlan(greedy_search.plan);
    ...
    printTimedQueryResult("Greedy optimizer", greedy_result);
    printTimedQueryResult("Selinger left-deep DP", selinger_result);
    if (greedy_result.result.row_count != selinger_result.result.row_count) {
