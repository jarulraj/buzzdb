        ...
        while (currentPageIndex < buffer_manager.getNumPages()) {
            ...
            while (currentSlotIndex < MAX_SLOTS) {
                if (!slot_array[currentSlotIndex].empty) {
                    ...
                    // Text based hard-coded deserialization
                    ...
                    while(current_count < required_count){
                        integerField = "";
                        for (const char* p = tuple_data + character_count; 
                            *p != ' ' && *p != '\0'; ++p) {
                            character_count++;
                            integerField += *p;
                        }
...
int main() {
    ...
    db.queryInterpretation();

    auto end = std::chrono::high_resolution_clock::now();

    // Calculate and print the elapsed time
    std::chrono::duration<double> elapsed = end - start;
    std::cout << "Query Intepretation:: Elapsed time: " << 
    ...
    elapsed = end - start;
    std::cout << "Query Compilation:: Elapsed time: " <<
    std::chrono::duration_cast<std::chrono::microseconds>(elapsed).count() 
          << " microseconds" << std::endl;
