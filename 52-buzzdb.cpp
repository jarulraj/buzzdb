...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v52 booking demo cannot open a buzzdb.dat containing "
    ...
    if (seed_booking_tables) {
        db.loadDataFile(data_filename, txn1);
        db.executeStatement("INSERT holds|1|1|1|alice|active", txn1);
        db.executeStatement("INSERT holds|2|1|2|bob|active", txn1);
    }
