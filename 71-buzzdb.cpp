...
private:
    LockRequestResult tryAcquireLocked(int txn_id,
        ...
        LockMode effective_mode = mode;
        for (const auto& grant : grants) {
            if (grant.txn_id == txn_id) {
                effective_mode = coalescedMode(grant.mode, mode);
                break;
            }
        }

        auto blockers = blockersForLocked(txn_id, grants, effective_mode);
        if (!blockers.empty()) {
        ...
        for (auto& grant : grants) {
            if (grant.txn_id == txn_id) {
                grant.mode = effective_mode;
                result.granted = true;
                return result;
            }
        }

        grants.push_back({txn_id, effective_mode});
        result.granted = true;
        return result;
    }
...
private:
    ...
    void acquireLock(const TxnPtr& txn,
        ...
        bool tuple_upgrade = mode == LockMode::X &&
                             lock_manager.hasLock(txn->id, resource, LockMode::S);
        bool table_six_upgrade = mode == LockMode::IX &&
                                  lock_manager.hasLock(txn->id, resource, LockMode::S);
        bool covered_by_six = mode == LockMode::IX &&
                              lock_manager.hasLock(txn->id, resource, LockMode::SIX);
        std::string request_label = LockManager::modeName(mode);
        if (tuple_upgrade) {
            request_label = "S->X upgrade";
        } else if (table_six_upgrade) {
            request_label = "S+IX -> SIX";
        } else if (covered_by_six) {
            request_label = "IX covered by SIX";
        }
...
private:
    ...
    void executeSelect(const ParsedStatement& statement,
        ...
        ScanLockMode lock_mode = ScanLockMode::None;
        if (txn_manager && txn) {
            lock_mode = statement.query.equalityCondition
                ? ScanLockMode::Tuple
                : ScanLockMode::Table;
        }
