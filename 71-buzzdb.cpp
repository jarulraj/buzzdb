...
constexpr uint16_t BUZZDB_VERSION = 71;
...
void RecoveryManager::recover() {
    ...
    auto collectWalRecord = [&](const std::string& record) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v71."
            );
        }
        if (txn_id > 0) {
...
public:
    ...
    bool hasSharedLock(int txn_id, const std::string& resource) {
        std::lock_guard<std::mutex> guard(latch);
        return hasLockAtLeast(locks[resource], txn_id, LockMode::S);
    }
    ...
    LockRequestResult waitFor(int txn_id,
        ...
        auto mode_to_grant = coalescedMode(locks[resource], txn_id, mode);
        while (!canGrant(txn_id, resource, mode_to_grant)) {
            auto blockers = blockersFor(txn_id, resource, mode_to_grant);
            ...
            if (lock_cv.wait_for(guard, timeout) == std::cv_status::timeout) {
                ...
                result.reason = incompatibleHolderLabel(
                    locks[resource], txn_id, mode_to_grant
                );
                removeWaitEdges(txn_id);
                return result;
            }
        ...
        removeWaitEdges(txn_id);
        grantLock(locks[resource], txn_id, mode_to_grant);
        recordGrant(resource, mode_to_grant);
        result.granted = true;
        return result;
    }
...
private:
    ...
    static bool strongerOrEqual(LockMode held, LockMode requested) {
        ...
        return (held == LockMode::IX || held == LockMode::S) &&
               requested == LockMode::IS;
    }

    static bool hasExactLock(const LockState& state, int txn_id, LockMode mode) {
        return std::any_of(
            state.grants.begin(),
            state.grants.end(),
            [txn_id, mode](const LockGrant& grant) {
                return grant.txn_id == txn_id && grant.mode == mode;
            }
        );
    }

    static bool hasLockAtLeast(const LockState& state, int txn_id, LockMode mode) {
        return std::any_of(
            state.grants.begin(),
            state.grants.end(),
            [txn_id, mode](const LockGrant& grant) {
    ...
    static LockMode coalescedMode(const LockState& state,
                                  int txn_id,
                                  LockMode mode) {
        if ((mode == LockMode::IX &&
             hasExactLock(state, txn_id, LockMode::S)) ||
            (mode == LockMode::S &&
             hasExactLock(state, txn_id, LockMode::IX))) {
            return LockMode::SIX;
        }
        return mode;
    }
...
public:
    ...
    void setIntentionLocksEnabled(bool enabled) {
        use_intention_locks = enabled;
    }

    void resetLockStats() {
        cc_lock_manager.resetStats();
    }

    void printLockStats(const std::string& label) {
        auto stats = cc_lock_manager.getStats();
        std::cout << "  Lock count (" << label << "): "
                  << stats.granted_requests << " granted, "
                  << stats.table_locks << " table, "
                  << stats.row_locks << " row, "
    ...
    void commit(const TxnPtr& tx) {
        ...
        cc_lock_manager.releaseAll(tx->id);
        logCC(txnLabel(tx) + " COMMIT; release locks");
    }

    void abort(const TxnPtr& tx) {
        txn_manager.abort(*tx);
        cc_lock_manager.releaseAll(tx->id);
        logCC(txnLabel(tx) + " ABORT; release locks");
    }
    ...
    static std::string tableResource(const std::string& table_name) {
    ...
    std::vector<std::string> rowLockResourcesForTable(
        const std::string& table_name) {
        auto& metadata = catalog.getTable(table_name);
        ...
        while (scan.next()) {
            ...
            resources.push_back(
                rowResource(
                    table_name,
                    column_name,
                    fieldToString(*fields[static_cast<size_t>(column_index)])
                )
            );
        }
        scan.close();
        ...
        resources.erase(
            std::unique(resources.begin(), resources.end()),
            resources.end()
        );
        return resources;
    }
    ...
    void logCC(const std::string& line) {
        std::lock_guard<std::mutex> guard(cc_output_latch);
    ...
    bool acquireTxnLock(const TxnPtr& txn,
        ...
        logCC(txnLabel(txn) + " inferred " + mode_name + " lock on " +
              resource + " for " + reason);
        logCC(txnLabel(txn) + " requests " + mode_name + " lock on " + resource);

        auto result = cc_lock_manager.waitFor(
            txn->id, resource, mode, std::chrono::milliseconds(500)
        );

        if (result.granted) {
            logCC(txnLabel(txn) + " " + mode_name +
                  " lock on " + resource + " granted");
            return true;
        }

        if (result.deadlock) {
            logCC("deadlock detected; waits-for graph cycle: " +
                  txnPathLabel(result.cycle));
            abort(txn);
            return false;
        }

        logCC(txnLabel(txn) + " " + mode_name + " lock on " + resource +
    ...
    bool acquireStatementLocks(const TxnPtr& txn,
        ...
        if (components.type == StatementType::UPDATE) {
            auto table_resource = tableResource(components.tableName);
            if (use_intention_locks) {
                auto table_mode = cc_lock_manager.hasSharedLock(
                    txn->id, table_resource
                ) ? LockMode::SIX : LockMode::IX;
                if (!acquireTxnLock(
                        txn,
                        table_mode,
                        table_resource,
                        "UPDATE " + components.tableName + " table intent"
                    )) {
                    return false;
                }
            } else {
                logCC(txnLabel(txn) +
                      " uses row-only lock path for UPDATE " +
    ...
    bool acquireRowLocksForFullScan(const TxnPtr& txn,
                                    const std::string& table_name,
                                    const std::string& reason) {
        auto resources = rowLockResourcesForTable(table_name);
        logCC(txnLabel(txn) + " row-only scan must lock " +
              std::to_string(resources.size()) + " row(s) in " + table_name);
        for (const auto& resource : resources) {
            if (!acquireTxnLock(txn, LockMode::S, resource, reason)) {
                return false;
            }
        }
        return true;
    ...
    bool acquireQueryLocks(const TxnPtr& txn,
        ...
        std::string resource = tableResource(components.tableName);
        ...
        if (components.equalityWhereCondition) {
            ...
            if (use_intention_locks) {
                if (!acquireTxnLock(
                        ...
                        LockMode::IS,
                        tableResource(components.tableName),
                        "PROJECT " + components.tableName + " table intent"
                    )) {
                    return false;
                }
            } else {
                logCC(txnLabel(txn) +
                      " uses row-only lock path for PROJECT " +
                      components.tableName);
            }
        } else if (!use_intention_locks) {
            return acquireRowLocksForFullScan(
                txn,
                components.tableName,
                "PROJECT " + components.tableName + " full scan"
            );
        }
        ...
        for (const auto& join : components.joins) {
            if (!acquireTxnLock(
                    txn,
                    LockMode::S,
                    tableResource(join.tableName),
                    "PROJECT " + join.tableName
                )) {
                return false;
            }
        }
    ...
    void executeStatement(const std::string& statement,
        ...
        if (components.type == StatementType::INSERT) {
            ...
            if (recovery_manager.isActive()) {
                throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v71.");
            }
...
void runTransactionLocking() {
    ...
    std::cout << "\nTransaction execution with row-only and intention locks" << std::endl;
    std::cout << "  Row updates take IX on the table, then X on one row." << std::endl;
    std::cout << "  A scan-then-update transaction upgrades S + IX into SIX." << std::endl;

    auto print = [&](const std::string& line) {
        db.logCC(line);
    };

    auto tupleIdLabel = [](const std::optional<TupleId>& tuple_id) {
        if (!tuple_id) {
            return std::string("TupleId(unknown)");
        }
        return "TupleId(page=" + std::to_string(tuple_id->page_id) +
               ", slot=" + std::to_string(tuple_id->slot_id) + ")";
    };

    auto readValue = [&](const TxnPtr& txn,
                         const std::string& table_name,
                         const std::string& key_column,
                         const std::string& key_value,
                         const std::string& value_column) {
        auto rows = db.executeQuery(
            "PROJECT {" + table_name + "." + key_column + "}, {" +
            table_name + "." + value_column + "} FROM " + table_name +
        ...
        for (const auto& row : rows) {
            if (row.fields.size() >= 2 &&
                fieldToString(*row.fields[0]) == key_value) {
        ...
        throw std::runtime_error(
            "Could not read " + table_name + "." + key_column + "=" + key_value
        );
    };

    auto printSeat = [&](const std::string& seat_no) {
        auto seat_status = readValue(nullptr, "seats", "seat_no", seat_no, "status");
        auto seat_customer = readValue(nullptr, "seats", "seat_no", seat_no, "customer");

        std::cout << "    seats[seat_no=" << seat_no << "].status="
                  << seat_status.first
                  << ", customer=" << seat_customer.first
                  << " at " << tupleIdLabel(seat_status.second) << std::endl;
    };

    auto holdSeat = [&](const std::string& txn_label,
                        const std::string& seat_no,
                        int hold_id,
                        const std::string& customer,
                        int sleep_before_commit_ms) {
        auto txn = db.begin(txn_label);
        auto seat_status = readValue(txn, "seats", "seat_no", seat_no, "status");
        print("  " + txn_label +
              " PROJECT {seats.seat_no}, {seats.status} FROM seats "
              "WHERE seat_no=" + seat_no + " -> " +
              seat_status.first + " at " + tupleIdLabel(seat_status.second));

        if (seat_status.first != "available") {
            print("  " + txn_label + " sees seat " + seat_no +
                  " is not available; no hold is created");
            db.commit(txn);
            return;
        }

        print("  " + txn_label +
              " EXECUTE UPDATE seats SET status=held, customer=" +
              customer + " WHERE seat_no=" + seat_no);
        db.execute(
        ...
        if (txn->state == TxnContext::ABORTED) {
            print("  " + txn_label + " booking stopped before creating a hold");
            return;
        }

        print("  " + txn_label +
              " EXECUTE UPDATE holds SET customer=" + customer +
              " WHERE id=" + std::to_string(hold_id));
        db.execute(
            "UPDATE holds SET customer=" + customer +
            " WHERE id=" + std::to_string(hold_id),
            txn,
            false
        );
        if (txn->state == TxnContext::ABORTED) {
            print("  " + txn_label + " booking stopped before commit");
            return;
        }

        if (sleep_before_commit_ms > 0) {
            std::this_thread::sleep_for(
                std::chrono::milliseconds(sleep_before_commit_ms)
            );
        }
        db.commit(txn);
    };

    auto runScanUpdateCase = [&](const std::string& label_prefix) {
        std::thread scan_then_update([&]() {
            auto txn = db.begin(label_prefix + "1");
            print("  " + label_prefix +
                  "1 EXECUTE PROJECT {seats.seat_no}, {seats.status} FROM seats");
            db.execute(
                "PROJECT {seats.seat_no}, {seats.status} FROM seats",
                txn,
                true
            ...
            print("  " + label_prefix +
                  "1 EXECUTE UPDATE seats SET status=held, customer=chen "
                  "WHERE seat_no=1A");
            db.execute(
                "UPDATE seats SET status=held, customer=chen WHERE seat_no=1A",
                txn,
                false
            );
        ...
        std::thread point_reader([&]() {
            auto txn = db.begin(label_prefix + "2");
            auto seat_status = readValue(txn, "seats", "seat_no", "1B", "status");
            print("  " + label_prefix +
                  "2 PROJECT {seats.seat_no}, {seats.status} FROM seats "
                  "WHERE seat_no=1B -> " + seat_status.first + " at " +
                  tupleIdLabel(seat_status.second));
            db.commit(txn);
        });
        ...
        std::thread updater([&]() {
            auto txn = db.begin(label_prefix + "3");
            print("  " + label_prefix +
                  "3 EXECUTE UPDATE seats SET status=held, customer=patel "
                  "WHERE seat_no=1B");
            db.execute(
        ...
        scan_then_update.join();
        point_reader.join();
        updater.join();

        std::cout << "  After scan/update interaction:" << std::endl;
        printSeat("1A");
        printSeat("1B");
    };

    db.resetLockStats();
    std::cout << "\nCase 1: two writers update different seat rows" << std::endl;
    std::cout << "  Purpose: show that IX table locks are compatible, so row writers do not block each other." << std::endl;
    std::cout << "  Watch for: T1 and T2 both get IX(table seats), then X on different seat rows." << std::endl;
    std::thread txn1_worker([&]() {
        holdSeat("T1", "1A", 1, "garcia", 80);
    });

    std::this_thread::sleep_for(std::chrono::milliseconds(10));

    std::thread txn2_worker([&]() {
        holdSeat("T2", "1B", 2, "patel", 0);
    });

    txn1_worker.join();
    txn2_worker.join();

    std::cout << "  After different-row updates:" << std::endl;
    printSeat("1A");
    printSeat("1B");
    db.printLockStats("different-row writers");

    resetBookingRows(db);

    db.setIntentionLocksEnabled(false);
    db.resetLockStats();
    std::cout << "\nCase 2: no intention locks, full scan takes row S locks"
              << std::endl;
    std::cout << "  Purpose: show the correct but expensive row-only alternative." << std::endl;
    std::cout << "  Watch for: N1 locks every seat row before the scan can be protected." << std::endl;
    runScanUpdateCase("N");
    db.printLockStats("row-only scan locking");

    resetBookingRows(db);

    db.setIntentionLocksEnabled(true);
    db.resetLockStats();
    std::cout << "\nCase 3: intention locks use S/SIX at the table"
              << std::endl;
    std::cout << "  Purpose: show how table locks summarize many row locks." << std::endl;
    std::cout << "  Watch for: I1 uses S then SIX(table seats), and I3 waits at IX(table seats)." << std::endl;
    runScanUpdateCase("I");
    db.printLockStats("intention locking");
}

int main() {
    runTransactionLocking();
    return 0;
}
