...
int main() {
    ...
    std::cout << "\nRestart after crash 3: finish COMMITTING txn, checkpoint, then run loser insert/delete/update work.\n";
    {
        BuzzDB db;

        try {
            std::cout << "\nCheckpoint 1: no active txn and no dirty pages, so v62 can truncate the old log prefix.\n";
            db.executeStatementsAndQueries({
                "CHECKPOINT",
            });

            std::cout << "\nTxn 4: dirty seat and hold pages, take a fuzzy checkpoint, then crash before COMMIT.\n";
            db.executeStatementsAndQueries({
                "BEGIN",
