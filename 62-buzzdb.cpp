...
// Recovery policy for v62: ARIES DPT/pageLSN redo, CLRs, and fuzzy checkpoints.
    ...
    BootstrapPage getBootstrap() {
        ...
        if (bootstrap.magic != BUZZDB_MAGIC || bootstrap.version != BUZZDB_VERSION) {
            throw std::runtime_error(
                ...
                "v62 uses ARIES checkpoint/pageLSN/CLR metadata; use a fresh directory or remove buzzdb.dat."
            );
        }
        return bootstrap;
    }
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v62.");
        }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v62 booking workload cannot open a buzzdb.dat containing "
