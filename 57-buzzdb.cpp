...
private:
    void forceLogBeforeFlush(PageID page_id, const std::string& reason) {
        ...
        std::cout << "WAL rule: page " << page_id
                  ...
                  << "; force log through that LSN before "
                  << reason << "." << std::endl;

        bool forced = log_force_callback(page_lsn);
        if (forced) {
            std::cout << "WAL rule: forced log through LSN "
                      << page_lsn << "; page flush is safe." << std::endl;
        } else {
