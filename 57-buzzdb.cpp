...
constexpr uint16_t BUZZDB_VERSION = 57;
...
public:
    ...
    void flushPage(int page_id, const std::string& tag = "explicit") {
        ...
        storage_manager.flush(page_id, getPage(page_id));
    ...
    BootstrapPage getBootstrap() {
        ...
        if (bootstrap.magic != BUZZDB_MAGIC || bootstrap.version != BUZZDB_VERSION) {
            throw std::runtime_error(
                ...
                "v57 uses undo/no-redo recovery metadata; use a fresh directory or remove buzzdb.dat."
            );
        }
        return bootstrap;
    }
...
// Recovery policy for v57: steal buffer pages plus undo before-images.
...
private:
    ...
    bool crash_after_force_before_commit = false;
    bool crash_after_steal_before_commit = false;
    std::vector<UndoUpdate> undo_updates;
...
public:
    ...
    void resetLog() { log_manager.reset(); }
    void simulateCrashAfterForceBeforeCommit() { crash_after_force_before_commit = true; }
    void simulateCrashAfterStealBeforeCommit() { crash_after_steal_before_commit = true; }
    void begin();
    void commit();
    void abort();
    void recover();
    void logBeforeImage(TableId table_id,
                        PageID page_id,
                        size_t slot_id,
                        std::unique_ptr<Tuple> before_tuple);
        ...
        for (PageID page_id : metadata.page_ids) {
            ...
            for (size_t slot_itr = 0; slot_itr < MAX_SLOTS; slot_itr++) {
                ...
                if (recovery_manager != nullptr) {
                    recovery_manager->logBeforeImage(
                        metadata.table_id,
                        page_id,
                        slot_itr,
                        old_tuple->clone()
...
void RecoveryManager::commit() {
    ...
    if (!undo_updates.empty()) {
        for (PageID page_id : dirty_pages) {
            buffer_manager.flushPage(page_id, "commit force");
        }
        data_pages_forced_at_commit += dirty_pages.size();

        if (crash_after_force_before_commit) {
            std::cout << "  recovery: forced " << dirty_pages.size()
                      << " dirty data page(s); before-image log is durable" << std::endl;
            std::cout << "  crash: after dirty page force, before COMMIT log" << std::endl;
            std::exit(2);
        }

        log_manager.append("COMMIT " + std::to_string(current_txn_id));
        committed_update_records += undo_updates.size();
        std::cout << "  recovery: forced " << dirty_pages.size()
                  << " dirty data page(s), wrote COMMIT log, redo not needed" << std::endl;
    } else {
    ...
    dirty_pages.clear();
    txn_logged = false;
    crash_after_force_before_commit = false;
...
void RecoveryManager::abort() {
    ...
    if (!undo_updates.empty()) {
        aborted_update_records += undo_updates.size();
        for (auto it = undo_updates.rbegin(); it != undo_updates.rend(); ++it) {
...
void RecoveryManager::recover() {
    ...
    std::map<int, bool> aborted;
    std::vector<UndoRecord> undo_records;
    for (const auto& record : log_manager.readAll()) {
    ...
    size_t applied = 0;
    std::map<int, bool> losers;
    for (auto it = undo_records.rbegin(); it != undo_records.rend(); ++it) {
        if (committed[it->txn_id] || aborted[it->txn_id]) {
            continue;
        }
        auto& metadata = catalog.getTable(it->table_id);
        TableHeap table(metadata, buffer_manager);
    ...
    for (const auto& loser : losers) {
        log_manager.append("ABORT " + std::to_string(loser.first));
        abort_log_records++;
    }

    restart_undo_records += applied;
    if (applied != 0) {
        std::cout << "Restart recovery: undid " << applied
                  << " loser before-image record(s); redid 0 record(s)." << std::endl;
    }
}

void RecoveryManager::logBeforeImage(TableId table_id,
                                     PageID page_id,
                                     size_t slot_id,
                                     std::unique_ptr<Tuple> before_tuple) {
    if (!txn_active) {
        throw std::runtime_error("Undo recovery update requested without BEGIN.");
    }
    ...
    for (const auto& update : undo_updates) {
        if (update.table_id == table_id &&
            ...
            std::cout << "  recovery: update " << metadata.name
                      << " page " << page_id
                      << " slot " << slot_id
                      << " in place; first before-image already logged" << std::endl;
            return;
        }
    }
    ...
    undo_updates.push_back(std::move(update));
    std::cout << "  recovery: update " << metadata.name
              << " page " << page_id
              << " slot " << slot_id
              << " in place; logged before-image, page may steal" << std::endl;
}
...
void RecoveryManager::printPolicy() const {
    std::cout << "Recovery policy: Undo/No-Redo (Steal + Force)" << std::endl;
    std::cout << "  Steal: Yes; dirty uncommitted pages may reach disk." << std::endl;
    std::cout << "  Force: Yes; commit forces dirty data pages before COMMIT log." << std::endl;
    std::cout << "  Restart: undo loser before-images; redo committed updates is not needed." << std::endl;
}
...
void RecoveryManager::printSummary() {
    ...
    std::cout << "  Policy: Undo/No-Redo (Steal + Force)" << std::endl;
    ...
    std::cout << "  Redo records applied on restart: 0" << std::endl;
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/no-redo update transaction is not supported in v57.");
        }
...
int main(int argc, char* argv[]) {
    ...
    bool crash_before_commit_record =
        mode == "--crash-before-commit-record";
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v57 booking workload cannot open a buzzdb.dat containing "
    ...
    std::string script;
    if (crash_after_uncommitted_flush || crash_before_commit_record) {
        ...
        if (crash_after_uncommitted_flush) {
            ...
            db.recovery_manager.simulateCrashAfterForceBeforeCommit();
        }
        script += R"(
BEGIN;
