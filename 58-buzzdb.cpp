...
public:
    ...
    void abort() {
        ...
        std::cout << "Abort txn " << current_txn_id
                  ...
                  << " operation(s), flushed "
                  << forced_pages
                  << " restored page(s), then forced ABORT LSN "
                  << abort_lsn << "."
                  << std::endl;
        LogRecord end_record{LogRecordType::END, current_txn_id};
        LSN end_lsn = appendTxnRecord(end_record);
...
private:
    ...
    std::set<int> redoWinnerTransactions(
        const std::vector<LogRecord>& records) const {
        std::set<int> redo_winner_txns;
        for (const auto& record : records) {
            if (record.type == LogRecordType::COMMIT) {
                redo_winner_txns.insert(record.txn_id);
            }
        }
        return redo_winner_txns;
    }
    ...
    bool shouldRedoRecord(const LogRecord& record,
                          const RecoveryAnalysis& analysis,
                          const std::set<int>& redo_winner_txns) const {
        if (!isTupleChangeRecord(record.type)) {
            return false;
        }
        if (redo_winner_txns.find(record.txn_id) != redo_winner_txns.end()) {
            return true;
        }
        return analysis.active_transaction_table.find(record.txn_id) !=
               analysis.active_transaction_table.end();
    }
    ...
    size_t countRedoRecords(const std::vector<LogRecord>& records,
        ...
        size_t count = 0;
        for (const auto& record : records) {
            if (shouldRedoRecord(record, analysis, redo_winner_txns)) {
                count++;
            }
        }
        return count;
    ...
    void redoPass(const std::vector<LogRecord>& records,
                  const RecoveryAnalysis& analysis,
                  const std::set<int>& redo_winner_txns) {
        size_t applied = 0;
        for (const auto& record : records) {
            if (!shouldRedoRecord(record, analysis, redo_winner_txns)) {
                continue;
        ...
        if (applied != 0) {
            std::cout << "ARIES REDO: replayed " << applied
                      << " committed or crash-active log record(s)."
                      << std::endl;
        }
    }
    ...
    void undoPass(const std::vector<LogRecord>& records,
        ...
        for (const auto& txn : analysis.active_transaction_table) {
            ...
            LSN end_lsn = appendRecoveryRecord(txn_id,
                                               ...
                                               abort_lsn);
            std::cout << "Recovery: txn " << txn_id
                      << " was a loser; wrote ABORT LSN " << abort_lsn
                      << " and END LSN " << end_lsn << "." << std::endl;
            ended_txns++;
        }

        if (applied != 0 || ended_txns != 0) {
            std::cout << "ARIES UNDO: followed prevLSN chains, undid "
                      << applied
                      << " loser log record(s), and ended "
                      << ended_txns
    ...
    void applyRedo(TableId table_id,
        ...
        page->setPageLSN(std::max(page->getPageLSN(), record_lsn));
        buffer_manager.flushPage(page_id);
    }
