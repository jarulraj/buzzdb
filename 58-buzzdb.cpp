...
public:
    ...
    void append(const std::string& record) {
        ...
        output << record << "\n";
        // v58 forces every log append; v59 will compare pageLSN to flushedLSN.
        output.flush();
        records_written++;
...
// Recovery policy for v58: WAL with undo before-images and redo after-images.
...
private:
    ...
    size_t wal_update_log_forces = 0;
...
public:
    ...
    void abort();
    void recover();
    void logUpdate(TableId table_id,
                   PageID page_id,
                   size_t slot_id,
                   std::unique_ptr<Tuple> before_tuple,
                   std::unique_ptr<Tuple> after_tuple);
    ...
    BootstrapPage getBootstrap() {
        ...
        if (bootstrap.magic != BUZZDB_MAGIC || bootstrap.version != BUZZDB_VERSION) {
            throw std::runtime_error(
                ...
                "v58 uses undo/redo WAL recovery metadata; use a fresh directory or remove buzzdb.dat."
            );
        }
        return bootstrap;
    }
...
void RecoveryManager::commit() {
    ...
    if (!page_update_log_records.empty()) {
        ...
        std::cout << "  recovery: forced COMMIT log, forced 0 data page(s), redo may be needed" << std::endl;
    } else {
...
void RecoveryManager::logUpdate(TableId table_id,
                                PageID page_id,
                                size_t slot_id,
                                std::unique_ptr<Tuple> before_tuple,
                                std::unique_ptr<Tuple> after_tuple) {
    ...
    std::cout << "  recovery: update " << metadata.name
              << " page " << page_id
              << " slot " << slot_id
              << " in place; WAL logged before+after image" << std::endl;
}
...
void RecoveryManager::printPolicy() const {
    std::cout << "Recovery policy: Undo/Redo WAL (Steal + No-Force)" << std::endl;
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v58.");
        }
