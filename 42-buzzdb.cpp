...
class CreateIndexOperator : public UnaryOperator {
private:
    int attributeIndex; // Index of the attribute to build the index on
    HashIndex hash_index;

public:
    CreateIndexOperator(Operator& input, int attributeIndex)
        : UnaryOperator(input), attributeIndex(attributeIndex) {}
    ...
    std::vector<std::unique_ptr<Field>> getOutput() override {
        // This operator does not modify the stream of tuples, so it doesn't produce output in the traditional sense.
        return {};
    }
};
...
public:
    ...
    void executeQuery() {
        ...
        CreateIndexOperator createIndexOperator(scanOperator, 0); // Assuming we want to index on the first attribute

        createIndexOperator.open(); // Builds the index
        createIndexOperator.close();

        // The index is now built and can be accessed
        ...
        std::vector<AggrFunc> aggrFuncs{
            {AggrFuncType::SUM, 1}, // Summing the values of the second attribute
        };

        std::vector<size_t> groupByAttrs{0}; // Group by the first attribute

        HashAggregationOperator hashAggregationOperator(selectOperator, groupByAttrs, aggrFuncs);

        hashAggregationOperator.open(); // Prepare the operator, perform aggregations
        while (hashAggregationOperator.next()) {
            const auto& aggregatedFields = hashAggregationOperator.getOutput(); // Get aggregated results

            std::cout << "Key: ";
            aggregatedFields[0]->print();
            std::cout << ", Value: ";
            aggregatedFields[1]->print();
            std::cout << "\n";
        }
        hashAggregationOperator.close(); // Cleanup
