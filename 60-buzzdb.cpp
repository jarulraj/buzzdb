...
constexpr uint16_t BUZZDB_VERSION = 60;
    ...
    BootstrapPage getBootstrap() {
        ...
        if (bootstrap.magic != BUZZDB_MAGIC || bootstrap.version != BUZZDB_VERSION) {
            throw std::runtime_error(
                ...
                "v60 uses ARIES/pageLSN recovery metadata; use a fresh directory or remove buzzdb.dat."
            );
        }
        return bootstrap;
    }
...
void RecoveryManager::recover() {
    ...
    for (const auto& record : log_records) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v60."
            );
        }
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v60.");
        }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v60 booking workload cannot open a buzzdb.dat containing "
