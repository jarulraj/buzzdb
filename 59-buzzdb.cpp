...
constexpr uint16_t BUZZDB_VERSION = 59;
...
public:
    ...
    void forceLogBeforeFlush(PageID page_id, const std::string& tag) {
        ...
        std::cout << "  WAL rule: before flushing page " << page_id
                  << " for " << tag
...
// Recovery policy for v59: WAL with LSN/pageLSN ordering.
    ...
    BootstrapPage getBootstrap() {
        ...
        if (bootstrap.magic != BUZZDB_MAGIC || bootstrap.version != BUZZDB_VERSION) {
            throw std::runtime_error(
                ...
                "v59 uses pageLSN WAL recovery metadata; use a fresh directory or remove buzzdb.dat."
            );
        }
        return bootstrap;
    }
...
void RecoveryManager::begin() {
    ...
    LSN begin_lsn = log_manager.append("BEGIN " + std::to_string(current_txn_id));
    std::cout << "  log: BEGIN txn " << current_txn_id
              << " LSN " << begin_lsn << std::endl;
    txn_logged = true;
}
...
void RecoveryManager::commit() {
    ...
    if (!wal_updates.empty()) {
        LSN commit_lsn = log_manager.append("COMMIT " + std::to_string(current_txn_id));
        std::cout << "  log: COMMIT txn " << current_txn_id
                  << " LSN " << commit_lsn << std::endl;
    ...
    LSN end_lsn = log_manager.append("END " + std::to_string(current_txn_id));
    std::cout << "  log: END txn " << current_txn_id
              << " LSN " << end_lsn << std::endl;
...
void RecoveryManager::abort() {
    ...
    if (!wal_updates.empty()) {
        ...
        LSN abort_lsn = log_manager.append("ABORT " + std::to_string(current_txn_id));
        std::cout << "  log: ABORT txn " << current_txn_id
                  << " LSN " << abort_lsn << std::endl;
...
void RecoveryManager::recover() {
    ...
    for (const auto& loser : losers) {
        LSN abort_lsn = log_manager.append("ABORT " + std::to_string(loser.first));
        forceLogUpTo(abort_lsn);
        LSN end_lsn = log_manager.append("END " + std::to_string(loser.first));
        forceLogUpTo(end_lsn);
        abort_log_records++;
    }
...
void RecoveryManager::printPolicy() const {
    std::cout << "Recovery policy: Undo/Redo WAL + pageLSN (Steal + No-Force)" << std::endl;
...
void RecoveryManager::printSummary() {
    ...
    std::cout << "  Policy: Undo/Redo WAL + pageLSN (Steal + No-Force)" << std::endl;
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v59.");
        }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v59 booking workload cannot open a buzzdb.dat containing "
