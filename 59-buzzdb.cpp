    ...
    void undoPass(const std::vector<LogRecord>& records,
        ...
        for (const auto& txn : analysis.active_transaction_table) {
            ...
            while (lsn != 0) {
                ...
                if (isTupleChangeRecord(record.type)) {
                    LogRecord clr = makeClrRecord(txn_id, txn_last_lsn, record);
                    LSN clr_lsn = log_manager.append(clr);
            ...
            if (entry.status != RecoveryAnalysis::TxnStatus::ABORTING) {
                abort_lsn = appendRecoveryRecord(txn_id,
                                                 LogRecordType::ABORT,
                                                 txn_last_lsn);
            ...
            if (abort_lsn != 0) {
                ...
                std::cout << "Recovery: txn " << txn_id
                          << " was already aborting; wrote END LSN "
                          << end_lsn << "." << std::endl;
            }
            ended_txns++;
        }
        ...
        if (applied != 0 || ended_txns != 0) {
            std::cout << "ARIES UNDO: followed prevLSN/undoNextLSN chains, wrote "
                      << applied
                      << " CLR(s), and ended "
                      << ended_txns
