...
int main() {
    ...
    std::cout << "\nA booking transaction updates 1A but does not commit.\n";
    auto booking = db.beginTransaction();
    db.execute(booking, "UPDATE seats SET status = held WHERE seat_no = 1A");
    db.execute(booking, "UPDATE seats SET customer = garcia WHERE seat_no = 1A");

    std::cout << "\nA dependent transaction reads 1A before the booking commits.\n";
    auto dependent = db.beginTransaction();
    db.execute(dependent, "SELECT {*} FROM seats WHERE seat_no = 1A");
    std::cout << "\nThe dependent transaction records a hold based on that dirty read.\n";
    ...
    std::cout << "\nBecause the hold depended on dirty data, the dependent transaction must abort too.\n";
    db.abort(dependent);
    ...
    std::cout << "\nProblem: one abort forced another transaction to abort because dirty reads created a dependency.\n";
