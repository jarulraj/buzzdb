...
private:
    ...
    Result executeOne(const RegisterReplicaGroupCommand& command) {
        ...
        insertSystemCatalogRows({
            InsertRowCommand{
                "__replica_groups",
                {command.group_id, std::to_string(command.config_version),
                 catalogJoin(command.voters)}}
        });
        return StatementOkResult{};
    }
...
void printReplicatedCatalogTrace(const std::string&) {
    std::cout << "Trace: replicated system catalog" << std::endl;
    ...
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 1, createTitleTableCommand(),
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
    ...
    state = deliverConsensusCommandToQuorum(
        ...
        RegisterRangeCommand{"range-1", "/table/title", "/table/title0",
                             "group-1", 1},
        {replicas[1], replicas[2]});
    ...
    std::cout << "  leader=" << leader
              ...
              << ", __tables rows="
              << catalogRowCountOn(leader_node, "__tables")
    ...
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        std::cout << "  " << replica
                  << " has title catalog row="
                  << (catalogHasCellOn(node, "__tables", "title")
                          ? "yes" : "no")
                  << ", range row="
                  << (catalogHasCellOn(node, "__ranges", "range-1")
                          ? "yes" : "no")
                  << std::endl;
    }
    std::cout << std::endl;
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v121; "
                  << "this version focuses on replicated catalog metadata."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v121: replicated catalog and config store" << std::endl;
    ...
    TestRunner tests;
    tests.test("Create table updates catalog through Raft", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 121, 1,
        ...
        for (const auto& replica : replicas) {
            ...
            tests.check(catalogHasRowOn(
                            node, "__tables",
                            {{"table_name", "title"}, {"row_count", "0"}}),
                        replica.str() +
                            " should have the replicated table catalog row");
            tests.check(catalogHasRowOn(
                            node, "__schema_versions",
                            {{"table_name", "title"},
                             {"schema_version", "1"}}),
                        replica.str() +
                            " should have the replicated schema version row");
        }
    });

    tests.test("Leader switch preserves catalog", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        ...
        SearchSettings settings;
        SearchState state = consensusClusterState(replicas);
        state = electConsensusLeader(state, settings, old_leader,
                                     {new_leader, replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, old_leader, client, 122, 1,
            createTitleTableCommand(), {new_leader, replicas[2]});
        state = electConsensusLeader(state, settings, new_leader,
                                     {old_leader, replicas[2]});

        const auto* node = state.nodeAs<ConsensusReplica>(new_leader);
        tests.check(node != nullptr && node->role() == ConsensusRole::Leader,
                    "new node should become leader");
        tests.check(catalogHasRowOn(
                        node, "__tables",
                        {{"table_name", "title"}, {"row_count", "0"}}),
                    "new leader should preserve the catalog row");
        tests.check(catalogHasRowOn(
                        node, "__schema_versions",
                        {{"table_name", "title"}, {"schema_version", "1"}}),
                    "new leader should preserve the schema version row");
        tests.check(node != nullptr && node->commitIndex() >= 1,
                    "new leader should keep the committed prefix");
    });

    tests.test("Duplicate catalog command is applied once", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        Command command = RegisterClusterNodeCommand{
            "server1", "voter", "active", 1};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 123, 1, command,
            {replicas[1], replicas[2]});
        const auto* leader_node = state.nodeAs<ConsensusReplica>(leader);
        int last_slot = leader_node == nullptr ? 0 : leader_node->lastNonEmpty();

        state.send(client, leader, ClientRequest{123, 1, command});
        state = stepRequired(
            state,
            requireMessageEvent(
                state,
                ...
                "duplicate catalog request"),
            settings);

        leader_node = state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->executionCount(123, 1) == 1,
                    "duplicate request should not re-execute");
        tests.check(leader_node != nullptr &&
                        catalogRowCountOn(leader_node,
                                          "__cluster_nodes") == 1,
                    "duplicate request should not add a second node row");
        tests.check(catalogHasRowOn(
                        leader_node, "__cluster_nodes",
                        {{"node_id", "server1"},
                         {"role", "voter"},
                         {"status", "active"},
                         {"epoch", "1"}}),
                    "cluster node row should preserve all registered fields");
        tests.check(leader_node != nullptr &&
                        leader_node->lastNonEmpty() == last_slot &&
                        clientReplyInNetworkFor(state, 123, 1),
                    "duplicate request should return from AMO cache");
    });

    tests.test("Follower catch-up gets catalog changes", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {up_to_date, lagging});
        Command command = RegisterReplicaGroupCommand{
            "group-1", {"server1", "server2", "server3"}, 1};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 124, 1, command,
            {up_to_date});

        const auto* lagging_node = state.nodeAs<ConsensusReplica>(lagging);
        tests.check(lagging_node != nullptr &&
                        !catalogHasRowOn(
                            lagging_node, "__replica_groups",
                            {{"group_id", "group-1"}}),
                    "lagging follower should not have the catalog row yet");
        ...
        tests.check(lagging_node != nullptr &&
                    ...
                    "lagging follower should learn catalog rows by log catch-up");
    });

    tests.test("Stale catalog read waits for committed index", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address current = replicas[1];
        Address stale = replicas[2];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        SearchState state = consensusClusterState(replicas);
        state = electConsensusLeader(state, settings, leader,
                                     {current, stale});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 125, 1,
            createTitleTableCommand(), {current});
        const auto* stale_node = state.nodeAs<ConsensusReplica>(stale);
        tests.check(stale_node != nullptr &&
                        !catalogHasRowOn(stale_node, "__tables",
                                         {{"table_name", "title"}}),
                    "stale follower should not have the new catalog row");

        SearchSettings partitioned;
        partitioned.partition({{stale, client}, {leader, current}});
        state.send(client, stale,
                   ClientRequest{125, 2,
                                 ReadSystemCatalogCommand{"__tables"}});
        state = stepRequired(
            state,
            requireMessageEvent(
                state,
                ...
                [&](const MessageEnvelope& envelope) {
                    const auto* request =
                        std::get_if<ClientRequest>(&envelope.message);
                    return request != nullptr &&
                           request->client_id == 125 &&
                           request->request_id == 2 &&
                           envelope.to.rootAddress() == stale;
                },
                "stale catalog read request"),
            partitioned);

        SearchSettings search = partitioned;
        search.max_depth = 4;
        search.max_states = 200;
        search.deliverTimers(false);
        search.invariants.push_back({
            "STALE_CATALOG_READ_DOES_NOT_REPLY",
            [](const SearchState& candidate) {
                return CheckResult{
                    !clientReplyInNetworkFor(candidate, 125, 2),
                    "stale follower replied to catalog read"};
            }
        });
        SearchResults result = bfs(state, search);
        tests.check(result.end_condition ==
                        SearchResults::EndCondition::SpaceExhausted,
                    "stale catalog read should wait safely, got " +
                        endConditionName(result.end_condition) + ": " +
                        result.message);
    });

    tests.test("SQL command and catalog command have one global order", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = deliverConsensusCommandToQuorum(
            ...
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 126, 2, range,
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 126, 3, insert,
            {replicas[1], replicas[2]});

        const auto* leader_node = state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->slotCommandMatches(1, create) &&
                        leader_node->slotCommandMatches(2, range) &&
                        leader_node->slotCommandMatches(3, insert),
                    "SQL and catalog commands should share one Raft order");
        tests.check(leader_node != nullptr &&
                        catalogHasRowOn(
                            leader_node, "__ranges",
                            {{"range_id", "range-title"},
                             {"start_key", "/table/title"},
                             {"end_key", "/table/title0"},
                             {"replica_group_id", "group-1"},
                             {"descriptor_version", "1"}}) &&
                        leader_node->executeReadOnlyForTest(
                            CountRowsCommand{"title"}) ==
                            Result{CountRowsResult{1}},
                    "catalog and SQL state should both reflect the log order");
    });
