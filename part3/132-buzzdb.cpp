...
void printV104BootstrapTrace(const std::string& data_file) {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v132-bootstrap-trace-" + std::to_string(::getpid()));
...
void printPartitionedSQLTrace(const std::string& data_file) {
    std::cout << "Trace: shardmaster-style range configs" << std::endl;
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v132; "
                  << "this version focuses on shardmaster-style "
                  << "range configs."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v132: shardmaster-style range configs" << std::endl;
