...
// Page 0 stores the system catalog page tables.
...
// Recovery policy for v54: copy-on-write shadow pages.
...
private:
    ...
    size_t crash_after_catalog_installs = 0;
...
public:
    ...
    void simulateCrashAfterCatalogInstalls(size_t count) { crash_after_catalog_installs = count; }
    void begin();
    void commit();
    void abort();
    PageID getShadowPage(PageID page_id) const;
    PageID ensureShadowPageForUpdate(PageID page_id);
};
...
public:
    ...
    size_t updateTuples(size_t where_column,
        ...
        for (PageID page_id : metadata.page_ids) {
            PageID write_page_id = recovery_manager == nullptr
                ? page_id
                : recovery_manager->getShadowPage(page_id);
            auto* page = &getPage(write_page_id);
            ...
            for (size_t slot_itr = 0; slot_itr < MAX_SLOTS; slot_itr++) {
                ...
                if (recovery_manager != nullptr && write_page_id == page_id) {
                    write_page_id = recovery_manager->ensureShadowPageForUpdate(page_id);
                    page = &getPage(write_page_id);
                    page_buffer = (*page)->page_data.get();
                    slot_array = reinterpret_cast<Slot*>(page_buffer);
                }
            ...
            if (page_updated) {
                buffer_manager.flushPage(write_page_id);
            }
        }
...
private:
    ...
    void initializeNewDatabase() {
        ...
        storeBootstrapPageIds(bootstrap_page.columns_page_count,
    ...
    void installSystemTables(const BootstrapPage& bootstrap_page) {
        ...
        auto tables_page_ids = bootstrapPageIds(bootstrap_page.tables_page_count,
                                                bootstrap_page.tables_pages);
        auto columns_page_ids = bootstrapPageIds(bootstrap_page.columns_page_count,
                                                 bootstrap_page.columns_pages);
...
void RecoveryManager::commit() {
    ...
    for (const auto& entry : shadow_page_table) {
        ...
        TableId table_id = buffer_manager.getPage(old_page_id)->getTableId();
        auto& metadata = catalog.getTable(table_id);
        TableHeap table(metadata, buffer_manager);
        table.replacePage(old_page_id, entry.second);
        catalog.persistTableMetadata(metadata);
        if (crash_after_catalog_installs != 0 && --crash_after_catalog_installs == 0) {
            std::cout << "Simulated crash during catalog metadata update." << std::endl;
            std::exit(2);
        }
    }

    shadow_page_table.clear();
    crash_after_catalog_installs = 0;
    txn_active = false;
}
...
PageID RecoveryManager::getShadowPage(PageID page_id) const {
    auto it = shadow_page_table.find(page_id);
    return it == shadow_page_table.end() ? page_id : it->second;
}

PageID RecoveryManager::ensureShadowPageForUpdate(PageID page_id) {
    if (!txn_active) {
        throw std::runtime_error("Shadow update requested without BEGIN.");
    }

    auto it = shadow_page_table.find(page_id);
    if (it != shadow_page_table.end()) {
        return it->second;
    }
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside a shadow transaction is not supported in v54.");
        }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v54 booking demo cannot open a buzzdb.dat containing "
    ...
    if (crash_demo) {
        ...
        db.recovery_manager.simulateCrashAfterCatalogInstalls(1);
        script += R"(
