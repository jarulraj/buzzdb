...
// Recovery policy for v64: ARIES plus fuzzy image-copy media recovery.
...
void RecoveryManager::recover() {
    ...
    auto collectWalRecord = [&](const std::string& record) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v64."
            );
        }
        if (txn_id > 0) {
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v64.");
        }
...
int main(int argc, char* argv[]) {
    ...
    std::string mode = argc > 1 && std::string(argv[1]).rfind("--", 0) == 0
        ? argv[1] : "";
    if (mode == "--media-corrupt") {
        StorageManager::corruptDatabasePage();
        std::cout << "Media failure: run ./a.out --media-recover to restore "
                  << "the fuzzy image copy and replay WAL." << std::endl;
        return 0;
    }
    if (mode == "--media-recover") {
        restoreImageCopyBeforeStartup();
    }
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v64 booking workload cannot open a buzzdb.dat containing "
