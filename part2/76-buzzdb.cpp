...
public:
    ...
    std::function<void(int, const std::vector<int>&)> cc_wait_hook;
    ...
    void queueGroupCommit(const TxnPtr& tx) {
        ...
        logCC(txnLabel(tx) + " COMMIT queued at LSN " +
        ...
        logCC(txnLabel(tx) + " COMMIT returns after durable group force");
    }
    ...
    void flushGroupCommit() {
        ...
        for (const auto& entry : batch) {
            ...
            cc_lock_manager.releaseAll(entry->txn->id);
            logCC(txnLabel(entry->txn) +
    ...
    bool acquireRestartUndoLock(TableId table_id, PageID page_id, size_t slot_id) {
        auto resource = rowResourceForSlot(table_id, page_id, slot_id);
        logCC("RECOVERY reacquires X lock on " + resource +
              " before physiological restart undo");
        auto result = cc_lock_manager.waitFor(
            RECOVERY_TXN_ID, resource, LockMode::X, std::chrono::milliseconds(500)
        );
        if (result.granted) {
            logCC("RECOVERY X lock on " + resource + " granted");
            return false;
        }
        if (!result.deadlock) {
            throw std::runtime_error(
        ...
        logCC("deadlock during early-availability restart undo; waits-for graph cycle: " +
        ...
        for (int txn_id : result.cycle) {
            ...
            logCC("RECOVERY cancels " + txnIdLabel(txn_id) +
                  " so restart undo can finish");
            cc_lock_manager.cancel(txn_id);
        }

        auto retry = cc_lock_manager.waitFor(
            RECOVERY_TXN_ID, resource, LockMode::X, std::chrono::milliseconds(500)
        );
        if (!retry.granted) {
            throw std::runtime_error("Recovery lock retry failed after deadlock.");
        }
        logCC("RECOVERY X lock on " + resource +
              " granted after deadlock resolution");
        return true;
    }

    void releaseRestartUndoLocks() {
        if (cc_lock_manager.releaseAll(RECOVERY_TXN_ID)) {
            logCC("RECOVERY releases restart undo locks");
        }
    }
    ...
    bool acquireTxnLock(const TxnPtr& txn,
        ...
        if (result.canceled) {
            logCC(txnLabel(txn) + " lock request canceled; " + result.reason);
    ...
    bool acquireQueryLocks(const TxnPtr& txn,
        ...
        auto resources = rowLockResourcesForTable(components.tableName);
        logCC(txnLabel(txn) + " full scan locks " +
              std::to_string(resources.size()) +
...
void runRestartRecoveryDeadlock() {
...

    {
        ...
        setup.cc_lock_manager.releaseAll(loser->id);
    ...
    std::thread post_restart_txn([&]() {
        ...
        bool got_second_lock = db.acquireTxnLock(
            txn,
            LockMode::X,
        ...
        if (got_second_lock) {
            ...
            db.logCC("Tlive stops after recovery deadlock handling");
        }
    });
