    ...
    BootstrapPage getBootstrap() {
        ...
        if (bootstrap.magic != BUZZDB_MAGIC || bootstrap.version != BUZZDB_VERSION) {
            throw std::runtime_error(
                ...
                "v56 uses redo recovery metadata; use a fresh directory or remove buzzdb.dat."
            );
        }
        return bootstrap;
    }
...
public:
    ...
    void simulateCrashAfterCommitBeforeApply() { crash_after_commit_before_apply = true; }
    void begin();
    void commit();
    void abort();
    void recover();
...
void RecoveryManager::commit() {
    ...
    if (!deferred_updates.empty()) {
        ...
        for (auto& update : deferred_updates) {
            auto after_image = update.after_tuple->serialize();
            tuple_after_image_bytes_logged += after_image.size();
            log_manager.append(
                "UPDATE " + std::to_string(current_txn_id) + " " +
                std::to_string(update.table_id) + " " +
                std::to_string(update.page_id) + " " +
                std::to_string(update.slot_id) + " " +
                after_image
            );
        }
...
void RecoveryManager::recover() {
    ...
    std::vector<RedoRecord> redo_records;
    ...
    redo_records_applied += applied;
    if (applied != 0) {
        std::cout << "Restart recovery: redid " << applied
                  << " committed tuple after-image record(s)." << std::endl;
    }
}
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v56 booking demo cannot open a buzzdb.dat containing "
    ...
    if (crash_mode) {
        ...
        db.recovery_manager.simulateCrashAfterCommitBeforeApply();
        script += R"(
