...
QueryComponents parseQuery(const std::string& query, const Catalog& catalog) {
    ...
    std::regex queryRegex(
        "^\\s*SUM\\(([A-Za-z_][A-Za-z0-9_]*)\\)\\s+FROM\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+GROUP\\s+BY\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+WHERE\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*>\\s*(-?\\d+)\\s+and\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*<\\s*(-?\\d+)\\s*$",
    ...
    const std::string sumColumnName = matches[1];
    const std::string tableName = matches[2];
    const std::string groupByColumnName = matches[3];
    const std::string lowerBoundColumnName = matches[4];
    const int lowerBound = std::stoi(matches[5]);
    const std::string upperBoundColumnName = matches[6];
    const int upperBound = std::stoi(matches[7]);
    ...
    components.sumAttributeIndex = metadata.schema.getColumnIndex(sumColumnName);
    ...
    components.selectAttributes.push_back(components.sumAttributeIndex);
...
private:
    ...
    std::unique_ptr<Tuple> makeTuple() {
...
public:
    InsertOperator(BufferManager& manager, int key, int value)
        : bufferManager(manager), key(key), value(value) {}
    ...
    bool next() override {
        ...
        auto newPageId = bufferManager.getNumPages() - 1;
        auto& newPage = bufferManager.getPage(newPageId);
        if (newPage->addTuple(makeTuple())) {
...
public:
    UpdateOperator(BufferManager& manager,
                   std::unique_ptr<IPredicate> predicate,
                   std::vector<std::pair<size_t, Field>> assignments)
...
public:
    DeleteOperator(BufferManager& manager,
                   std::unique_ptr<IPredicate> predicate)
        : bufferManager(manager),
          predicate(std::move(predicate)) {}
...
public:
    ...
    void insert(int key, int value) {
        executeStatementsAndQueries({
            "INSERT INTO records VALUES (" + std::to_string(key) + ", " + std::to_string(value) + ")"
        }, false);
    }

    void executeStatementsAndQueries(const std::vector<std::string>& statements,
                                     bool printResult = true) {
        auto executeStatementOperator = [printResult](Operator& statementOperator) {
        ...
        for (const auto& statement : statements) {
            ...
            std::regex insertRegex("^\\s*INSERT\\s+INTO\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+VALUES\\s*\\(\\s*(-?\\d+)\\s*,\\s*(-?\\d+)\\s*\\)\\s*;?\\s*$",
                                   std::regex_constants::icase);
            if (std::regex_match(statement, matches, insertRegex)) {
                const std::string tableName = matches[1];
                const int insertKey = std::stoi(matches[2]);
                const int insertValue = std::stoi(matches[3]);

                const auto& metadata = catalog.getTable(tableName);
                if (metadata.schema.columns.size() != 2 ||
                    metadata.schema.columns[0].type != INT ||
                    metadata.schema.columns[1].type != INT) {
                    throw std::runtime_error("INSERT expects a two-column INT table.");
                }

                InsertOperator insertOp(buffer_manager, insertKey, insertValue);
                executeStatementOperator(insertOp);
                continue;
            }

            std::regex updateRegex("^\\s*UPDATE\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+SET\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*(-?\\d+)\\s+WHERE\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*(-?\\d+)\\s*;?\\s*$",
                                   std::regex_constants::icase);
            if (std::regex_match(statement, matches, updateRegex)) {
                const std::string tableName = matches[1];
                const std::string setColumnName = matches[2];
                const int setValue = std::stoi(matches[3]);
                const std::string whereColumnName = matches[4];
                const int whereValue = std::stoi(matches[5]);

                const auto& metadata = catalog.getTable(tableName);
                const size_t setColumn = static_cast<size_t>(
                    metadata.schema.getColumnIndex(setColumnName));
                const size_t whereColumn = static_cast<size_t>(
                    metadata.schema.getColumnIndex(whereColumnName));

                auto predicate = std::make_unique<SimplePredicate>(
                    SimplePredicate::Operand(whereColumn),
                    SimplePredicate::Operand(std::make_unique<Field>(whereValue)),
                    SimplePredicate::ComparisonOperator::EQ
                );
                UpdateOperator updateOp(
                    buffer_manager,
                    std::move(predicate),
                    {{setColumn, Field(setValue)}}
                );
                executeStatementOperator(updateOp);
                continue;
            }

            std::regex deleteRegex("^\\s*DELETE\\s+FROM\\s+([A-Za-z_][A-Za-z0-9_]*)\\s+WHERE\\s+([A-Za-z_][A-Za-z0-9_]*)\\s*=\\s*(-?\\d+)\\s*;?\\s*$",
                                   std::regex_constants::icase);
            if (std::regex_match(statement, matches, deleteRegex)) {
                const std::string tableName = matches[1];
                const std::string whereColumnName = matches[2];
                const int whereValue = std::stoi(matches[3]);
                ...
                DeleteOperator deleteOp(buffer_manager, std::move(predicate));
                executeStatementOperator(deleteOp);
                continue;
            }
...
int main() {
    ...
    constexpr size_t max_input_tuples = 5000;
    constexpr size_t load_every_nth_tuple = 100;

    int field1, field2;
    size_t input_tuple_count = 0;
    while (inputFile >> field1 >> field2) {
        input_tuple_count++;
        if (input_tuple_count >= max_input_tuples) {
            break;
        }
        if (input_tuple_count % load_every_nth_tuple != 0) {
            continue;
        }
    ...
    db.executeStatementsAndQueries({
        "UPDATE records SET value = 7 WHERE key = 3",
        "DELETE FROM records WHERE key = 4",
        "INSERT INTO records VALUES (6, 6)",
        "SELECT SUM(value) FROM records GROUP BY key WHERE key > 2 and key < 6"
    });
