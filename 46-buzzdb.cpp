...
// Implement a 2 phase locking mechanism for concurrency control
// granularity of locking is at the page level
...
private:
    std::mutex mtx;
    std::unordered_map<int, std::pair<LockType, int>> lockTable; // Page ID -> (LockType, Count)
    std::condition_variable cv;

public:
    LockManager(){}
    void acquireLock(int pageId, LockType type) {
        std::unique_lock<std::mutex> lock(mtx);
        cv.wait(lock, [&]() {
            if (type == SHARED) {
                return lockTable[pageId].first != EXCLUSIVE;
            } else {
                return lockTable[pageId].second == 0;
            }
        });

        std::cout<< "Acquired Required Lock on pageId " << pageId << "\n";

        if (type == SHARED) {
            lockTable[pageId].first = SHARED;
            lockTable[pageId].second++;
        } else {
            lockTable[pageId].first = EXCLUSIVE;
            lockTable[pageId].second = 1;
        }
    }

    void releaseLock(int pageId) {
        std::unique_lock<std::mutex> lock(mtx);
        if (--lockTable[pageId].second == 0) {
            lockTable.erase(pageId);
        }
        cv.notify_all();
    }
};
...
private:
    std::unordered_map<int, std::vector<int>> txnManager;
public:
    TransactionManager(){}
    void addTransactionLock(int transactionId, int pageId){
        auto it = txnManager.find(transactionId);
        if(it != txnManager.end()){
            it->second.push_back(pageId);
        }
        else{
            txnManager[transactionId] = {pageId};
        }
    }
    void deleteTransaction(int transactionId){
        ...
        if(it != txnManager.end()){
            txnManager.erase(it);
            std::cout << "Successfully Removed Transaction %d from txnManager" << transactionId << "\n";
        }
    }
    std::vector<int> getTransactionHistory(int transactionId) {
        auto it = txnManager.find(transactionId);
        if(it != txnManager.end()){
            return it->second;
        }
        else{
            std::cout<<"Transaction does not hold any locks !" << "\n";
        }
        return {};
    }
};

enum Phase {GROWING, SHRINKING};
...
public:
    Transaction(int id, Phase phase, TransactionManager& txnManager, LockManager& lockManager) : txnId(id), 
    phase(phase), txnManager(txnManager), lockManager(lockManager) {}
    void commit() {
        std::cout << "All Operations Successful, releasing all locks held" << "\n";
        phase = Phase::SHRINKING;
        std::vector<int> locksHeld = txnManager.getTransactionHistory(txnId);
        if(locksHeld.size() > 0){
            for(int i = 0; i < locksHeld.size(); i++){
                int pageId = locksHeld.at(i);
                lockManager.releaseLock(pageId);
            }
        }
    }
    int getTxnId() {
...
public:
    InsertOperator(BufferManager& manager, TransactionManager& txnManager, LockManager& lockManager) : bufferManager(manager), 
    txnManager(txnManager), lockManager(lockManager) {}
    ...
    bool nextWithConcurrency(Transaction t) {
        ...
        // ToDo: Decide what type of locking needs to be done when extending Database
...
public:
    ...
    // insert function
    void insert(int key, int value, Transaction t) {
        tuple_insertion_attempt_counter += 1;
        // Create a new tuple with the given key and value
        // std::cout << "Trying to insert data " << key << value << "\n"; 
        ...
        InsertOperator insertOp(buffer_manager, txnManager, lockManager);
        insertOp.setTupleToInsert(std::move(newTuple));
        bool status = insertOp.nextWithConcurrency(t);
        // std::cout << "Is tuples inserted " << status << "\n";
        std::cout << "Are tuples inserted " << "\n";
        //assert(status == true);
