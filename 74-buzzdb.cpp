...
public:
    ...
    static bool isSelectStatement(const std::string& statement) {
        static const std::regex selectRegex("^\\s*SELECT\\s+.*",
                                            std::regex_constants::icase);
        return std::regex_match(statement, selectRegex);
    }
    ...
    void acquireStatementLocks(const TxnPtr& txn,
        ...
        if (std::regex_match(statement, matches, insertRegex)) {
            ...
            if (usePredicateLocks() &&
                statusColumn &&
                values.size() == metadata.schema.columns.size()) {
...
int main() {
    ...
    std::cout << "Isolation levels as locking policies\n";
