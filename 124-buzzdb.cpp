...
std::vector<std::string> resultWireFields(const Result& result) {
    return std::visit(
        [](const auto& value) {
            ...
            } else if constexpr (std::is_same_v<T, ClusterRejectedResult>) {
                fields.push_back("ClusterRejected");
            } else if constexpr (std::is_same_v<T, ConfigRejectedResult>) {
                fields.push_back("ConfigRejected");
            }
            return fields;
        },
        result);
...
Result deserializeResult(const std::string& encoded) {
    ...
    if (kind == "ClusterRejected" && fields.size() == 1) return ClusterRejectedResult{};
    if (kind == "ConfigRejected" && fields.size() == 1) return ConfigRejectedResult{};
...
std::string makeScratchBuzzDBSnapshotFile() {
    static size_t counter = 0;
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-snapshot-" + std::to_string(::getpid()) + "-" +
...
private:
    ...
    Result executeForReplicatedApply(
        const RegisterClusterNodeCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const RegisterReplicaGroupCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const AddLearnerToGroupCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const MarkLearnerCaughtUpCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const BeginJointConfigCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const FinalizeJointConfigCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(const RegisterRangeCommand& command,
                                     const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const BootstrapClusterCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const DiscoverClusterNodeCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const ReadClusterIdentityCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }

    Result executeForReplicatedApply(
        const ReadSystemCatalogCommand& command,
        const TxnPtr&) {
        return executeOne(command);
    }
...
using Timer = std::variant<RetryTimer, ElectionTimer, AppendRetryTimer>;
...
bool isReadOnlyCommand(const Command& command) {
    return std::holds_alternative<SelectAllCommand>(command) ||
           ...
           std::holds_alternative<ReadClusterIdentityCommand>(command) ||
           std::holds_alternative<ReadSystemCatalogCommand>(command);
}
...
class ConsensusClientWorker : public Node {
public:
    ConsensusClientWorker(Address address,
                          std::vector<Address> replicas,
                          int client_id,
                          Workload workload)
        : Node(std::move(address)),
          replicas_(normalizeReplicas(std::move(replicas))),
          client_id_(client_id),
          workload_(std::move(workload)) {
        if (replicas_.empty()) {
            throw std::runtime_error("consensus client needs at least one replica");
        }
    }
    ...
    std::unique_ptr<Node> clone() const override {
        return std::make_unique<ConsensusClientWorker>(*this);
    }
    ...
    std::string digest() const override {
        std::ostringstream out;
        out << "ConsensusClient(waiting=" << waiting_
            << ", req=" << in_flight_request_id_
            << ", results=" << results_.size()
            << ", stale=" << stale_replies_
            << ", retries=" << retries_
            << ", next_replica=" << replicas_[next_replica_]
            << ", workload=" << workload_.digest() << ")";
        return out.str();
    }
...
private:
    ...
    void sendInFlight(NodeContext& ctx) {
        ctx.send(replicas_[next_replica_], ClientRequest{
            client_id_,
            in_flight_request_id_,
            in_flight_command_
        });
...
void printV104BootstrapTrace(const std::string& data_file) {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v124-bootstrap-trace-" + std::to_string(::getpid()));
...
public:
    ...
    ConsensusSlotStatus slotStatus(int index) const {
        if (index < first_non_cleared_) {
            return ConsensusSlotStatus::Cleared;
        }
        auto it = log_.find(index);
        if (it == log_.end()) {
            return ConsensusSlotStatus::Empty;
        }
        return it->second.status;
    }

    bool hasAccepted(int index) const {
        ConsensusSlotStatus status = slotStatus(index);
        return status == ConsensusSlotStatus::Accepted ||
               status == ConsensusSlotStatus::Chosen;
    }
    ...
    bool acceptedEntryMatches(int index,
        ...
        const ConsensusLogEntry& entry = it->second;
        return entry.term == term &&
               entry.client_id == client_id &&
               entry.request_id == request_id &&
               entry.command == command &&
               entry.status != ConsensusSlotStatus::Empty;
    }

    bool slotCommandMatches(int index, const Command& command) const {
        auto it = log_.find(index);
        return it != log_.end() && it->second.command == command;
    }

    std::optional<Command> slotCommand(int index) const {
        auto it = log_.find(index);
        if (it == log_.end() ||
            it->second.status == ConsensusSlotStatus::Empty ||
            it->second.status == ConsensusSlotStatus::Cleared) {
            return std::nullopt;
        }
        return it->second.command;
    }

    std::optional<ConsensusLogEntry> logEntryForTest(int index) const {
        auto it = log_.find(index);
        if (it == log_.end()) return std::nullopt;
        return it->second;
    }

    const std::map<std::string, int>& executionCountsForTest() const {
        return execution_count_;
    }

    bool hasCommittedRequestForTest(int client_id, int request_id) const {
        for (int slot = first_non_cleared_; slot <= commit_index_; ++slot) {
            auto it = log_.find(slot);
            if (it == log_.end()) continue;
            const ConsensusLogEntry& entry = it->second;
            if (entry.status == ConsensusSlotStatus::Chosen &&
                entry.client_id == client_id &&
                entry.request_id == request_id) {
                return true;
            }
    ...
    size_t acceptVoteCount(int index) const {
        size_t count = 0;
        for (const auto& replica : replicas_) {
            auto it = match_index_.find(replica.rootAddress());
...
SearchState deliverConsensusRequestAtLeaderToQuorum(
    SearchState state,
    const SearchSettings& settings,
    const Address& leader,
    const Address& client,
    ...
    int request_id,
    const std::vector<Address>& acceptors,
    bool deliver_client_reply = true) {
    state = stepRequired(
        state,
        requireMessageEvent(
            state,
            settings,
            [&](const MessageEnvelope& envelope) {
                if (!(envelope.to.rootAddress() == leader.rootAddress())) {
                    return false;
                }
                ...
                if (const auto* forward =
                    ...
                    return forward->client == client.rootAddress() &&
                           forward->request.client_id == client_id &&
                           forward->request.request_id == request_id;
                }
                return false;
            },
            "client request at consensus leader"),
        settings);

    int slot = 0;
    for (const auto& acceptor : acceptors) {
        EventRef append_event = requireMessageEvent(
            state,
            settings,
            [&](const MessageEnvelope& envelope) {
                const auto* append =
            ...
            "append entry " + leader.str() + " -> " + acceptor.str());
        const auto* envelope = messageForEvent(state, append_event);
        const auto* append = std::get_if<AppendEntries>(&envelope->message);
        slot = append->entries.back().index;
        state = stepRequired(state, append_event, settings);
    }
    ...
    if (deliver_client_reply) {
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                [&](const MessageEnvelope& envelope) {
                    const auto* reply =
                        std::get_if<ClientReply>(&envelope.message);
                    return reply != nullptr &&
                           reply->client_id == client_id &&
                           reply->request_id == request_id &&
                           envelope.to.rootAddress() == client.rootAddress();
                },
                "consensus client reply"),
            settings);
    }
...
SearchState deliverConsensusAppendsForRequestToQuorum(
    SearchState state,
    const SearchSettings& settings,
    const Address& leader,
    const Address& client,
    ...
    const std::vector<Address>& acceptors,
    bool deliver_client_reply = true,
    uint64_t min_append_message_id = 0) {
    int slot = 0;
    for (const auto& acceptor : acceptors) {
        EventRef append_event = requireMessageEvent(
            state,
            ...
            [&](const MessageEnvelope& envelope) {
                const auto* append =
                    std::get_if<AppendEntries>(&envelope.message);
                return append != nullptr &&
                       envelope.id > min_append_message_id &&
                       !append->entries.empty() &&
                       append->entries.back().client_id == client_id &&
                       append->entries.back().request_id == request_id &&
                       envelope.from.rootAddress() == leader.rootAddress() &&
            ...
            "pending append entry " + leader.str() + " -> " +
                acceptor.str());
        const auto* envelope = messageForEvent(state, append_event);
        const auto* append = std::get_if<AppendEntries>(&envelope->message);
        slot = append->entries.back().index;
    ...
    for (const auto& acceptor : acceptors) {
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                "pending append reply " + acceptor.str()),
            settings);
    }
    ...
    for (const auto& acceptor : acceptors) {
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                "pending commit append " + leader.str() + " -> " +
                    acceptor.str()),
            settings);
    }
    ...
    if (deliver_client_reply) {
        state = stepRequired(
            ...
            requireMessageEvent(
                ...
                "pending consensus client reply"),
            settings);
    }
...
bool clientReplyResultInNetworkFor(const SearchState& state,
                                   int client_id,
                                   int request_id,
...
CheckResult consensusClientDone(const SearchState& state,
                                const Address& client) {
    const auto* worker = state.nodeAs<ConsensusClientWorker>(client);
    if (worker == nullptr) {
        return {false, client.str() + " is missing"};
    }
    if (!worker->done()) {
        return {false, client.str() + " is not done"};
    }
    if (!worker->resultsOk()) {
        return {false, client.str() + " produced an unexpected result"};
    }
    return {true, ""};
}
...
CheckResult consensusMarkersValid(const SearchState& state,
                                  const std::vector<Address>& replicas) {
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        if (node == nullptr) {
        ...
        int first = node->firstNonCleared();
        int last = node->lastNonEmpty();
        if (first < 1) {
            return {false, replica.str() + " returned invalid first slot"};
        }
        if (last < 0 || first > last + 1) {
            return {false, replica.str() + " returned invalid log markers"};
        }
        if (node->slotStatus(first) == ConsensusSlotStatus::Cleared) {
            return {false, replica.str() + " first non-cleared slot is cleared"};
        }
        if (last > 0 && node->slotStatus(last) == ConsensusSlotStatus::Empty) {
            return {false, replica.str() + " last non-empty slot is empty"};
        }
        if (node->slotStatus(last + 1) != ConsensusSlotStatus::Empty) {
            return {false, replica.str() + " slot after last non-empty is not empty"};
        }
    }
    return {true, ""};
}

CheckResult consensusSlotValid(const SearchState& state,
                               const std::vector<Address>& replicas,
                               int slot) {
    ...
    for (const auto& replica : replicas) {
        ...
        if ((status == ConsensusSlotStatus::Empty ||
             status == ConsensusSlotStatus::Cleared) &&
            command.has_value()) {
            return {false, replica.str() + " returned a command for an empty slot"};
        }
        if (status == ConsensusSlotStatus::Chosen) {
            if (saw_chosen && !(command == chosen)) {
                return {false, "two different commands chosen for slot " +
                                   std::to_string(slot)};
            }
            chosen = command;
            saw_chosen = true;
    ...
    if (matching < replicas.size() / 2 + 1) {
        return {false, "chosen slot " + std::to_string(slot) +
                           " lacks a matching majority"};
    }
    return {true, ""};
}

CheckResult consensusLogsConsistentAllSlots(
    const SearchState& state,
    const std::vector<Address>& replicas) {
    CheckResult markers = consensusMarkersValid(state, replicas);
    if (!markers.ok) {
        return markers;
    }
    int max_slot = 0;
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        if (node != nullptr) {
            max_slot = std::max(max_slot, node->lastNonEmpty());
        }
    }
    for (int slot = 1; slot <= max_slot; ++slot) {
        CheckResult result = consensusSlotValid(state, replicas, slot);
        if (!result.ok) {
            return result;
        }
    }
...
CheckResult consensusAppliedPrefixValid(
    const SearchState& state,
    const std::vector<Address>& replicas) {
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        ...
        if (node->appliedIndex() > node->commitIndex()) {
            return {false, replica.str() + " applied past commit index"};
        }
        for (int slot = node->firstNonCleared();
             slot <= node->appliedIndex(); ++slot) {
            if (node->slotStatus(slot) != ConsensusSlotStatus::Chosen) {
                return {false, replica.str() +
                                   " applied non-chosen slot " +
                                   std::to_string(slot)};
            }
            if (!node->slotCommand(slot).has_value()) {
                return {false, replica.str() +
                                   " applied missing command at slot " +
                                   std::to_string(slot)};
            }
        }
    }
...
CheckResult consensusCommittedPrefixesIdentical(
    const SearchState& state,
    const std::vector<Address>& replicas) {
    for (size_t i = 0; i < replicas.size(); ++i) {
        const auto* left = state.nodeAs<ConsensusReplica>(replicas[i]);
        if (left == nullptr) return {false, replicas[i].str() + " is missing"};
        for (size_t j = i + 1; j < replicas.size(); ++j) {
            const auto* right = state.nodeAs<ConsensusReplica>(replicas[j]);
            if (right == nullptr) {
                return {false, replicas[j].str() + " is missing"};
            }
            int first = std::max(left->firstNonCleared(),
                                 right->firstNonCleared());
            int last = std::min(left->commitIndex(), right->commitIndex());
            for (int slot = first; slot <= last; ++slot) {
                auto left_command = left->slotCommand(slot);
                auto right_command = right->slotCommand(slot);
                if (!left_command.has_value() ||
                    !right_command.has_value() ||
                    !(left_command == right_command)) {
                    return {false, "committed slot " +
                                       std::to_string(slot) +
                                       " differs between " +
                                       replicas[i].str() + " and " +
                                       replicas[j].str()};
...
CheckResult consensusNoDoubleApply(
    const SearchState& state,
    const std::vector<Address>& replicas) {
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        ...
        for (const auto& [request, count] :
             node->executionCountsForTest()) {
            if (count > 1) {
                return {false, replica.str() +
...
CheckResult consensusClientRepliesAreCommitted(
    const SearchState& state,
    const std::vector<Address>& replicas) {
    for (const auto& envelope : state.network()) {
        const auto* reply = std::get_if<ClientReply>(&envelope.message);
        ...
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            if (node != nullptr &&
                node->hasCommittedRequestForTest(reply->client_id,
                                                 reply->request_id)) {
                committed = true;
                break;
            }
        ...
        if (!committed) {
            return {false, "reply for client " +
                               std::to_string(reply->client_id) +
                               " request " +
                               std::to_string(reply->request_id) +
                               " has no committed request"};
        }
    }
    return {true, ""};
}

SearchState catchUpConsensusFollower(SearchState state,
                                     const SearchSettings& settings,
                                     const Address& leader,
                                     const Address& follower) {
    EventRef first_append = requireMessageEvent(
        state,
        settings,
        [&](const MessageEnvelope& envelope) {
            const auto* append =
        ...
        "initial catch-up append");
    state = stepRequired(state, first_append, settings);
    ...
    EventRef first_reply = requireMessageEvent(
        ...
        [&](const MessageEnvelope& envelope) {
            ...
            return reply != nullptr &&
                   reply->replica == follower.rootAddress() &&
                   envelope.from.rootAddress() == follower.rootAddress() &&
                   envelope.to.rootAddress() == leader.rootAddress();
        },
        "initial catch-up reply");
    const auto* reply_envelope = messageForEvent(state, first_reply);
    bool success = std::get_if<AppendReply>(&reply_envelope->message)->success;
    ...
    state = stepRequired(
        ...
        requireMessageEvent(
            ...
            "prefix catch-up reply"),
        settings);
    return state;
}

SearchState catchUpConsensusFollowerThrough(SearchState state,
                                            const SearchSettings& settings,
                                            const Address& leader,
                                            const Address& follower,
                                            int target_commit_index) {
    for (int attempt = 0; attempt < 8; ++attempt) {
        const auto* node = state.nodeAs<ConsensusReplica>(follower);
        if (node != nullptr &&
            node->commitIndex() >= target_commit_index &&
            node->appliedIndex() >= target_commit_index) {
            return state;
        }
        state = catchUpConsensusFollower(state, settings, leader, follower);
    }
    return state;
}

SearchState fireConsensusAppendRetry(SearchState state,
                                     const SearchSettings& settings,
                                     const Address& leader) {
    int generation = 0;
    auto timers_for_leader = state.timers().find(leader.rootAddress());
    if (timers_for_leader != state.timers().end()) {
        for (const auto& timer : timers_for_leader->second) {
            if (const auto* retry =
                    std::get_if<AppendRetryTimer>(&timer.timer)) {
                generation = std::max(generation, retry->generation);
            }
        }
    }
    if (generation == 0) {
        throw std::runtime_error("missing append retry timer for " +
                                 leader.str());
    }
    return stepRequired(
        state,
        requireTimerEvent(
            ...
            [&](const TimerEnvelope& timer) {
                const auto* retry =
                    std::get_if<AppendRetryTimer>(&timer.timer);
                return timer.to.rootAddress() == leader.rootAddress() &&
                       retry != nullptr &&
                       retry->generation == generation;
            },
            "append retry for " + leader.str()),
        settings);
}
...
bool catalogHasCellOn(const ConsensusReplica* node,
                      const std::string& table,
                      const std::string& value) {
    Result result = TableNotFoundResult{};
    const auto* rows = catalogRowsOn(node, table, &result);
...
bool catalogHasRowPrefixOn(const ConsensusReplica* node,
                           const std::string& table,
    ...
    if (rows == nullptr) return false;
    for (const auto& row : rows->rows) {
        if (row.size() < prefix.size()) continue;
        bool matches = true;
...
CheckResult replicaGroupConfigHistoryValid(
    const SearchState& state,
    const std::vector<Address>& replicas) {
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        ...
        const auto* rows = catalogRowsOn(node, "__replica_groups", &storage);
        if (rows == nullptr) {
            return {false, replica.str() +
                               " cannot read replica group catalog"};
        }

        std::map<std::string, int> last_version;
        std::map<std::string, std::set<int>> seen_versions;
        for (const auto& row : rows->rows) {
            auto group = selectAllValue(*rows, row, "group_id");
            auto version_text =
                selectAllValue(*rows, row, "config_version");
            auto voters_text = selectAllValue(*rows, row, "voters");
            auto old_text =
                selectAllValue(*rows, row, "joint_old_voters");
            auto new_text =
                selectAllValue(*rows, row, "joint_new_voters");
            auto phase = selectAllValue(*rows, row, "phase");
            if (!group || group->empty() || !version_text ||
                !voters_text || !old_text || !new_text || !phase) {
                return {false, replica.str() +
                                   " has malformed replica group row"};
            }

            int version = 0;
            try {
                version = std::stoi(*version_text);
            } catch (const std::exception&) {
                return {false, replica.str() +
                                   " has non-numeric config version"};
            }
            if (version <= 0) {
                return {false, replica.str() +
                                   " has non-positive config version"};
            }
            if (seen_versions[*group].count(version) != 0) {
                return {false, replica.str() +
                                   " has duplicate config version " +
                                   std::to_string(version) +
                                   " for " + *group};
            }
            seen_versions[*group].insert(version);
            auto last = last_version.find(*group);
            if (last != last_version.end() && version <= last->second) {
                return {false, replica.str() +
                                   " config versions do not move forward"};
            }
            last_version[*group] = version;

            auto voters = canonicalCatalogList(splitCatalogList(*voters_text));
            auto old_voters =
                canonicalCatalogList(splitCatalogList(*old_text));
            auto new_voters =
                canonicalCatalogList(splitCatalogList(*new_text));
            if (voters.empty()) {
                return {false, replica.str() +
                                   " has config row without voters"};
            }
            if (*phase == "stable") {
                if (!old_voters.empty() || !new_voters.empty()) {
                    return {false, replica.str() +
                                       " stable config keeps joint voters"};
                }
            } else if (*phase == "joint") {
                if (old_voters.empty() || new_voters.empty()) {
                    return {false, replica.str() +
                                       " joint config lacks both voter sets"};
                }
            } else {
                return {false, replica.str() +
                                   " has unknown config phase " + *phase};
            }
        }
    }
    return {true, ""};
...
CheckResult catalogTableMatchesAcrossReplicas(
    const SearchState& state,
    const std::vector<Address>& replicas,
    const std::string& table) {
    std::optional<SelectAllResult> first;
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        if (node == nullptr) {
            return {false, replica.str() + " is missing"};
        ...
        Result storage = TableNotFoundResult{};
        const auto* rows = catalogRowsOn(node, table, &storage);
        if (rows == nullptr) {
            return {false, replica.str() + " cannot read " + table};
        }
        if (!first.has_value()) {
            first = *rows;
        } else if (!(*rows == *first)) {
            return {false, table + " differs on " + replica.str()};
        }
    }
    return {true, ""};
}

std::vector<NamedPredicate> consensusAndConfigInvariants(
    std::vector<Address> replicas) {
    return {
        {"ONE_LEADER_PER_TERM",
        ...
        {"CONSENSUS_LOGS_CONSISTENT",
         [replicas](const SearchState& state) {
             return consensusLogsConsistentAllSlots(state, replicas);
         }},
        {"APPLIED_PREFIX_VALID",
         [replicas](const SearchState& state) {
             return consensusAppliedPrefixValid(state, replicas);
         }},
        {"COMMITTED_PREFIX_IDENTICAL",
         [replicas](const SearchState& state) {
             return consensusCommittedPrefixesIdentical(state, replicas);
         }},
        {"NO_DOUBLE_APPLY",
         [replicas](const SearchState& state) {
             return consensusNoDoubleApply(state, replicas);
         }},
        {"CLIENT_REPLIES_ARE_COMMITTED",
         [replicas](const SearchState& state) {
             return consensusClientRepliesAreCommitted(state, replicas);
         }},
        {"REPLICA_GROUP_CONFIG_HISTORY_VALID",
         [replicas](const SearchState& state) {
             return replicaGroupConfigHistoryValid(state, replicas);
         }}
    };
}

void addConsensusAndConfigInvariants(SearchSettings& settings,
                                     const std::vector<Address>& replicas) {
    auto invariants = consensusAndConfigInvariants(replicas);
    settings.invariants.insert(settings.invariants.end(),
                               invariants.begin(),
                               invariants.end());
}
...
Command discoverServer4Command() {
    return DiscoverClusterNodeCommand{
        "cluster-A", "server4", "learner", "discovered", 2};
}

Command addServer4LearnerCommand(int version = 2) {
    return AddLearnerToGroupCommand{"group-1", "server4", version};
}

Command markServer4CaughtUpCommand(int version = 3) {
    return MarkLearnerCaughtUpCommand{"group-1", "server4", version};
}

Command beginAddServer4JointCommand(int version = 4) {
    return BeginJointConfigCommand{
        "group-1",
...
Command finalizeAddServer4Command(int version = 5) {
    return FinalizeJointConfigCommand{
        "group-1",
        {"server1", "server2", "server3", "server4"},
        {"server1", "server3"},
        {"server1", "server2", "server4"},
...
Command beginRemoveServer1JointCommand() {
    return BeginJointConfigCommand{
        "group-1",
        {"server2", "server3"},
        {"server1", "server2"},
...
Command finalizeRemoveServer1Command() {
    return FinalizeJointConfigCommand{
        "group-1",
        {"server2", "server3"},
        {"server1", "server2"},
...
SearchState electedConsensusState(const SearchSettings& settings,
                                  const std::vector<Address>& replicas,
                                  const Address& leader) {
    SearchState state = consensusClusterState(replicas);
    return electConsensusLeader(state, settings, leader,
...
SearchState buildInitialReplicaGroupState(
    ...
    const Address& client,
    int client_id) {
    SearchState state = electedConsensusState(settings, replicas, leader);
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 1,
        bootstrapClusterACommand(), {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 2,
        registerGroup1Command(), {replicas[1], replicas[2]});
    return state;
}

SearchState buildServer4ReadyState(const SearchSettings& settings,
                                   const std::vector<Address>& replicas,
                                   const Address& leader,
                                   const Address& client,
                                   int client_id) {
    SearchState state = buildInitialReplicaGroupState(
        settings, replicas, leader, client, client_id);
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 3,
        discoverServer4Command(), {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 4,
        addServer4LearnerCommand(), {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 5,
        markServer4CaughtUpCommand(), {replicas[1], replicas[2]});
    return state;
}

SearchState buildServer4PromotedState(const SearchSettings& settings,
                                      const std::vector<Address>& replicas,
                                      const Address& leader,
                                      const Address& client,
                                      int client_id) {
    SearchState state = buildServer4ReadyState(
        settings, replicas, leader, client, client_id);
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 6,
        beginAddServer4JointCommand(), {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, client_id, 7,
        finalizeAddServer4Command(), {replicas[1], replicas[2]});
    return state;
}

bool searchFinishedWithoutInvariantFailure(const SearchResults& result) {
    return result.end_condition !=
               SearchResults::EndCondition::InvariantViolated &&
...
void printMembershipTrace(const std::string&) {
    std::cout << "Trace: replica-group membership metadata" << std::endl;
    ...
    state = electConsensusLeader(state, settings, leader,
                                 {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 1,
        BootstrapClusterCommand{
            "cluster-A", "server1",
            {"server1", "server2", "server3"}, 1},
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 2,
        RegisterReplicaGroupCommand{
            "group-1", {"server1", "server2", "server3"}, 1},
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 3,
        DiscoverClusterNodeCommand{
            "cluster-A", "server4", "learner", "discovered", 2},
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 4,
        AddLearnerToGroupCommand{"group-1", "server4", 2},
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 5,
        MarkLearnerCaughtUpCommand{"group-1", "server4", 3},
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 6,
        BeginJointConfigCommand{
            "group-1",
            {"server1", "server2", "server3", "server4"},
            {"server1", "server2"},
        ...
        {replicas[1], replicas[2]});
    state = deliverConsensusCommandToQuorum(
        state, settings, leader, client, 1, 7,
        FinalizeJointConfigCommand{
            "group-1",
            {"server1", "server2", "server3", "server4"},
            {"server1", "server3"},
    ...
    std::cout << "  leader=" << leader
              ...
              << ", __cluster_identity rows="
              << catalogRowCountOn(leader_node, "__cluster_identity")
              << ", __replica_groups rows="
              << catalogRowCountOn(leader_node, "__replica_groups")
              << std::endl;
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        std::cout << "  " << replica
                  << " has final group config="
                  << (catalogHasRowOn(
                          node, "__replica_groups",
                          {{"group_id", "group-1"},
                           {"config_version", "5"},
                           {"voters",
                            "server1,server2,server3,server4"},
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v124; "
                  << "this version focuses on config and replication invariants."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v124: config and replication invariant suite" << std::endl;
    ...
    TestRunner tests;
    tests.test("Invariant suite holds during catalog update search", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        ...
        state.send(client, leader,
                   ClientRequest{421, 1, bootstrapClusterACommand()});

        SearchSettings search;
        search.max_depth = 7;
        search.max_states = 800;
        search.deliverTimers(false);
        ...
        tests.check(searchFinishedWithoutInvariantFailure(result),
                    "catalog update search failed: " +
                        endConditionName(result.end_condition) + " " +
                        result.message);
    });

    tests.test("Invariant suite holds during membership-change search", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings scripted;
        SearchState state = buildServer4ReadyState(
            scripted, replicas, leader, client, 422);
        state.send(client, leader,
                   ClientRequest{422, 6, beginAddServer4JointCommand()});

        SearchSettings search;
        search.max_depth = 8;
        search.max_states = 1200;
        search.deliverTimers(false);
        addConsensusAndConfigInvariants(search, replicas);
        SearchResults result = bfs(state, search);
        tests.check(searchFinishedWithoutInvariantFailure(result),
                    "membership-change search failed: " +
                        endConditionName(result.end_condition) + " " +
                        result.message);
    });

    tests.test("Duplicate request across config change applies once", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        SearchState state = buildServer4ReadyState(
            settings, replicas, leader, client, 423);
        Command begin_joint = beginAddServer4JointCommand();
        Command finalize = finalizeAddServer4Command();
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 423, 6, begin_joint,
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 423, 7, finalize,
            {replicas[1], replicas[2]});

        const auto* leader_node = state.nodeAs<ConsensusReplica>(leader);
        int last_slot = leader_node == nullptr ? 0 : leader_node->lastNonEmpty();
        state.send(client, leader, ClientRequest{423, 7, finalize});
        state = stepRequired(
            state,
            requireMessageEvent(
                state,
        ...
        for (const auto& replica : replicas) {
            ...
            tests.check(node != nullptr &&
                            node->executionCount(423, 7) == 1,
                        replica.str() +
                            " should apply finalization once");
        }
        tests.check(clientReplyResultInNetworkFor(
                        state, 423, 7, Result{StatementOkResult{}}),
                    "duplicate finalization should return cached result");
        leader_node = state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->lastNonEmpty() == last_slot,
                    "duplicate finalization should not append a new slot");
    });

    tests.test("Removed leader cannot commit new work", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        SearchState state = buildInitialReplicaGroupState(
            settings, replicas, leader, client, 424);
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 424, 3,
            beginRemoveServer1JointCommand(), {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 424, 4,
            finalizeRemoveServer1Command(), {replicas[1], replicas[2]});

        const auto* old_leader = state.nodeAs<ConsensusReplica>(leader);
        tests.check(old_leader != nullptr &&
                        old_leader->role() == ConsensusRole::Follower,
                    "removed leader should be a follower");
        int last_slot = old_leader == nullptr ? 0 : old_leader->lastNonEmpty();
        Command after_removal =
            RegisterRangeCommand{"range-after-remove", "/a", "/b",
                                 "group-1", 1};
        state.send(client, leader,
                   ClientRequest{424, 5, after_removal});
        state = stepRequired(
            state,
            requireMessageEvent(
                state,
                ...
                [&](const MessageEnvelope& envelope) {
                    const auto* request =
                        std::get_if<ClientRequest>(&envelope.message);
                    return request != nullptr &&
                           request->client_id == 424 &&
                           request->request_id == 5 &&
                           envelope.to.rootAddress() == leader;
                },
                "request to removed leader"),
            settings);

        tests.check(!clientReplyInNetworkFor(state, 424, 5),
                    "removed leader should not reply to new work");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(node != nullptr &&
                            node->lastNonEmpty() == last_slot,
                        replica.str() +
                            " should not append after leader removal");
            tests.check(!catalogHasRowOn(
                            node, "__ranges",
                            {{"range_id", "range-after-remove"}}),
                        replica.str() +
                            " should not apply work from removed leader");
        }
    });

    tests.test("Catalog metadata matches across caught-up replicas", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        SearchState state = buildServer4PromotedState(
            settings, replicas, leader, client, 425);

        tests.check(catalogTableMatchesAcrossReplicas(
                        state, replicas, "__cluster_identity").ok,
                    "cluster identity should match on all replicas");
        tests.check(catalogTableMatchesAcrossReplicas(
                        state, replicas, "__cluster_nodes").ok,
                    "cluster nodes should match on all replicas");
        CheckResult groups = catalogTableMatchesAcrossReplicas(
            state, replicas, "__replica_groups");
        tests.check(groups.ok, groups.message);
        tests.check(replicaGroupConfigHistoryValid(state, replicas).ok,
                    "replica group history should be structurally valid");
    });

    tests.test("Bounded random search keeps consensus invariants", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client("client-random");
        SearchSettings scripted;
        SearchState state = electedConsensusState(scripted, replicas, leader);
        std::vector<Command> commands{
            bootstrapClusterACommand(),
            registerGroup1Command(),
            discoverServer4Command()
        };
        ...
        state.addNode(std::make_unique<ConsensusClientWorker>(
            client, replicas, 426, Workload(commands, expected)));

        SearchSettings search;
        search.max_depth = 18;
        search.max_states = 900;
        search.random_dfs_probes = 40;
        search.seed = 124;
        addConsensusAndConfigInvariants(search, replicas);
    ...
    tests.test("Invariant suite still permits valid full membership flow", [&] {
        ...
        SearchSettings settings;
        SearchState state = buildServer4ReadyState(
            settings, replicas, leader, client, 427);
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 427, 6,
            beginAddServer4JointCommand(), {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 427, 7,
            finalizeAddServer4Command(), {replicas[1], replicas[2]});
        ...
        for (const auto& replica : replicas) {
            ...
            tests.check(catalogHasRowOn(
                            ...
                            {{"group_id", "group-1"},
                             ...
                             {"phase", "stable"}}),
                        replica.str() +
                            " should have final promoted config");
        }
    });
