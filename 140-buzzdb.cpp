...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v140; "
    ...
    std::cout << "BuzzDB v140: HLC MVCC and distributed transaction liveness"
              << std::endl;
    ...
    tests.test("HLC MVCC snapshot reader keeps old version after commit", [&] {
        withLocalMVCCDB([&](BuzzDB& db) {
            auto seed = db.begin("seed");
            db.executeStatement("INSERT kv|1|10", seed, 0, false);
            db.commit(seed);

            auto reader = db.begin("reader");
            auto writer = db.begin("writer");
            db.executeStatement("UPDATE kv SET value=20 WHERE id=1", writer, 0, false);
            db.commit(writer);

            tests.check(localValueFor(db, reader) == 10,
                        "old reader should keep its read timestamp snapshot");
            tests.check(localValueFor(db, nullptr) == 20,
                        "new reads should see the committed MVCC version");
            db.commit(reader);
        });
    });

    tests.test("HLC MVCC updater reads its own unpublished version", [&] {
        withLocalMVCCDB([&](BuzzDB& db) {
            auto seed = db.begin("seed");
            db.executeStatement("INSERT kv|1|10", seed, 0, false);
            db.commit(seed);
            ...
            auto writer = db.begin("writer");
            db.executeStatement("UPDATE kv SET value=20 WHERE id=1", writer, 0, false);
            tests.check(localValueFor(db, writer) == 20,
                        "writer should read its own uncommitted successor version");
            tests.check(localValueFor(db, nullptr) == 10,
                        "outside readers should not see unpublished versions");
            db.commit(writer);
            tests.check(localValueFor(db, nullptr) == 20,
                        "committed writer version should become the latest row");
        });
    });

    tests.test("HLC MVCC abort discards unpublished versions", [&] {
        withLocalMVCCDB([&](BuzzDB& db) {
            auto seed = db.begin("seed");
            db.executeStatement("INSERT kv|1|10", seed, 0, false);
            db.commit(seed);

            auto updater = db.begin("aborted-updater");
            db.executeStatement("UPDATE kv SET value=20 WHERE id=1", updater, 0, false);
            tests.check(localValueFor(db, updater) == 20,
                        "writer should see its unpublished successor before abort");
            db.abort(updater);
            tests.check(localValueFor(db, nullptr) == 10,
                        "aborted update should leave the committed predecessor visible");

            auto inserter = db.begin("aborted-inserter");
            db.executeStatement("INSERT kv|2|99", inserter, 0, false);
            db.abort(inserter);
            QueryTable inserted =
                db.executeQuery("PROJECT * FROM kv WHERE {id}=2", nullptr, false);
            tests.check(inserted.empty(),
                        "aborted insert should not leave a visible version");

            auto next = db.begin("next-writer");
            db.executeStatement("UPDATE kv SET value=30 WHERE id=1", next, 0, false);
            db.commit(next);
            tests.check(localValueFor(db, nullptr) == 30,
                        "later writer should commit after aborted versions are cleaned");
        });
    });

    tests.test("HLC MVCC rejects concurrent writers without 2PL waiting", [&] {
        withLocalMVCCDB([&](BuzzDB& db) {
            auto seed = db.begin("seed");
            db.executeStatement("INSERT kv|1|10", seed, 0, false);
            db.commit(seed);

            auto first = db.begin("first-writer");
            auto second = db.begin("second-writer");
            db.executeStatement("UPDATE kv SET value=20 WHERE id=1", first, 0, false);

            bool conflict = false;
            try {
                db.executeStatement("UPDATE kv SET value=30 WHERE id=1", second, 0, false);
            } catch (const std::runtime_error& error) {
                conflict = std::string(error.what()).find("MVCC") != std::string::npos;
            }
            tests.check(conflict,
                        "second writer should fail on MVCC successor conflict");
            db.abort(second);
            db.commit(first);
            tests.check(localValueFor(db, nullptr) == 20,
                        "first writer should still commit after rejecting the conflict");
        });
    });

    tests.test("HLC MVCC stores numeric version timestamps in tuple metadata", [&] {
        withLocalMVCCDB([&](BuzzDB& db) {
            auto seed = db.begin("seed");
            db.executeStatement("INSERT kv|1|10", seed, 0, false);
            db.commit(seed);
            ...
            auto& metadata = db.catalog.getTable("kv");
            TableHeap heap(metadata, db.buffer_manager);
            auto versions = heap.readAllTuples();
            bool saw_old_closed = false;
            bool saw_current = false;
            for (const auto& version : versions) {
                if (version->fields.size() < 2 ||
                    version->fields[0]->asInt() != 1 ||
                    !version->has_mvcc_metadata) {
                    continue;
                }
                if (version->fields[1]->asInt() == 10) {
                    saw_old_closed =
                        version->mvcc.begin.physical >= 0 &&
                        version->mvcc.end.physical > 0 &&
                        !version->mvcc.end.isInfinity();
                }
                if (version->fields[1]->asInt() == 20) {
                    saw_current =
                        version->mvcc.committed &&
                        version->mvcc.begin.physical > 0 &&
                        version->mvcc.end.isInfinity();
                }
            }
            tests.check(saw_old_closed && saw_current,
                        "physical versions should carry numeric HLC begin/end metadata");
        });
    });
    ...
    tests.test("Range transfer moves physical index entries before ownership switch", [&] {
        ...
        auto caught_up = latestTransferOn(scenario.catalog, range_id);
        tests.check(caught_up.has_value() &&
                        caught_up->status == "caught_up" &&
                        caught_up->source_copy_key_count == source_before &&
                        caught_up->target_copy_key_count == source_before,
                    "catch-up should record runtime-measured source and target copy proof");
        tests.check(indexRouteGroupOnCatalog(scenario.catalog, year) ==
                        scenario.source.group_id,
    ...
    tests.test("Catalog-only transfer cannot commit without copy proof", [&] {
        auto scenario = buildIndexMovementScenario();
        std::string range_id = defaultRangeIdForIndex(title_year_index);
        std::string year = scenario.fixture.duplicate_year;
        size_t source_before = physicalIndexEntryCount(scenario.source, range_id);
        ...
        tests.check(commitCommand(
                        ...
                        MoveRangeCommand{range_id, scenario.target.group_id}) ==
                        Result{StatementOkResult{}},
                    "move should create a requested transfer");
        auto requested = latestTransferOn(scenario.catalog, range_id);
        tests.check(requested.has_value(),
                    "requested transfer should be visible in catalog");
        int epoch = requested->transfer_epoch;
        tests.check(commitCommand(
                        scenario.catalog,
                        PrepareRangeTransferCommand{
                            range_id, scenario.target.group_id, epoch}) ==
                        Result{StatementOkResult{}},
                    "direct prepare may snapshot metadata but cannot copy data");
        tests.check(physicalIndexEntryCount(scenario.target, range_id) == 0,
                    "direct catalog prepare should not invent target rows");
        tests.check(commitCommand(
                        scenario.catalog,
                        CatchUpRangeTransferCommand{range_id, epoch}) ==
                        Result{ConfigRejectedResult{}},
                    "catch-up without runtime copy proof should be rejected");
        tests.check(commitCommand(
                        scenario.catalog,
                        CommitRangeTransferCommand{range_id, epoch}) ==
                        Result{ConfigRejectedResult{}},
                    "commit should reject a transfer without copied target rows");
        tests.check(indexRouteGroupOnCatalog(scenario.catalog, year) ==
                        scenario.source.group_id &&
                        physicalIndexEntryCount(scenario.source, range_id) ==
                            source_before &&
                        physicalIndexEntryCount(scenario.target, range_id) == 0,
                    "failed catalog-only transfer should leave source ownership and data intact");
    });
    ...
    tests.test("Control plane runtime commits participants and ends transaction", [&] {
        ...
        auto commitTimestampFor = [&](const SelectAllResult& rows,
                                      const std::string& txn_id,
                                      const std::string& status) {
            for (const auto& row_values : rows.rows) {
                auto row_txn_id = selectAllValue(rows, row_values, "txn_id");
                auto row_status = selectAllValue(rows, row_values, "status");
                auto physical = selectAllValue(rows,
                                               row_values,
                                               "commit_physical");
                auto logical = selectAllValue(rows,
                                              row_values,
                                              "commit_logical");
                if (row_txn_id.has_value() && *row_txn_id == txn_id &&
                    row_status.has_value() && *row_status == status &&
                    physical.has_value() && logical.has_value()) {
                    return *physical + "." + *logical;
                }
            }
            return std::string{};
        };
        std::string coordinator_commit_ts = commitTimestampFor(
            catalogTableOn(scenario.catalog,
                           scenario.catalog.leader,
                           "__distributed_txns"),
            "txn-runtime-commit",
            "committed");
        std::string table_commit_ts = commitTimestampFor(
            catalogTableOn(scenario.table_owner,
                           scenario.table_owner.leader,
                           "__participant_txns"),
            "txn-runtime-commit",
            "committed");
        std::string index_commit_ts = commitTimestampFor(
            catalogTableOn(scenario.index_owner,
                           scenario.index_owner.leader,
                           "__participant_txns"),
            "txn-runtime-commit",
            "committed");
        tests.check(!coordinator_commit_ts.empty() &&
                        coordinator_commit_ts != "0.0" &&
                        coordinator_commit_ts == table_commit_ts &&
                        coordinator_commit_ts == index_commit_ts,
                    "coordinator commit timestamp should be durably shared by participants");
        tests.check(firstRowValue(
                        selectTitleById(scenario.table_owner, row.values[0]),
                        "production_year") == new_year,
