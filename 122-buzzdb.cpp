...
void printV104BootstrapTrace(const std::string& data_file) {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v122-bootstrap-trace-" + std::to_string(::getpid()));
...
void printClusterBootstrapTrace(const std::string&) {
    std::cout << "Trace: cluster bootstrap and discovery" << std::endl;
    ...
    std::cout << "  leader=" << leader
              ...
              << ", __cluster_nodes rows="
              << catalogRowCountOn(leader_node, "__cluster_nodes")
              << std::endl;
    for (const auto& replica : replicas) {
        const auto* node = state.nodeAs<ConsensusReplica>(replica);
        std::cout << "  " << replica
                  << " has cluster-A="
                  << (catalogHasRowOn(
                          node, "__cluster_identity",
                          {{"cluster_id", "cluster-A"},
                           {"bootstrap_node_id", "server1"}})
                          ? "yes" : "no")
                  << ", server4 learner="
                  << (catalogHasRowOn(
                          node, "__cluster_nodes",
                          {{"node_id", "server4"},
                           {"role", "learner"},
                           {"status", "discovered"}})
                          ? "yes" : "no")
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v122; "
                  << "this version focuses on cluster bootstrap and discovery."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v122: cluster bootstrap and node discovery" << std::endl;
    const std::string imdb_file =
        argc > imdb_arg ? argv[imdb_arg] : defaultImdbInputFile();
    if (!tests_only) {
        printLocalBuzzDBTrace(imdb_file);
        printClusterBootstrapTrace(imdb_file);
        printV104BootstrapTrace(imdb_file);
    }

    TestRunner tests;
    tests.test("First node bootstraps cluster id through Raft", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        Command bootstrap = BootstrapClusterCommand{
            "cluster-A", "server1",
            {"server1", "server2", "server3"}, 1};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 221, 1, bootstrap,
            {replicas[1], replicas[2]});
        ...
        for (const auto& replica : replicas) {
            ...
            tests.check(catalogHasRowOn(
                            ...
                            {{"cluster_id", "cluster-A"},
                             {"epoch", "1"},
                             {"bootstrap_node_id", "server1"},
                             {"seed_nodes", "server1,server2,server3"}}),
                        replica.str() +
                            " should have the replicated cluster identity");
            tests.check(catalogHasRowOn(
                            node, "__cluster_nodes",
                            {{"node_id", "server1"},
                             {"role", "voter"},
                             ...
                             {"epoch", "1"}}),
                        replica.str() +
                            " should register the bootstrap node");
        }
    });

    tests.test("Duplicate bootstrap does not create two clusters", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        Command bootstrap = BootstrapClusterCommand{
            ...
            {"server1", "server2", "server3"}, 1};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 222, 1, bootstrap,
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 222, 2, bootstrap,
            {replicas[1], replicas[2]});

        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(catalogRowCountOn(node, "__cluster_identity") == 1,
                        replica.str() +
                            " should have exactly one cluster identity row");
            tests.check(catalogRowCountOn(node, "__cluster_nodes") == 1,
                        replica.str() +
                            " should not duplicate the bootstrap node");
            tests.check(catalogHasRowOn(
                            node, "__cluster_identity",
                            {{"cluster_id", "cluster-A"},
                             {"seed_nodes", "server1,server2,server3"}}),
                        replica.str() +
                            " should keep the original bootstrap metadata");
        }
    });

    tests.test("Restarted node reloads cluster id and node id", [&] {
        std::string database_file = makeScratchBuzzDBFile();
        std::filesystem::path cleanup_dir =
            std::filesystem::path(database_file).parent_path();
        try {
            runWithSuppressedStdout([&]() {
                ...
                tests.check(
                    ...
                    "bootstrap should succeed before restart");
                db.execute(CheckpointCommand{});
                return 0;
            });
            ...
            runWithSuppressedStdout([&]() {
                ...
                Result identity =
                    reopened.execute(ReadClusterIdentityCommand{});
                const auto* identity_rows =
                    std::get_if<SelectAllResult>(&identity);
                tests.check(
                    ...
                    "reopened database should reload cluster identity");
                Result nodes =
                    reopened.execute(ReadSystemCatalogCommand{
                        "__cluster_nodes"});
                const auto* node_rows = std::get_if<SelectAllResult>(&nodes);
                tests.check(
                    node_rows != nullptr &&
                        selectAllHasRow(
                            *node_rows,
                            {{"node_id", "server1"},
                             ...
                             {"status", "active"},
                             {"epoch", "1"}}),
                    "reopened database should reload node identity");
                return 0;
            });
        } catch (...) {
            std::error_code ec;
    ...
    tests.test("Node with wrong cluster id is rejected", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 224, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        Command wrong_cluster = DiscoverClusterNodeCommand{
            "cluster-B", "server9", "learner", "discovered", 2};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 224, 2, wrong_cluster,
            {replicas[1], replicas[2]});

        const auto* leader_node = state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->slotCommandMatches(2, wrong_cluster),
                    "foreign discovery should still have one ordered log slot");
        tests.check(clientReplyResultInNetworkFor(
                        state, 224, 2, Result{ClusterRejectedResult{}}),
                    "foreign discovery should reply ClusterRejected");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(!catalogHasRowOn(node, "__cluster_nodes",
                                         {{"node_id", "server9"}}),
                        replica.str() +
                            " should not register the foreign node");
        }
    });

    tests.test("New node discovers current cluster metadata", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 225, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        Command discover = DiscoverClusterNodeCommand{
            "cluster-A", "server4", "learner", "discovered", 2};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 225, 2, discover,
            {replicas[1], replicas[2]});

        tests.check(clientReplyResultInNetworkFor(
                        state, 225, 2, Result{StatementOkResult{}}),
                    "valid discovery should reply StatementOk");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(catalogHasRowOn(
                            node, "__cluster_identity",
                            {{"cluster_id", "cluster-A"},
                             {"bootstrap_node_id", "server1"}}),
                        replica.str() +
                            " should expose the current cluster identity");
            tests.check(catalogHasRowOn(
                            node, "__cluster_nodes",
                            {{"node_id", "server4"},
                             {"role", "learner"},
                             ...
                             {"epoch", "2"}}),
                        replica.str() +
                            " should replicate the discovered node");
        }
    });

    tests.test("Discovered node remains non-voting until membership change", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 226, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 226, 2,
            DiscoverClusterNodeCommand{
                "cluster-A", "server4", "learner", "discovered", 2},
            {replicas[1], replicas[2]});
        ...
        const auto* leader_node = state.nodeAs<ConsensusReplica>(leader);
        tests.check(leader_node != nullptr &&
                        leader_node->majoritySize() == 2,
                    "discovery should not change the voting quorum");
        tests.check(state.nodeAs<ConsensusReplica>(Address("server4")) ==
                        nullptr,
                    "discovery should not create a voting replica");
        tests.check(leader_node != nullptr &&
                        catalogHasRowOn(
                            ...
                            {{"node_id", "server4"},
                             {"role", "learner"},
                             {"status", "discovered"}}),
                    "discovered node should be cataloged as a learner");
    });
