...
public:
    ...
    void printAnalyzeStats(const QueryComponents& components,
        ...
        std::set<TableId> printed_tables;
        for (const auto& table_ref : components.table_refs) {
            if (!printed_tables.insert(table_ref.table_id).second) {
                continue;
            }

            auto& metadata = catalog.getTable(table_ref.table_id);
            const auto& table_stats = stats.at(table_ref.table_id);
            std::cout << "  " << table_stats.table_name
                      << " rows=" << table_stats.row_count
                      << " pages=" << table_stats.page_count
                      << std::endl;

            for (size_t column = 0; column < metadata.schema.columns.size(); column++) {
                const auto& column_schema = metadata.schema.columns[column];
                const auto& column_stats = table_stats.columns.at(column);
                std::cout << "    " << column_schema.name
                          << " type=" << fieldTypeName(column_stats.type)
                          << " distinct=" << column_stats.distinct_count;
                if (column_stats.min_value && column_stats.max_value) {
                    std::cout << " min=" << fieldToString(*column_stats.min_value)
                              << " max=" << fieldToString(*column_stats.max_value);
                }
                std::cout << std::endl;
            }
        }
    ...
    double estimateEqualityFilterRows(double input_rows,
                                      const ColumnStats& column_stats) const {
    ...
    double estimateJoinRows(double left_rows,
                            ...
                            const ColumnStats& right_stats) const {
        if (left_rows <= 0.0 || right_rows <= 0.0) {
            return 0.0;
        }

        size_t denominator = std::max(
    ...
    std::map<TableRefId, double> estimateFilteredBaseRows(
            const QueryComponents& components,
            const std::map<TableId, TableStats>& stats) const {
...
double qError(double estimate, double actual) {
    if (estimate <= 0.0 || actual <= 0.0) {
        return estimate == actual ? 1.0 : std::numeric_limits<double>::infinity();
    }
    return std::max(estimate / actual, actual / estimate);
}

std::string formatQError(double estimate, double actual) {
    double error = qError(estimate, actual);
    if (!std::isfinite(error)) {
        return "inf";
    }
    return formatEstimate(error);
}

bool rowMatchesTableFilters(const QueryComponents& components,
                            TableRefId table_ref_id,
                            const std::vector<std::unique_ptr<Field>>& row) {
    for (const auto& filter : components.filters) {
        if (filter.column.table_ref_id != table_ref_id) {
            continue;
        }
        if (!(*row[filter.column.column_index] == *filter.value)) {
            return false;
        }
    }
    return true;
...
std::vector<std::vector<std::unique_ptr<Field>>> collectFilteredRows(
        const QueryComponents& components,
        const TableRef& table_ref,
        Catalog& catalog,
        PageManager& page_manager) {
    std::vector<std::vector<std::unique_ptr<Field>>> rows;
    auto& metadata = catalog.getTable(table_ref.table_id);
    TableHeap table_heap(metadata, page_manager);
...
void printEstimateLine(const std::string& label,
                       double estimate,
                       size_t actual) {
    std::cout << "  " << label
...
std::vector<std::vector<std::unique_ptr<Field>>> cloneRows(
        const std::vector<std::vector<std::unique_ptr<Field>>>& rows) {
    std::vector<std::vector<std::unique_ptr<Field>>> cloned;
    for (const auto& row : rows) {
        cloned.push_back(cloneFields(row));
    }
    return cloned;
}

std::string fieldKey(const Field& field) {
    return std::to_string(static_cast<int>(field.getType())) + ":" +
           fieldToString(field);
}

std::vector<std::vector<std::unique_ptr<Field>>> hashJoinRows(
        const std::vector<std::vector<std::unique_ptr<Field>>>& left_rows,
        const std::vector<std::vector<std::unique_ptr<Field>>>& right_rows,
        size_t left_attr_index,
        size_t right_attr_index) {
    std::unordered_map<
        std::string,
        std::vector<const std::vector<std::unique_ptr<Field>>*>
    > right_by_key;

    for (const auto& right_row : right_rows) {
        right_by_key[fieldKey(*right_row[right_attr_index])].push_back(
            &right_row
        );
    }

    std::vector<std::vector<std::unique_ptr<Field>>> joined_rows;
    for (const auto& left_row : left_rows) {
        auto matches = right_by_key.find(fieldKey(*left_row[left_attr_index]));
        if (matches == right_by_key.end()) {
            continue;
        }

        for (const auto* right_row : matches->second) {
            auto output = cloneFields(left_row);
            for (const auto& field : *right_row) {
                output.push_back(field->clone());
            }
            joined_rows.push_back(std::move(output));
        }
    }
...
bool rowMatchesColumnFilters(
        const QueryComponents& components,
        const std::map<TableRefId, size_t>& table_offsets,
        const std::vector<std::unique_ptr<Field>>& row) {
    for (const auto& filter : components.column_filters) {
        if (!(*row[physicalOffset(filter.left, table_offsets)] ==
              *row[physicalOffset(filter.right, table_offsets)])) {
            return false;
        }
    }
    return true;
...
void printCardinalityEstimates(
        const QueryComponents& components,
        QueryOptimizer& optimizer,
        Catalog& catalog,
        PageManager& page_manager,
        const std::map<TableId, TableStats>& stats) {
    std::cout << "\nCardinality estimates:" << std::endl;

    auto base_estimates = optimizer.estimateFilteredBaseRows(components, stats);
    std::map<TableRefId, std::vector<std::vector<std::unique_ptr<Field>>>>
        base_rows;
    ...
    for (const auto& filter : components.filters) {
        const auto& table_ref =
            tableRefForId(components, filter.column.table_ref_id);
        const auto& table_stats = stats.at(table_ref.table_id);
        double before_filter =
            static_cast<double>(table_stats.row_count);
        double estimate = optimizer.estimateEqualityFilterRows(
            before_filter,
            optimizer.columnStatsFor(components, stats, filter.column)
        );
        printEstimateLine(
            "Filter " + columnRefLabel(components, catalog, filter.column) +
            " = " + fieldToString(*filter.value),
            estimate,
            base_rows[filter.column.table_ref_id].size()
        );
    }
    ...
    const auto& base_ref =
        tableRefForId(components, components.base_table_ref_id);
    table_widths[base_ref.id] =
        catalog.getTable(base_ref.table_id).schema.columns.size();

    auto current_rows = cloneRows(base_rows[components.base_table_ref_id]);
    double current_estimate = base_estimates[components.base_table_ref_id];
    size_t output_width = table_widths[components.base_table_ref_id];

    for (const auto& join : components.joins) {
        const auto& input_ref = tableRefForId(
            components,
            join.input_table_ref_id
        );
        table_widths[input_ref.id] =
            catalog.getTable(input_ref.table_id).schema.columns.size();
        ...
        if (table_offsets.find(join.left.table_ref_id) != table_offsets.end() &&
            ...
            left_column = join.left;
            right_column = join.right;
            ...
            left_column = join.right;
            right_column = join.left;
        ...
        double join_estimate = optimizer.estimateJoinRows(
            current_estimate,
            base_estimates[join.input_table_ref_id],
            optimizer.columnStatsFor(components, stats, left_column),
            optimizer.columnStatsFor(components, stats, right_column)
        );

        auto joined_rows = hashJoinRows(
            current_rows,
            base_rows[join.input_table_ref_id],
            left_attr_index,
            right_attr_index
        );

        printEstimateLine(
            "Join " + columnRefLabel(components, catalog, left_column) +
            " = " + columnRefLabel(components, catalog, right_column),
            join_estimate,
            joined_rows.size()
        );
        ...
        current_rows = std::move(joined_rows);
        current_estimate = join_estimate;
    }

    size_t final_actual = 0;
    for (const auto& row : current_rows) {
        if (rowMatchesColumnFilters(components, table_offsets, row)) {
            final_actual++;
        }
    }

    printEstimateLine("Final query rows", current_estimate, final_actual);
}
...
QueryResult executeJoinQuery(const QueryComponents& components,
    ...
    for (const auto& join : components.joins) {
        ...
        rootOp = hash_join_buffers.back().get();
        table_offsets[join.input_table_ref_id] = output_width;
        output_width += table_widths[join.input_table_ref_id];
    }
...
public:
    ...
    QueryResult executeJoinPlan(const QueryComponents& components,
                                size_t sample_limit = 5) {
        return executeJoinQuery(components, catalog, page_manager, sample_limit);
    }
...
public:
    ...
    QueryResult executeJoinPlan(const QueryComponents& components,
                                size_t sample_limit = 5) {
        return query_executor.executeJoinPlan(components, sample_limit);
    }
...
void runImdbHashJoin(BuzzDB& db) {
    std::cout << "\nIMDB Query: 9-table company/rating/release-date join"
              << std::endl;

    auto join_reader = db.beginTransaction();
    auto query_start = std::chrono::high_resolution_clock::now();
    db.execute(join_reader, imdb_join_query);
    auto query_end = std::chrono::high_resolution_clock::now();
    db.commit(join_reader);
    std::chrono::duration<double> query_elapsed = query_end - query_start;
    std::cout << "Elapsed HashJoin query processing time: "
              << query_elapsed.count() << " seconds" << std::endl;
}

void runImdbAnalyzeAndHashJoin() {
    BuzzDB db;
    ...
    db.query_processor.optimizer().printAnalyzeStats(components, stats);
    ...
    std::cout << "\nResult: Stats produce cardinality estimates.\n";
}

int main() {
    try {

    std::cout << "IMDB cardinality estimation\n";

    runImdbAnalyzeAndHashJoin();
