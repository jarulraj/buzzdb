...
public:
    ...
    Result executeReplicated(int op_number,
        ...
        try {
            Result result = executeForReplicatedApply(command, txn);
    ...
    void loadFromDisk() {
        ...
        while (std::getline(in, line)) {
            ...
            if (line.rfind("record=COMMIT", 0) == 0) {
                ...
                entries_.push_back(parseEntry(line));
            }
        }
...
void printV104BootstrapTrace(const std::string& data_file) {
    ...
    std::filesystem::path dir = std::filesystem::temp_directory_path() /
        ("buzzdb-v112-bootstrap-trace-" + std::to_string(::getpid()));
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "BuzzDB v112: committed replicated log replay for primary-backup BuzzDB" << std::endl;
    ...
    tests.test("P8 duplicate unreliable delivery remains at-most-once", [&] {
        ...
        EventRef client_request = requireMessageEvent(
            ...
            "initial unreliable client request");
        state = stepRequired(state, client_request, settings);
        ...
        EventRef retry_timer = requireTimerEvent(
            ...
            "unreliable client retry timer");
        state = stepRequired(state, retry_timer, settings);

        EventRef duplicate_client_request = requireMessageEvent(
            state,
            settings,
            [&](const MessageEnvelope& envelope) {
                const auto* request =
                ...
                return request != nullptr &&
                       ...
                       request->request_id == 1 &&
                       envelope.to.rootAddress() == primary &&
                       envelope.id != client_request.id;
            },
            "duplicate unreliable client request");
        state = stepRequired(state, duplicate_client_request, settings);
        ...
        tests.check(primary_node->pendingCount() == 1,
                    "duplicate request should reuse the pending op");
        tests.check(primary_node->replicationLogSize() == 1,
                    "duplicate request should not append another op");
        ...
        tests.check(primary_node->executionCount(1, 1) == 0,
                    "primary should wait for backup before executing");
        ...
        tests.check(backup_node->executionCount(1, 1) == 1,
                    "duplicate backup apply should not re-execute");
        tests.check(backup_node->replicationLogSize() == 1,
                    "duplicate backup apply should not append another entry");
        tests.check(backup_node->commitIndex() == 1,
                    "duplicate backup apply should leave commit index stable");
    });

    tests.test("P10 concurrent creates do not commit while backup link is down", [&] {
        Address primary = ScenarioAddress::server1();
        Address backup = ScenarioAddress::backup1();
        Address client1 = ScenarioAddress::client1();
        Address client2 = ScenarioAddress::client2();
        SearchSettings settings;
        settings.linkActive(primary, backup, false);
        ...
        tests.check(primary_node != nullptr, "primary should exist");
        tests.check(primary_node->pendingCount() == 2,
                    "both creates should remain pending");
        tests.check(primary_node->commitIndex() == 0,
                    "no create should commit without backup ack");
        tests.check(primary_node->executionCount(1, 1) == 0 &&
                        primary_node->executionCount(2, 1) == 0,
                    "primary should not execute uncommitted creates");
        tests.check(first_client != nullptr && second_client != nullptr &&
                        first_client->resultCount() == 0 &&
                        second_client->resultCount() == 0,
                    "clients should not receive replies before backup ack");

        SearchSettings blocked_search = settings;
        ...
        blocked_search.goals.push_back(
            Predicates::clientsDone(std::vector<Address>{client1, client2}));
        SearchResults result = bfs(state, blocked_search);
        tests.check(result.end_condition !=
                        SearchResults::EndCondition::GoalFound,
                    "blocked backup link should not allow both creates to finish");
        tests.check(result.end_condition !=
                        SearchResults::EndCondition::InvariantViolated &&
                        result.end_condition !=
                        SearchResults::EndCondition::ExceptionThrown,
                    "blocked create search should remain safe: " +
                        result.message);
    });

    tests.test("P11 concurrent inserts do not pass prior commit when backup fails", [&] {
        Address primary = ScenarioAddress::server1();
        Address backup = ScenarioAddress::backup1();
        Address client = ScenarioAddress::client1();
        Address client2 = ScenarioAddress::client2();
        ...
        const auto* primary_node =
            state.nodeAs<PrimaryBackupServer>(primary);
        const auto* second_client = state.nodeAs<ClientWorker>(client2);
        const auto* third_client = state.nodeAs<ClientWorker>(client3);
        tests.check(primary_node != nullptr, "primary should exist");
        tests.check(primary_node->commitIndex() == 1,
                    "commit index should stay at prior committed create");
        tests.check(primary_node->pendingCount() == 2,
                    "both inserts should wait for backup ack");
        tests.check(primary_node->executionCount(2, 1) == 0 &&
                        primary_node->executionCount(3, 1) == 0,
                    "unreplicated inserts should not execute on primary");
        tests.check(second_client != nullptr && third_client != nullptr &&
                        second_client->resultCount() == 0 &&
                        third_client->resultCount() == 0,
                    "insert clients should not receive replies while backup is down");

        SearchSettings blocked_search = settings;
        blocked_search.max_depth = 6;
        blocked_search.max_states = 5000;
        blocked_search.invariants.push_back(
            Predicates::resultsOk(std::vector<Address>{client2, client3}));
        blocked_search.invariants.push_back(
            Predicates::primaryBackupAtMostOnce(primary, backup));
        blocked_search.goals.push_back(
            Predicates::clientsDone(std::vector<Address>{client2, client3}));
        SearchResults result = bfs(state, blocked_search);
        tests.check(result.end_condition !=
                        SearchResults::EndCondition::GoalFound,
                    "blocked backup link should not allow both inserts to finish");
        tests.check(result.end_condition !=
                        SearchResults::EndCondition::InvariantViolated &&
                        result.end_condition !=
                        SearchResults::EndCondition::ExceptionThrown,
    ...
    tests.test("Restart replay ignores uncommitted operation log tail", [&] {
        ...
        std::filesystem::path dir =
            ...
            ("buzzdb-v112-uncommitted-tail-test-" +
    ...
    tests.test("Restart replays committed operation missing the DB apply fence", [&] {
        ...
        std::filesystem::path dir =
            std::filesystem::temp_directory_path() /
            ("buzzdb-v112-committed-missing-fence-test-" +
             std::to_string(::getpid()));
        std::filesystem::remove_all(dir);
        std::filesystem::create_directories(dir);
    ...
    tests.test("Restart trusts DB apply fence when END op record is missing", [&] {
        ...
        std::filesystem::path dir =
            std::filesystem::temp_directory_path() /
            ("buzzdb-v112-fence-without-applied-test-" +
             std::to_string(::getpid()));
        std::filesystem::remove_all(dir);
        std::filesystem::create_directories(dir);
        std::string db_file = (dir / "buzzdb.dat").string();
    ...
    tests.test("Restart rejects local apply fence beyond protocol commit", [&] {
        ...
        std::filesystem::path dir =
            std::filesystem::temp_directory_path() /
            ("buzzdb-v112-uncommitted-fence-test-" +
             std::to_string(::getpid()));
        std::filesystem::remove_all(dir);
        std::filesystem::create_directories(dir);
        std::string db_file = (dir / "buzzdb.dat").string();
    ...
    tests.test("Full IMDB committed log replay recovers the JOB query", [&] {
        Address primary = ScenarioAddress::server1();
        Address backup = ScenarioAddress::backup1();
        Address client = ScenarioAddress::client1();
