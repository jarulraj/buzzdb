...
// Legacy primary-backup data-plane implementation retained for older version
// continuity in this monolithic file. v118 no longer uses this path in main or
// tests; replicated consensus below is the active implementation.
...
void printQuorumLeadershipTrace(const std::string& data_file) {
    std::cout << "Trace: replicated consensus log without a view server" << std::endl;
...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v118; "
    ...
    std::cout << "BuzzDB v118: multi-client consensus log for BuzzDB" << std::endl;
