...
const std::string imdb_smoke_query =
    "SELECT {*} FROM title WHERE id = 2008135";
const std::string imdb_table_scan_smoke_query =
...
void runImdbBaselineWorkload() {
    std::cout << "\nWorkload: IMDB with 2PL, Repeatable Read, and ARIES"
              << std::endl;

    BuzzDB db;
    db.useTwoPhaseLockingPolicy();
    printImportResult(ensureImdbDatasetLoaded(db));

    std::cout << "Concurrency-control policy: "
              << db.concurrencyControlPolicyName() << std::endl;
    std::cout << "Isolation level: "
              << db.isolationLevelName() << std::endl;

    std::cout << "\nIMDB Basic Query: exact lookup in title.\n";
    db.executeStatementsAndQueries({
        imdb_smoke_query,
    });

    std::cout << "\nRepeatable Read smoke transaction over IMDB.\n";
    auto reader = db.beginTransaction();
    db.execute(reader, imdb_smoke_query);
    db.execute(reader, imdb_smoke_query);
    db.commit(reader);

    std::cout << "\nTable-scan lock smoke transaction for future joins.\n";
    auto scan_reader = db.beginTransaction();
    db.execute(scan_reader, imdb_table_scan_smoke_query);
    db.commit(scan_reader);
...
int main() {
    ...
    std::cout << "Optimization chapter baseline\n";
    std::cout << "Load IMDB once, then reuse the persisted database.\n";

    runImdbBaselineWorkload();
