...
public:
    ...
    virtual ConcurrencyControlResult commit(int txn_id) = 0;
...
public:
    explicit MVTOPolicy(std::function<void(const std::string&)> log,
                        std::string timestamp_label = "MVTO")
        : log(std::move(log)),
          timestamp_label(std::move(timestamp_label)) {}
    ...
    std::string completionAction() const override {
        return "finish " + timestamp_label + " transaction";
    }
    ...
    void begin(int txn_id, const std::string& txn_label) override {
        ...
        log(txn_label + " " + timestamp_label + " timestamp=" +
    ...
    ConcurrencyControlResult commit(int txn_id) override {
        std::lock_guard<std::mutex> guard(latch);
        auto txn_it = txns.find(txn_id);
        if (txn_it == txns.end()) {
        ...
        if (state.has_writes) {
            log(state.label + " " + timestamp_label +
                " writer commits at timestamp=" +
                std::to_string(state.timestamp));
        } else {
            log(state.label + " " + timestamp_label + " releases timestamp=" +
                std::to_string(state.timestamp));
        }
        return {};
    }
...
public:
    ...
    void useMVTOPolicy(const std::string& timestamp_label = "MVTO") {
    ...
    bool commit(const TxnPtr& tx,
        ...
        auto result = concurrency_control_policy->commit(tx->id);
        if (!result.granted) {
            logConcurrencyControl("commit failed; " + result.reason);
            abort(tx);
            return false;
        }
    ...
    void executeStatement(const std::string& statement,
        ...
        (void)acquire_concurrency_control;
...
struct CustomerAssignmentPredicate {
    std::string customer;
    std::vector<std::string> seat_nos;
};
...
class MVTO {
    ...
    bool enforce_mvto_write_check = true;
    bool ssi_enabled = false;
    std::map<int, std::string> txn_labels;
    std::vector<SireadLock> siread_locks;
    std::set<std::pair<int, int>> rw_edges;

    int nextVersionId() {
        return run_id * 100 + next_version_id++;
    }

    static std::string versionName(int version_id) {
        return "v" + std::to_string(version_id % 100);
    }

    static int seatId(const std::string& seat_no) {
        if (seat_no == "1A") {
            return 1;
        }
        if (seat_no == "1B") {
    ...
    static std::string joinSeatNames(const std::vector<std::string>& seat_nos) {
        std::string result;
        for (size_t index = 0; index < seat_nos.size(); index++) {
            if (index > 0) {
                result += ",";
            }
            result += seat_nos[index];
        }
        return result;
    }

    static int valueId(const std::string& status,
                       const std::string& customer) {
        if (status == "available" && customer == "unassigned") {
            return 0;
        }
        if (status == "held" && customer == "garcia") {
            return 1;
        }
    ...
    static int txnId(const std::string& txn_label) {
    ...
    static std::string txnName(int txn_id) {
        return txn_id == 0 ?
            std::string("bootstrap") :
            "T" + std::to_string(txn_id);
    }

    std::string txnLabel(int txn_id) const {
        auto label_it = txn_labels.find(txn_id);
        if (label_it != txn_labels.end()) {
            return label_it->second;
        }
        return txnName(txn_id);
    }
    ...
    void requireSeatVersionsTableIfExistingDatabase() {
        if (db.isNewDatabase()) {
            return;
        }

        try {
            (void)db.catalog.getTable("seat_versions");
        } catch (const std::runtime_error&) {
            throw std::runtime_error(
                "buzzdb.dat was created before seat_versions; use a fresh "
                "database file for this MVCC version."
            );
        }
    }

    std::unique_ptr<IPredicate> eqPredicate(size_t column,
                                            int value) const {
        return std::make_unique<SimplePredicate>(
            SimplePredicate::Operand(column),
            SimplePredicate::Operand(std::make_unique<Field>(value)),
            SimplePredicate::ComparisonOperator::EQ
        );
    }

    std::unique_ptr<IPredicate> buildCustomerAssignmentPredicate(
        const CustomerAssignmentPredicate& predicate) const {
        auto seat_predicate =
            std::make_unique<ComplexPredicate>(ComplexPredicate::LogicOperator::OR);
        for (const auto& seat_no : predicate.seat_nos) {
            seat_predicate->addPredicate(eqPredicate(2, seatId(seat_no)));
        }

        auto all_predicates =
            std::make_unique<ComplexPredicate>(ComplexPredicate::LogicOperator::AND);
        all_predicates->addPredicate(eqPredicate(1, run_id));
        all_predicates->addPredicate(std::move(seat_predicate));
        all_predicates->addPredicate(eqPredicate(3, valueId("held", predicate.customer)));
        return all_predicates;
    }

    std::vector<std::unique_ptr<Field>> versionFields(
        const MVTOVersion& version) const {
        std::vector<std::unique_ptr<Field>> fields;
        fields.push_back(std::make_unique<Field>(version.version_id));
        fields.push_back(std::make_unique<Field>(run_id));
        fields.push_back(std::make_unique<Field>(version.seat_id));
        fields.push_back(std::make_unique<Field>(version.value_id));
        fields.push_back(std::make_unique<Field>(version.wts));
        fields.push_back(std::make_unique<Field>(version.rts));
        fields.push_back(std::make_unique<Field>(version.creator_id));
        fields.push_back(std::make_unique<Field>(version.committed ? 1 : 0));
        return fields;
    }

    bool hasRWCycleFrom(int txn_id) const {
        std::set<int> seen;
        std::vector<int> stack = {txn_id};
        while (!stack.empty()) {
            int current = stack.back();
            stack.pop_back();
            for (const auto& edge : rw_edges) {
                ...
                if (seen.insert(edge.second).second) {
    ...
    void recordSIREAD(const TxnPtr& txn,
                      const CustomerAssignmentPredicate& predicate) {
        if (!ssi_enabled) {
            return;
        }
        int reader_id = txnId(BuzzDB::txnLabel(txn));
        std::string description =
            "customer=" + predicate.customer +
            " assigned over seats " + joinSeatNames(predicate.seat_nos);
        siread_locks.push_back({
            reader_id,
            description,
            buildCustomerAssignmentPredicate(predicate)
        });
        std::cout << "  SSI SIREAD predicate mark: "
                  << BuzzDB::txnLabel(txn)
                  << " read " << description
                  << " using BuzzDB predicates"
                  << std::endl;
    }

    void recordSSIWrite(const TxnPtr& txn, const MVTOVersion& version) {
        if (!ssi_enabled) {
            return;
        }
        int writer_id = txnId(BuzzDB::txnLabel(txn));
        auto fields = versionFields(version);
        for (const auto& lock : siread_locks) {
            int reader_id = lock.reader_txn_id;
            if (reader_id == writer_id || !lock.predicate->check(fields)) {
                continue;
            }
            if (rw_edges.insert({reader_id, writer_id}).second) {
                std::cout << "  SSI rw-antidependency: "
                          << txnLabel(reader_id) << " -> "
                          << txnLabel(writer_id) << " because "
                          << txnLabel(writer_id) << "'s write to "
                          << seatName(version.seat_id)
                          << " matched " << txnLabel(reader_id)
                          << "'s predicate (" << lock.description << ")"
                          << std::endl;
            }
        }
        if (hasRWCycleFrom(writer_id)) {
            std::cout << "  SSI dangerous structure: rw-antidependency cycle"
                      << " now involves " << txnLabel(writer_id)
                      << std::endl;
        }
    }

public:
    MVTO(bool enforce_mvto_write_check = true,
         bool ssi_enabled = false)
        : enforce_mvto_write_check(enforce_mvto_write_check),
          ssi_enabled(ssi_enabled) {
        auto ticks = std::chrono::steady_clock::now()
            .time_since_epoch()
            .count();
        run_id = static_cast<int>(ticks % 1000000);

        requireSeatVersionsTableIfExistingDatabase();
        db.createTable("seat_versions", {
            {"version_id", INT},
            {"run_id", INT},
            {"seat_id", INT},
            {"value_id", INT},
        ...
        db.useMVTOPolicy(ssi_enabled ? "SSI" : "MVTO");
    ...
    TxnPtr beginReadOnly(const std::string& txn_label) {
        ...
        txn_labels[txnId(txn_label)] = txn_label;
    ...
    void endReadOnly(const TxnPtr& txn) {
        db.commit(txn);
    }
    ...
    std::optional<MVTOVersion> readSeat(const TxnPtr& txn,
        ...
        if (!ssi_enabled && timestamp > version->rts) {
    ...
    bool readNoCustomerAssignment(const TxnPtr& txn,
        ...
        std::cout << txn_label << " checks predicate: no "
                  << predicate.customer << " seat assignment over seats "
                  << joinSeatNames(predicate.seat_nos) << std::endl;
        auto buzzdb_predicate = buildCustomerAssignmentPredicate(predicate);
        bool has_assignment = false;
        for (const auto& seat_no : predicate.seat_nos) {
            auto version = readSeat(txn, seat_no);
            has_assignment = has_assignment ||
                             (version && buzzdb_predicate->check(versionFields(*version)));
        }
        recordSIREAD(txn, predicate);
    ...
    std::optional<PendingMVTOWrite> writeSeat(const TxnPtr& txn,
        ...
        if (enforce_mvto_write_check && previous && previous->rts > timestamp) {
        ...
        int new_value_id = valueId(status, customer);
        insertVersion(
            version_id,
            seat_id,
            new_value_id,
        ...
        std::cout << "  " << (ssi_enabled ? "SSI" : "MVTO")
                  << " accepts write; create active "
                  << versionName(version_id) << " [wts=rts="
                  << timestamp << "]" << std::endl;
        recordSSIWrite(
            txn,
            {version_id, seat_id, new_value_id, timestamp,
             timestamp, txnId(txn_label), false}
        );
        return PendingMVTOWrite{version_id, seat_id};
    }

    bool commitWrites(const TxnPtr& txn,
                      const std::vector<PendingMVTOWrite>& writes) {
        int txn_id = txnId(BuzzDB::txnLabel(txn));
        if (ssi_enabled && hasRWCycleFrom(txn_id)) {
            std::cout << BuzzDB::txnLabel(txn)
                      << " SSI commit check aborts transaction to break"
                      << " the rw-antidependency cycle" << std::endl;
            abortWrites(txn, writes);
            return false;
        }
        ...
        std::cout << txn_label << " certifies " << writes.size()
                  << " version(s)" << std::endl;
        std::cout << "  recovery link: committed "
                  << (ssi_enabled ? "SSI" : "MVTO") << " versions are the "
    ...
    void printVersionChain(const std::string& seat_no) {
        ...
        for (const auto& version : chain) {
            std::cout << "  " << versionName(version.version_id)
                      ...
                      << ", rts=" << version.rts << "] "
                      << valueLabel(version.value_id)
                      << " by " << txnLabel(version.creator_id)
    ...
    void collectGarbage(const std::string& label) {
        ...
        std::cout << "  GC deleted " << deleted_count
                  << " obsolete version row(s)" << std::endl;
    }
};

void runSSISchedule() {
    MVTO mvto(false, true);

    std::cout << "\nSerializable Snapshot Isolation" << std::endl;
    ...
    std::cout << "  SIREAD marks do not block writers; they remember"
              << " predicate reads so SSI can detect write skew." << std::endl;

    auto s1 = mvto.beginUpdater("S1");
    auto s2 = mvto.beginUpdater("S2");
    CustomerAssignmentPredicate garcia_assignment{"garcia", {"1A", "1B"}};
    mvto.readNoCustomerAssignment(s1, garcia_assignment);
    mvto.readNoCustomerAssignment(s2, garcia_assignment);

    std::cout << "S1 tries to assign 1A to Garcia" << std::endl;
    auto s1_write = mvto.writeSeat(s1, "1A", "held", "garcia");
    if (s1_write) {
        mvto.commitWrites(s1, {*s1_write});
    }

    std::cout << "S2 tries to assign 1B to Garcia" << std::endl;
    auto s2_write = mvto.writeSeat(s2, "1B", "held", "garcia");
    if (s2_write) {
        bool committed = mvto.commitWrites(s2, {*s2_write});
        if (!committed) {
            std::cout << "  SSI leaves only S1's assignment visible, so"
                      << " Garcia does not get two seats." << std::endl;
        }
    }

    std::cout << "Without SSI, plain SI would allow both commits because"
              << " S1 and S2 write different seat rows." << std::endl;
