...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v143; "
                  << "this version focuses on safe timestamp reads and locality."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v143: safe timestamp reads and locality"
              << std::endl;
    ...
    auto directClientCommand = [&](ConsensusGroupHarness& group,
    ...
    tests.test("Control plane runtime advances work only on the leader", [&] {
        auto movement = buildIndexMovementScenario();
        std::string range_id = defaultRangeIdForIndex(title_year_index);
        size_t source_before =
            physicalIndexEntryCount(movement.source, range_id);

        tests.check(commitCommand(
                        movement.catalog,
                        MoveRangeCommand{range_id,
                                         movement.target.group_id}) ==
                        Result{StatementOkResult{}},
                    "move should create durable pending transfer metadata");
        ControlPlaneRuntime non_leader_movement{
            [&](const Command& command) {
                return commitCommand(movement.catalog, command);
            },
            false};
        non_leader_movement.registerReplicaGroup(
            movement.source.group_id,
            [&](const Command& command) {
                return commitCommand(movement.source, command);
            });
        non_leader_movement.registerReplicaGroup(
            movement.target.group_id,
            [&](const Command& command) {
                return commitCommand(movement.target, command);
            });

        auto blocked_move = non_leader_movement.tick();
        tests.check(!blocked_move.advanced &&
                        blocked_move.status_after == "not_leader" &&
                        std::holds_alternative<ConfigRejectedResult>(
                            blocked_move.result) &&
                        latestTransferOn(movement.catalog, range_id)->status ==
                            "requested" &&
                        indexRouteGroupOnCatalog(
                            movement.catalog,
                            movement.fixture.duplicate_year) ==
                            movement.source.group_id &&
                        physicalIndexEntryCount(movement.source, range_id) ==
                            source_before &&
                        physicalIndexEntryCount(movement.target, range_id) == 0,
                    "non-leader runtime should not prepare, copy, or reroute a pending transfer");

        auto txn_scenario = buildIndexScenario();
        const auto& row = txn_scenario.fixture.initial_rows[0];
        std::string txn_id = "txn-nonleader-runtime";
        DistributedTxnResult prepared = runPrepare(
            txn_scenario.catalog,
            txn_id,
        ...
        tests.check(prepared.status == "prepared" &&
                        txnHasStatus(txn_scenario.catalog,
                                     txn_id,
                                     "prepared"),
                    "transaction should start as durable pending coordinator metadata");

        ControlPlaneRuntime non_leader_txn{
            [&](const Command& command) {
                return commitCommand(txn_scenario.catalog, command);
            },
            false};
        non_leader_txn.registerReplicaGroup(
            txn_scenario.table_owner.group_id,
            [&](const Command& command) {
                return commitCommand(txn_scenario.table_owner, command);
            });
        non_leader_txn.registerReplicaGroup(
            txn_scenario.index_owner.group_id,
            [&](const Command& command) {
                return commitCommand(txn_scenario.index_owner, command);
            });

        auto blocked_txn = non_leader_txn.tick();
        tests.check(!blocked_txn.advanced &&
                        blocked_txn.status_after == "not_leader" &&
                        std::holds_alternative<ConfigRejectedResult>(
                            blocked_txn.result) &&
                        txnHasStatus(txn_scenario.catalog,
                                     txn_id,
                                     "prepared") &&
                        catalogTableOn(txn_scenario.table_owner,
                                       txn_scenario.table_owner.leader,
                                       "__participant_txns").rows.empty() &&
                        catalogTableOn(txn_scenario.index_owner,
                                       txn_scenario.index_owner.leader,
                                       "__participant_txns").rows.empty(),
                    "non-leader runtime should not prepare participants or decide a pending transaction");
    });
    ...
    tests.test("Follower reads are served only at safe timestamps", [&] {
        auto scenario = buildIndexScenario();
        const auto& row = scenario.fixture.initial_rows[0];
        Address follower = scenario.table_owner.replicas[1];

        Result safe_result = directClientCommand(
            scenario.table_owner,
            follower,
            ReadReplicaSafeTimestampCommand{});
        const auto* safe =
            std::get_if<ReplicaSafeTimestampResult>(&safe_result);
        tests.check(safe != nullptr &&
                        !safe->safe_ts.isZero() &&
                        !safe->leader,
                    "follower should report an applied safe timestamp");

        Result stale_read = directClientCommand(
            scenario.table_owner,
            follower,
            SelectWhereCommand{"title", "id", row.values[0],
                               safe != nullptr
                                   ? safe->safe_ts
                                   : HybridLogicalTimestamp::zero()});
        const auto* stale_rows =
            std::get_if<SelectAllResult>(&stale_read);
        tests.check(stale_rows != nullptr &&
                        firstRowValue(*stale_rows, "production_year") ==
                            row.values[3],
                    "follower should serve a read at its safe timestamp");

        HybridLogicalTimestamp future_ts{
            safe != nullptr ? safe->safe_ts.physical + 1000000 : 1000000,
            safe != nullptr ? safe->safe_ts.logical : 0};
        Result rejected = directClientCommand(
            scenario.table_owner,
            follower,
            SelectWhereCommand{"title", "id", row.values[0], future_ts});
        const auto* rejection =
            std::get_if<FollowerReadRejectedResult>(&rejected);
        tests.check(rejection != nullptr &&
                        rejection->read_ts == future_ts,
                    "follower should fail closed above its safe timestamp");

        auto sendFollowerRequest = [&](const Command& command) {
            int request_id = scenario.table_owner.next_request_id++;
            scenario.table_owner.state.send(
                scenario.table_owner.client,
                follower,
                ClientRequest{scenario.table_owner.client_id,
                              request_id,
                              command});
            scenario.table_owner.state = stepRequired(
                scenario.table_owner.state,
                requireMessageEvent(
                    scenario.table_owner.state,
                    [&](const MessageEnvelope& envelope) {
                        const auto* request =
                        ...
                        return request != nullptr &&
                               request->client_id ==
                                   scenario.table_owner.client_id &&
                               request->request_id == request_id &&
                               envelope.from.rootAddress() ==
                                   scenario.table_owner.client.rootAddress() &&
                               envelope.to.rootAddress() ==
                                   follower.rootAddress();
                    },
                    "direct follower request",
                    SearchSettings{}),
                SearchSettings{});
            return request_id;
        };
        auto forwardedToLeader = [&](int request_id) {
            for (const auto& envelope : scenario.table_owner.state.network()) {
                const auto* forward =
                    std::get_if<ClientForward>(&envelope.message);
                if (forward != nullptr &&
                    envelope.from.rootAddress() == follower.rootAddress() &&
                    envelope.to.rootAddress() ==
                        scenario.table_owner.leader.rootAddress() &&
                    forward->request.client_id ==
                        scenario.table_owner.client_id &&
        ...
        int latest_read_request = sendFollowerRequest(
            SelectWhereCommand{"title", "id", row.values[0]});
        tests.check(!clientReplyFor(scenario.table_owner.state,
                                    scenario.table_owner.client_id,
                                    latest_read_request).has_value() &&
                        forwardedToLeader(latest_read_request),
                    "follower should forward untimestamped reads instead of serving latest locally");

        int write_request = sendFollowerRequest(
            UpdateRowsCommand{"title",
                              "production_year",
                              replacementProductionYear(row.values[3]),
                              "id",
                              row.values[0]});
        tests.check(!clientReplyFor(scenario.table_owner.state,
                                    scenario.table_owner.client_id,
                                    write_request).has_value() &&
                        forwardedToLeader(write_request),
                    "follower should forward writes instead of applying them locally");
    });

    tests.test("Follower stale read returns the old MVCC version", [&] {
        auto scenario = buildIndexScenario();
        const auto& row = scenario.fixture.initial_rows[0];
        std::string old_year = row.values[3];
        std::string new_year = replacementProductionYear(old_year);
        ...
        const auto* safe =
            std::get_if<ReplicaSafeTimestampResult>(&safe_result);
        HybridLogicalTimestamp read_ts =
            safe != nullptr ? safe->safe_ts : HybridLogicalTimestamp::zero();
        tests.check(!read_ts.isZero(),
                    "test requires a follower safe timestamp before the write");
        ...
        tests.check(commitCommand(
                    ...
                    "leader should commit a later real update");

        Result stale_read = directClientCommand(
            scenario.table_owner,
            follower,
            SelectWhereCommand{"title", "id", row.values[0], read_ts});
        const auto* stale_rows =
            std::get_if<SelectAllResult>(&stale_read);
        tests.check(stale_rows != nullptr &&
                        firstRowValue(*stale_rows, "production_year") ==
                    ...
                    "follower stale read should see the pre-write version while latest sees the write");
    });

    tests.test("Locality demo measures colocated and cross-range transactions", [&] {
        struct RuntimeCounters {
            size_t catalog = 0;
            size_t table = 0;
            size_t index = 0;
            size_t table_participant = 0;
            size_t index_participant = 0;
        };

        auto isParticipantProtocolCommand = [](const Command& command) {
            return std::holds_alternative<PrepareTxnParticipantCommand>(
                       command) ||
                   std::holds_alternative<CommitTxnParticipantCommand>(
                       command) ||
                   std::holds_alternative<AbortTxnParticipantCommand>(
                       command) ||
                   std::holds_alternative<ReadTxnParticipantStatusCommand>(
                       command);
        };

        auto countedRuntime =
            [&](IndexScenario& scenario, RuntimeCounters& counters) {
            ControlPlaneRuntime runtime{
                [&](const Command& command) {
                    ++counters.catalog;
                    return commitCommand(scenario.catalog, command);
            ...
            runtime.registerReplicaGroup(
                ...
                [&](const Command& command) {
                    ++counters.table;
                    if (isParticipantProtocolCommand(command)) {
                        ++counters.table_participant;
                    }
                    return commitCommand(scenario.table_owner, command);
                });
            runtime.registerReplicaGroup(
        ...
        auto colocated = buildIndexScenario();
        const auto& colocated_row = colocated.fixture.initial_rows[0];
        std::vector<std::string> colocated_statements{
            "UPDATE title SET title=BuzzDB_Locality WHERE id=" +
            colocated_row.values[0]};
        DistributedTxnResult colocated_prepare =
            runPrepare(colocated.catalog,
                       "txn-locality-colocated",
                       colocated_statements);
        RuntimeCounters colocated_counts;
        auto colocated_runtime =
            countedRuntime(colocated, colocated_counts);
        size_t colocated_steps = colocated_runtime.runUntilIdle();

        auto cross = buildIndexScenario();
        const auto& cross_row = cross.fixture.initial_rows[0];
        std::vector<std::string> cross_statements{
            "UPDATE title SET production_year=" +
            replacementProductionYear(cross_row.values[3]) +
            " WHERE id=" + cross_row.values[0]};
        DistributedTxnResult cross_prepare =
            runPrepare(cross.catalog,
                       "txn-locality-cross-range",
                       cross_statements);
        RuntimeCounters cross_counts;
        auto cross_runtime = countedRuntime(cross, cross_counts);
        size_t cross_steps = cross_runtime.runUntilIdle();

        tests.check(colocated_prepare.participant_range_ids.size() == 1 &&
                        cross_prepare.participant_range_ids.size() == 2,
                    "locality demo should compare one participant with two participants");
        tests.check(colocated_steps > 0 && cross_steps > 0 &&
                        colocated_counts.index_participant == 0 &&
                        cross_counts.index_participant > 0 &&
                        (cross_counts.table_participant +
                         cross_counts.index_participant) >
                            (colocated_counts.table_participant +
                             colocated_counts.index_participant),
                    "cross-range transaction should use more real participant commands than the colocated transaction");
    });
