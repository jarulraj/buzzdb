...
private:
    using Row = std::vector<std::unique_ptr<Field>>;
    ...
    static bool rowLess(const Row& left,
                        const Row& right,
                        size_t left_attr_index,
                        size_t right_attr_index) {
        return compareFields(
            *left[left_attr_index],
            *right[right_attr_index]
        ) < 0;
    }

    static bool rowKeyEquals(const Row& left,
                             const Row& right,
                             size_t left_attr_index,
                             size_t right_attr_index) {
        return compareFields(
...
public:
    ...
    void open() override {
        ...
        std::sort(left_rows.begin(), left_rows.end(),
                  [&](const Row& left, const Row& right) {
                      return rowLess(left, right, left_attr_index, left_attr_index);
                  });
        std::sort(right_rows.begin(), right_rows.end(),
                  [&](const Row& left, const Row& right) {
                      return rowLess(left, right, right_attr_index, right_attr_index);
                  });
        ...
        while (left_index < left_rows.size() && right_index < right_rows.size()) {
            int comparison = compareFields(
                *left_rows[left_index][left_attr_index],
                *right_rows[right_index][right_attr_index]
            );
            if (comparison < 0) {
                left_index++;
                continue;
            ...
            size_t left_group_end = left_index + 1;
            while (left_group_end < left_rows.size() &&
                   rowKeyEquals(left_rows[left_index], left_rows[left_group_end],
                                left_attr_index, left_attr_index)) {
                left_group_end++;
            }

            size_t right_group_end = right_index + 1;
            while (right_group_end < right_rows.size() &&
                   rowKeyEquals(right_rows[right_index], right_rows[right_group_end],
                                right_attr_index, right_attr_index)) {
                right_group_end++;
            }

            for (size_t left_group_index = left_index;
                 left_group_index < left_group_end;
                 left_group_index++) {
                for (size_t right_group_index = right_index;
                     right_group_index < right_group_end;
                     right_group_index++) {
                    Row output = cloneFields(left_rows[left_group_index]);
                    for (const auto& field : right_rows[right_group_index]) {
    ...
    bool next() override {
        current_output.clear();
        has_next = false;
        if (output_index >= joined_rows.size()) {
            return false;
        }

        current_output = cloneFields(joined_rows[output_index]);
        output_index++;
        has_next = true;
        return true;
    }
...
std::string joinKindName(PhysicalJoinKind kind) {
    switch (kind) {
...
QueryResult executeJoinQuery(const QueryComponents& components,
    ...
    for (const auto& join : components.joins) {
        ...
        if (join_kind == PhysicalJoinKind::SortMergeJoin) {
            sort_merge_join_buffers.push_back(std::make_unique<SortMergeJoinOperator>(
                *rootOp,
                right_scan,
                left_attr_index,
                right_attr_index
            ));
            rootOp = sort_merge_join_buffers.back().get();
        } else {
...
void printJoinQueryResult(const QueryComponents& components,
                          ...
                          PhysicalJoinKind join_kind) {
    std::cout << "Join operator: " << joinKindName(join_kind) << std::endl;
    ...
    void execute(const ParsedStatement& statement,
        ...
        switch (statement.kind) {
            ...
            case ParsedStatement::Kind::Select:
                executeSelect(statement, txn, join_kind);
                return;
        }
    ...
    void execute(const ParsedStatement& statement,
        ...
        switch (statement.kind) {
            ...
            case ParsedStatement::Kind::Delete:
                ...
                return;
            case ParsedStatement::Kind::Select:
                executeSelectBody(statement, join_kind);
                return;
        }
        throw std::runtime_error("Unsupported parsed statement.");
    }
...
public:
    ...
    void execute(const ParsedStatement& statement,
                 const TxnPtr& txn,
                 PhysicalJoinKind join_kind) {
        query_executor.execute(statement, txn, join_kind);
    }

    void execute(const std::string& statement,
                 const TxnPtr& txn,
                 PhysicalJoinKind join_kind) {
        execute(query_parser.parseStatement(statement), txn, join_kind);
    }
    ...
    void execute(const std::string& statement,
                 PhysicalJoinKind join_kind) {
        execute(query_parser.parseStatement(statement), join_kind);
    }
...
public:
    ...
    void useJoinKind(PhysicalJoinKind kind) {
        join_kind = kind;
    }

    void execute(const TxnPtr& txn, const std::string& statement) {
        requireRunningTransaction(txn);
        query_processor.execute(statement, txn, join_kind);
    }
    ...
    void executeStatementsAndQueries(const std::vector<std::string>& statements,
        ...
        for (const auto& statement : statements) {
            ...
            if (active_txn) {
                ...
                query_processor.execute(statement, join_kind);
            }
            statementFinished();
        }
    }
...
const std::string imdb_physical_join_query =
...
void runImdbPhysicalJoin(PhysicalJoinKind join_kind) {
    std::cout << "\nIMDB Query: title joined with rating metadata using "
              << joinKindName(join_kind) << std::endl;

    BuzzDB db;
    db.useJoinKind(join_kind);
...
int main() {
    ...
    std::cout << "IMDB physical join operators\n";

    runImdbPhysicalJoin(PhysicalJoinKind::HashJoin);
    runImdbPhysicalJoin(PhysicalJoinKind::SortMergeJoin);

    std::cout << "\nResult: Same SQL and same rows; only the physical join "
              << "operator changes.\n";
