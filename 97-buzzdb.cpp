...
private:
    Catalog& catalog;
    PageManager& page_manager;
    std::map<IndexDescriptor, BPlusTreeIndex> range_indexes;

public:
    RangeIndexManager(Catalog& catalog, PageManager& page_manager)
        : catalog(catalog), page_manager(page_manager) {}

    RangeIndexBuildResult buildRangeIndex(const RangeIndexSpec& spec) {
        ...
        auto& index = range_indexes[descriptor];
        index.clear();
    ...
    bool hasRangeIndex(const IndexDescriptor& descriptor) const {
        return range_indexes.find(descriptor) != range_indexes.end();
    }

    const BPlusTreeIndex& rangeIndex(const IndexDescriptor& descriptor) const {
        auto it = range_indexes.find(descriptor);
        if (it == range_indexes.end()) {
            throw std::runtime_error("Range index is not built.");
        }
        return it->second;
    }
    ...
    IndexRangeLookupResult rangeLookup(const IndexDescriptor& descriptor,
                                       int lower_bound,
                                       int upper_bound) const {
        return rangeIndex(descriptor).rangeLookup(lower_bound, upper_bound);
    }
};
...
QueryResult executeJoinQuery(const QueryComponents& components,
    ...
    std::function<BuiltPlanOperator(const std::shared_ptr<JoinPlanNode>&)>
        buildPlanNode =
            [&](const std::shared_ptr<JoinPlanNode>& node) -> BuiltPlanOperator {
        if (!node) {
            throw std::runtime_error("Missing join plan node.");
        }
        if (node->is_leaf) {
...
private:
    ...
    std::shared_ptr<JoinPlanNode> makeLeafJoinPlanNode(
            TableRefId table_ref_id,
            const RelationStats& relation) {
    ...
    std::shared_ptr<JoinPlanNode> makeJoinPlanNode(
    ...
    static bool planContainsTableRef(const JoinPlanNode& node,
                                     TableRefId table_ref_id) {
    ...
    std::string joinPlanTreeString(const QueryComponents& components,
        ...
        return physicalJoinKindName(node->chosen_join_kind) + "(" +
               joinPlanTreeString(components, node->left) + ", " +
               joinPlanTreeString(components, node->right) + ")";
    }

    void appendJoinPlanTree(const QueryComponents& components,
                            const std::shared_ptr<JoinPlanNode>& node,
                            size_t depth,
                            std::ostringstream& output) {
        if (!node) {
        ...
        output << physicalJoinKindName(node->chosen_join_kind)
        ...
        appendJoinPlanTree(components, node->right, depth + 1, output);
    }

    std::string prettyJoinPlanTree(
            const QueryComponents& components,
            const std::shared_ptr<JoinPlanNode>& node) {
        std::ostringstream output;
        appendJoinPlanTree(components, node, 0, output);
        return output.str();
    }
...
public:
    ...
    AccessPathChoice chooseRangeAccessPath(const TableStats& table_stats,
                                           ...
                                           int lower_bound,
                                           int upper_bound,
                                           const RangeIndexManager& range_index_manager) const {
        ...
        choice.index_available =
            range_index_manager.hasRangeIndex(choice.index_descriptor);
    ...
    void printJoinPlanTree(const std::string& title,
                           const QueryComponents& components,
                           const std::shared_ptr<JoinPlanNode>& plan_root) {
        std::cout << "\n" << title << ":" << std::endl;
        std::cout << prettyJoinPlanTree(components, plan_root);
    }
...
public:
    ...
    PageManager page_manager;
    Catalog catalog;
    RangeIndexManager range_index_manager;
    ...
    TransactionalStorageManager()
        : log_manager(),
          ...
          range_index_manager(catalog, page_manager),
          txn_manager() {
...
public:
    ...
    AccessPathExecutionResult executeRangeCount(
        ...
        if (access_path == AccessPathKind::TableScan) {
            ...
            auto lookup = transactional_storage_manager.range_index_manager
...
public:
    ...
    RangeIndexManager& rangeIndexes() {
        return transactional_storage_manager.range_index_manager;
    }
...
void runImdbAccessPathSelection() {
    ...
    auto index_build = db.rangeIndexes().buildRangeIndex({
