...
//
// FoundationDB's write path separates ordering, durable logging, and storage
// application: commit proxies assign versions, TLogs replicate the mutation
// stream, and storage servers later consume that stream. This teaching snapshot
// introduces the first two pieces at BuzzDB scale: a sequenced commit node sends
// each command to two log replicas and applies it only after both logs ack.
...
public:
        ...
        for (const auto& entry : tables_) {
            ...
            for (const auto& column : table.columns) {
                ...
                if (!seen_columns.insert(column).second) {
                    return "table " + entry.first + " has duplicate column " +
                           column;
                }
            }
    ...
    Result createTable(const CreateTableCommand& command) {
        if (!validSchema(command)) {
            return InvalidSchemaResult{};
        }
        if (tables_.find(command.table) != tables_.end()) {
            return TableAlreadyExistsResult{};
        }
        tables_.emplace(command.table, Table{command.columns, {}});
        return CreateTableOkResult{};
    }

    Result insertRow(const InsertRowCommand& command) {
        auto table = tables_.find(command.table);
        if (table == tables_.end()) {
            return TableNotFoundResult{};
        }
        if (command.values.size() != table->second.columns.size()) {
            return SchemaMismatchResult{};
        }
        table->second.rows.push_back(command.values);
        return InsertOkResult{};
    }

    Result selectAll(const SelectAllCommand& command) const {
        auto table = tables_.find(command.table);
        if (table == tables_.end()) {
            return TableNotFoundResult{};
        }
        return SelectAllResult{table->second.rows};
    }

    Result countRows(const CountRowsCommand& command) const {
        auto table = tables_.find(command.table);
        if (table == tables_.end()) {
            return TableNotFoundResult{};
        }
        return CountRowsResult{table->second.rows.size()};
    }

private:
    bool validSchema(const CreateTableCommand& command) const {
        if (command.table.empty() || command.columns.empty()) {
            return false;
        }
        std::set<std::string> seen_columns;
...
public:
    ...
    void onMessage(NodeContext& ctx,
        ...
        if (entries_.find(request->entry.version) == entries_.end()) {
...
public:
    SequencedReplicatedBuzzDBServer(Address address, std::vector<Address> logs)
        : Node(std::move(address)), logs_(std::move(logs)), quorum_(logs_.size()) {
        if (logs_.empty()) {
...
Workload createInsertSelectWorkload() {
    std::vector<Command> commands{
        CreateTableCommand{"people", {"name", "role"}},
        InsertRowCommand{"people", {"Ada", "systems"}},
        InsertRowCommand{"people", {"Grace", "compilers"}},
        CountRowsCommand{"people"},
        SelectAllCommand{"people"}
    };
...
Workload createTableWorkload() {
    std::vector<Command> commands{
        CreateTableCommand{"people", {"id", "name"}}
    };
    return Workload(commands, buzzDBOracleResults(commands));
}

Workload createInsertScanWorkload() {
    std::vector<Command> commands{
        CreateTableCommand{"people", {"name", "role"}},
        InsertRowCommand{"people", {"Ada", "systems"}},
        SelectAllCommand{"people"}
...
Workload schemaMismatchWorkload() {
    std::vector<Command> commands{
        CreateTableCommand{"people", {"name"}},
        InsertRowCommand{"people", {"Ada", "systems"}},
        CountRowsCommand{"people"},
        SelectAllCommand{"people"}
    };
...
std::vector<Command> deterministicBuzzDBApiCommands(uint32_t seed, int steps) {
    std::vector<Command> commands;
    if (steps <= 0) {
        return commands;
    }

    auto add = [&](Command command) {
        if (static_cast<int>(commands.size()) < steps) {
            commands.push_back(std::move(command));
        }
    };

    add(CreateTableCommand{"people", {"id", "name"}});
    add(CreateTableCommand{"people", {"id", "name"}});
    add(CreateTableCommand{"bad_schema", {"id", "id"}});
    add(InsertRowCommand{"orders", {"1", "open"}});
    add(InsertRowCommand{"people", {"1"}});

    std::mt19937 rng(seed);
    std::uniform_int_distribution<int> table_pick(0, 3);
    std::uniform_int_distribution<int> op_pick(0, 7);
    std::vector<std::string> tables{"people", "orders", "events", "audit"};

    for (int i = static_cast<int>(commands.size()); i < steps; ++i) {
        std::string table = tables[table_pick(rng)];
        int op = op_pick(rng);
        std::string id = std::to_string(i);
        std::string value = "v" + std::to_string(seed % 1000) + ":" + id;

        switch (op) {
            case 0:
                commands.push_back(
                    CreateTableCommand{table, {"id", "value"}});
                break;
            case 1:
                commands.push_back(InsertRowCommand{table, {id, value}});
                break;
            case 2:
                commands.push_back(InsertRowCommand{table, {id}});
                break;
            case 3:
                commands.push_back(CountRowsCommand{table});
                break;
            case 4:
                commands.push_back(SelectAllCommand{table});
                break;
            case 5:
                commands.push_back(
                    CreateTableCommand{"generated_" + id, {"id", "payload"}});
                break;
            case 6:
                commands.push_back(
                    CreateTableCommand{"", {"id", "payload"}});
                break;
            default:
                commands.push_back(
                    SelectAllCommand{"missing_" + std::to_string(i % 3)});
                break;
        }
    }
...
Workload deterministicBuzzDBApiWorkload(uint32_t seed, int steps) {
    std::vector<Command> commands = deterministicBuzzDBApiCommands(seed, steps);
    return Workload(commands, buzzDBOracleResults(commands));
}
...
int main() {
    ...
    tests.test("BuzzDBCore creates tables, inserts rows, and scans", [&] {
        ...
        tests.check(!db.validate().has_value(),
                    "valid command sequence should preserve core invariants");
        tests.check(db.digest().find("people") != std::string::npos,
                    "database digest should include table state");
    });

    tests.test("BuzzDB oracle derives expected search workload results", [&] {
        Workload workload = createInsertSelectWorkload();
        tests.check(workload.expectedResult(0) == Result{CreateTableOkResult{}},
                    "oracle should expect table creation");
        tests.check(workload.expectedResult(1) == Result{InsertOkResult{}},
                    "oracle should expect first insert");
        tests.check(workload.expectedResult(2) == Result{InsertOkResult{}},
                    "oracle should expect second insert");
        tests.check(workload.expectedResult(3) == Result{CountRowsResult{2}},
                    "oracle should expect two rows");
        tests.check(workload.expectedResult(4) ==
                        Result{SelectAllResult{
                            {{"Ada", "systems"}, {"Grace", "compilers"}}}},
                    "oracle should expect the full table scan");
    });

    tests.test("BuzzDB rejects schema mismatches without partial writes", [&] {
        Workload workload = schemaMismatchWorkload();
        tests.check(workload.expectedResult(0) == Result{CreateTableOkResult{}},
                    "oracle should expect table creation");
        tests.check(workload.expectedResult(1) == Result{SchemaMismatchResult{}},
                    "schema mismatch should be reported");
        tests.check(workload.expectedResult(2) == Result{CountRowsResult{0}},
                    "failed insert should not add a row");
        tests.check(workload.expectedResult(3) ==
                        Result{SelectAllResult{{}}},
                    "failed insert should leave the table empty");
    });

    tests.test("BuzzDB deterministic API workload is repeatable", [&] {
        std::vector<Command> first =
            deterministicBuzzDBApiCommands(kDefaultSearchSeed, 24);
        std::vector<Command> second =
            deterministicBuzzDBApiCommands(kDefaultSearchSeed, 24);
        tests.check(first.size() == 24,
                    "deterministic API workload should have requested length");
        tests.check(second.size() == first.size(),
                    "same seed should produce the same command count");
        for (size_t i = 0; i < first.size(); ++i) {
            tests.check(describeCommand(first[i]) == describeCommand(second[i]),
                        "same seed should produce identical commands");
        }

        std::vector<Result> expected = buzzDBOracleResults(first);
        Workload workload =
            deterministicBuzzDBApiWorkload(kDefaultSearchSeed, 24);
        tests.check(workload.commandCount() == first.size(),
                    "workload should expose all generated commands");
        tests.check(workload.expectedCount() == expected.size(),
                    "workload should expose all model-backed results");
        for (size_t i = 0; i < expected.size(); ++i) {
            tests.check(workload.expectedResult(i) == expected[i],
                        "workload expected result should match the oracle");
        }
    });

    tests.test("BuzzDB deterministic API workload covers failure cases", [&] {
        std::vector<Result> expected = buzzDBOracleResults(
            deterministicBuzzDBApiCommands(kDefaultSearchSeed, 24));
        auto has_result = [&](const Result& wanted) {
            return std::any_of(
                expected.begin(),
                expected.end(),
                [&](const Result& result) {
        ...
        tests.check(has_result(Result{TableAlreadyExistsResult{}}),
                    "workload should cover duplicate table creation");
        tests.check(has_result(Result{InvalidSchemaResult{}}),
                    "workload should cover invalid schemas");
        tests.check(has_result(Result{TableNotFoundResult{}}),
                    "workload should cover missing tables");
        tests.check(has_result(Result{SchemaMismatchResult{}}),
                    "workload should cover row/schema mismatches");
    });

    tests.test("BuzzDB validation invariant holds during API workload", [&] {
        std::vector<Command> commands =
            deterministicBuzzDBApiCommands(kDefaultSearchSeed, 24);
        std::vector<Result> expected = buzzDBOracleResults(commands);
        BuzzDBCore db;
        for (size_t i = 0; i < commands.size(); ++i) {
            Result actual = db.execute(commands[i]);
            tests.check(actual == expected[i],
                        "model and execution should agree at step " +
                            std::to_string(i));
            std::optional<std::string> invariant = db.validate();
            tests.check(!invariant.has_value(),
                        "BuzzDB invariant failed after " +
                            describeCommand(commands[i]) + ": " +
                            invariant.value_or(""));
        }
    });
    ...
    tests.test("Replicated BuzzDB waits for both log acks before replying", [&] {
        Address log1 = DemoAddress::log1();
        Address log2 = DemoAddress::log2();
        Scenario scenario = oneClientReplicatedBuzzDBScenario(createTableWorkload());
        ...
        tests.check(containsEnvelopeMatching(
                        *after_request,
                        [server](const MessageEnvelope& envelope) {
                            return envelope.from == server &&
                                   std::holds_alternative<LogAppendRequest>(
                                       envelope.message);
                        },
                        "log append request").ok,
                    "server should send log append requests");
        ...
        const auto* log1_node = after_log1->nodeAs<BuzzDBLogReplica>(log1);
        tests.check(log1_node != nullptr && log1_node->hasVersion(1),
                    "first log should store version 1");
        ...
        const auto* log2_node = after_log2->nodeAs<BuzzDBLogReplica>(log2);
        tests.check(log2_node != nullptr && log2_node->hasVersion(1),
                    "second log should store version 1");
    ...
    tests.test("Single-client BuzzDB create/insert/select search", [&] {
        Scenario scenario = oneClientBuzzDBScenario(createInsertScanWorkload());
        SearchSettings settings = scenario.settings;
        settings.max_depth = 12;
        settings.max_states = 10000;
        settings.invariants.push_back(
        ...
        SearchResults result = bfs(scenario.state, settings);
        tests.check(result.end_condition == SearchResults::EndCondition::GoalFound,
                    "BuzzDB client should finish");
        tests.check(result.terminal_state.has_value(),
                    "goal search should return a terminal state");
        const auto* worker =
            result.terminal_state->nodeAs<ClientWorker>(client);
        tests.check(worker != nullptr, "client should exist in terminal state");
        tests.check(worker->resultCount() == 3,
                    "BuzzDB workload should produce three results");
        tests.check(worker->results().back() ==
                        Result{SelectAllResult{{{"Ada", "systems"}}}},
                    "terminal result should be the full table scan");
    });

    tests.test("Single-client replicated BuzzDB search reaches CLIENTS_DONE", [&] {
        Address log1 = DemoAddress::log1();
        Address log2 = DemoAddress::log2();
        Scenario scenario =
        ...
        const auto* replicated =
            result.terminal_state->nodeAs<SequencedReplicatedBuzzDBServer>(
                server);
        tests.check(replicated != nullptr && replicated->committedCount() == 1,
                    "replicated server should have one committed command");
    });
