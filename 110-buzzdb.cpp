...
// BuzzDB v110: FDB-style model-backed BuzzDB API correctness workloads.
//
// This version keeps the v109 BuzzDB state machine in the simulator and adds
// stronger single-node correctness checks: schema validation, deterministic API
// workload generation, and model-backed expected results. Replication can now
// target a more realistic database oracle instead of only hand-written traces.
...
int main() {
    std::cout << "BuzzDB v110: FDB-style model-backed API workloads"
              << std::endl;
    TestRunner tests;
