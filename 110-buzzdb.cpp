...
// BuzzDB v110: durable BuzzDB storage and table-operator checks.
//
...
// page-file storage stack. Its tests now stay close to the durable database
// surface: table operators, SQL parsing, catalog metadata, heap pages, and
// reopening node-local buzzdb.dat files.
...
void printDurableBuzzDBDemo() {
    const std::string database_file = "/tmp/buzzdb-v110-buzzdb.dat";
...
int main() {
    std::cout << "BuzzDB v110: durable storage and table operators"
              << std::endl;
    printBuzzDBDemo();
