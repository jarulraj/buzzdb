...
public:
    ...
    void processPageRange(size_t start_page, size_t end_page) {

        for (size_t page_itr = start_page; page_itr < end_page; page_itr++) {
            auto& page = buffer_manager.getPage(page_itr);
    ...
    void parallelProcessPages(size_t num_threads) {
        auto num_pages = buffer_manager.getNumPages();
        ...
        for (size_t i = 0; i < num_threads; i++) {
            ...
            size_t end_page = (i + 1 == num_threads) ? num_pages : start_page + pages_per_thread; // Last thread gets any remaining pages

            threads.emplace_back(
                &BuzzDB::processPageRange, 
                this, 
                start_page, end_page);
        }
    ...
    void scanTableToBuildIndex(){
        ...
        parallelProcessPages(num_threads);
        ...
        std::cout << "Elapsed index construction time: " << elapsed.count() << " seconds" << std::endl;
