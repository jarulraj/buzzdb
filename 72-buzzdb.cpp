...
void RecoveryManager::recover() {
    ...
    auto collectWalRecord = [&](const std::string& record) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v72."
            );
        }
        if (txn_id > 0) {
...
public:
    ...
    void setPredicateLocksEnabled(bool enabled) {
        use_predicate_locks = enabled;
    }
    ...
    bool acquireQueryLocks(const TxnPtr& txn,
        ...
        if (components.equalityWhereCondition) {
            ...
            if (use_predicate_locks &&
                ...
                // Matching inserts request X on the same predicate string.
                return acquireTxnLock(
                    txn,
                    LockMode::S,
                    predicateResource(
    ...
    void executeStatement(const std::string& statement,
        ...
        if (components.type == StatementType::INSERT) {
            ...
            if (recovery_manager.isActive()) {
                throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v72.");
            }
...
void runTransactionLocking() {
    ...
    std::cout << "\nPhantom reads with tuple and predicate locks" << std::endl;
    std::cout << "  Tuple locks protect rows that already exist." << std::endl;
    std::cout << "  Predicate locks protect the condition that future rows may match." << std::endl;
    ...
    auto runPhantomCase = [&](const std::string& label_prefix,
                              bool predicate_locks,
                              const std::string& stats_label) {
        resetBookingRows(db);
        db.setPredicateLocksEnabled(predicate_locks);
        db.resetLockStats();
    ...
    std::cout << "\nCase 1: tuple locks allow a phantom" << std::endl;
    std::cout << "  Purpose: show that locking existing rows does not lock the gap." << std::endl;
    std::cout << "  Watch for: T2 inserts 3A while T1 is still active, then T1 sees it." << std::endl;
    runPhantomCase("T", false, "tuple locks only");

    std::cout << "\nCase 2: predicate lock prevents the phantom" << std::endl;
    std::cout << "  Purpose: protect the condition seats.status=available." << std::endl;
    std::cout << "  Watch for: P2 waits on X(predicate seats.status=available)." << std::endl;
    runPhantomCase("P", true, "predicate locking");
}
