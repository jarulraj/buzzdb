...
private:
    ...
    bool recordCascadesCandidate(
            GroupId group_id,
            const MemoPlanChoice& candidate,
...
public:
    ...
    MemoOptimizationResult optimizeWithCascadesQueue(
        ...
        auto final_choice = winners.find(memo.finalGroupId());
...
void runImdbCascadesQueue() {
    ...
    std::cout << "\nTakeaway: Cascades turns memo optimization into explicit"
              << " work. Groups are optimized by tasks, expressions wait for"
              << " input winners, and the result is still a typed physical"
              << " plan tree.\n";
}
...
int main() {
    ...
    std::cout << "IMDB Cascades task queue\n";

    runImdbCascadesQueue();
