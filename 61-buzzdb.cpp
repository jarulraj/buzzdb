...
constexpr uint16_t BUZZDB_VERSION = 61;
...
void RecoveryManager::recover() {
    ...
    for (const auto& record : log_records) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v61."
            );
        }
    ...
    for (const auto& record : wal_records) {
        ...
        auto dirty_page = dirty_page_table.find(record.page_id);
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v61.");
        }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& table_name : existing_tables) {
        if (!isBookingTableName(table_name)) {
            throw std::runtime_error(
                "This v61 booking workload cannot open a buzzdb.dat containing "
