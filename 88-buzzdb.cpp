...
const std::vector<std::string> poor_join_order_aliases = {
...
void runImdbJoinOrdering() {
    ...
    auto poor_components = makeJoinPlanFromAliases(
        components,
        poor_join_order_aliases
    );
    auto greedy_plan = chooseGreedyJoinOrder(components, stats);
    std::cout << "\nPoor legal join order:" << std::endl;
    std::cout << "  " << joinOrderString(poor_components) << std::endl;
    printJoinOrderPlan("Optimizer chosen join order", greedy_plan, db.catalog);

    auto poor_join_kinds = choosePhysicalJoinKindsForOrder(
        poor_components,
        stats
    );
    ...
    auto poor_result = executeJoinQueryInTransaction(
        db,
        poor_components,
        &poor_join_kinds
    );
    ...
    printTimedQueryResult("Poor legal order", poor_result);
    printTimedQueryResult(
        "Optimizer chosen order",
        greedy_physical_result
    );
    if (poor_result.result.row_count != greedy_physical_result.result.row_count) {
