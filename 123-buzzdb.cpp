...
int main(int argc, char* argv[]) {
    if (argc > 1 && std::string(argv[1]) == "--measure-restart") {
        std::cerr << "--measure-restart is not available in v123; "
                  << "this version focuses on replica-group membership metadata."
                  << std::endl;
        return 2;
    }
    ...
    std::cout << "BuzzDB v123: replica-group membership metadata" << std::endl;
    ...
    TestRunner tests;
    tests.test("Replica group config is replicated through Raft", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 321, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        Command register_group = RegisterReplicaGroupCommand{
            "group-1", {"server1", "server2", "server3"}, 1};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 321, 2, register_group,
            {replicas[1], replicas[2]});
        ...
        for (const auto& replica : replicas) {
            ...
            tests.check(catalogHasRowOn(
                            ...
                            {{"group_id", "group-1"},
                             {"config_version", "1"},
                             {"voters", "server1,server2,server3"},
                             {"phase", "stable"}}),
                        replica.str() +
                            " should have the initial group config");
        }
    });

    tests.test("Learner can be caught up and promoted by joint config", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 2,
            RegisterReplicaGroupCommand{
                "group-1", {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 3,
            DiscoverClusterNodeCommand{
                "cluster-A", "server4", "learner", "discovered", 2},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 4,
            AddLearnerToGroupCommand{"group-1", "server4", 2},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 5,
            MarkLearnerCaughtUpCommand{"group-1", "server4", 3},
            {replicas[1], replicas[2]});
        Command begin_joint = BeginJointConfigCommand{
            "group-1",
            {"server1", "server2", "server3", "server4"},
            {"server1", "server2"},
            {"server1", "server2", "server4"},
            4};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 6, begin_joint,
            {replicas[1], replicas[2]});
        Command finalize = FinalizeJointConfigCommand{
            "group-1",
            {"server1", "server2", "server3", "server4"},
            {"server1", "server3"},
            {"server1", "server2", "server4"},
            5};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 322, 7, finalize,
            {replicas[1], replicas[2]});
        ...
        for (const auto& replica : replicas) {
            ...
            tests.check(catalogHasRowOn(
                            ...
                            {{"group_id", "group-1"},
                             ...
                             {"phase", "joint"}}),
                        replica.str() +
                            " should record the joint config row");
            tests.check(catalogHasRowOn(
                            node, "__replica_groups",
                            {{"group_id", "group-1"},
                             {"config_version", "5"},
                             ...
                             {"phase", "stable"}}),
                        replica.str() +
                            " should finalize server4 as a voter");
        }
    });

    tests.test("Promotion before learner catch-up is rejected", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 323, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 323, 2,
            RegisterReplicaGroupCommand{
                "group-1", {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 323, 3,
            DiscoverClusterNodeCommand{
                "cluster-A", "server4", "learner", "discovered", 2},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 323, 4,
            AddLearnerToGroupCommand{"group-1", "server4", 2},
            {replicas[1], replicas[2]});
        Command premature = BeginJointConfigCommand{
            "group-1",
            {"server1", "server2", "server3", "server4"},
            {"server1", "server2"},
            {"server1", "server2", "server4"},
            3};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 323, 5, premature,
            {replicas[1], replicas[2]});

        tests.check(clientReplyResultInNetworkFor(
                        state, 323, 5, Result{ConfigRejectedResult{}}),
                    "promotion before catch-up should be rejected");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(!catalogHasRowOn(
                            node, "__replica_groups",
                            {{"group_id", "group-1"},
                             {"config_version", "3"}}),
                        replica.str() +
                            " should not append the rejected config row");
        }
    });

    tests.test("Joint config requires old and new majority proofs", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 324, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 324, 2,
            RegisterReplicaGroupCommand{
                "group-1", {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 324, 3,
            DiscoverClusterNodeCommand{
                "cluster-A", "server4", "learner", "discovered", 2},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 324, 4,
            AddLearnerToGroupCommand{"group-1", "server4", 2},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 324, 5,
            MarkLearnerCaughtUpCommand{"group-1", "server4", 3},
            {replicas[1], replicas[2]});
        Command bad_quorum = BeginJointConfigCommand{
            "group-1",
            {"server1", "server2", "server3", "server4"},
            {"server1"},
            {"server1", "server2"},
            4};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 324, 6, bad_quorum,
            {replicas[1], replicas[2]});

        tests.check(clientReplyResultInNetworkFor(
                        state, 324, 6, Result{ConfigRejectedResult{}}),
                    "joint config without both majorities should be rejected");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(!catalogHasRowOn(
                            node, "__replica_groups",
                            {{"group_id", "group-1"},
                             {"config_version", "4"}}),
                        replica.str() +
                            " should not append the bad quorum config row");
        }
    });

    tests.test("Stale config version is rejected", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 325, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 325, 2,
            DiscoverClusterNodeCommand{
                "cluster-A", "server4", "learner", "discovered", 2},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 325, 3,
            RegisterReplicaGroupCommand{
                "group-1", {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        Command stale_add =
            AddLearnerToGroupCommand{"group-1", "server4", 1};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 325, 4, stale_add,
            {replicas[1], replicas[2]});

        tests.check(clientReplyResultInNetworkFor(
                        state, 325, 4, Result{ConfigRejectedResult{}}),
                    "stale config version should be rejected");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(catalogRowCountOn(node, "__replica_groups") == 1,
                        replica.str() +
                            " should keep only the initial config row");
        }
    });

    tests.test("Leader steps down when finalized config removes it", [&] {
        std::vector<Address> replicas = quorumReplicaAddresses(3);
        Address leader = replicas[0];
        Address client = ScenarioAddress::client1();
        SearchSettings settings;
        ...
        state = electConsensusLeader(state, settings, leader,
                                     {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 326, 1,
            BootstrapClusterCommand{
                "cluster-A", "server1",
                {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 326, 2,
            RegisterReplicaGroupCommand{
                "group-1", {"server1", "server2", "server3"}, 1},
            {replicas[1], replicas[2]});
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 326, 3,
            BeginJointConfigCommand{
                "group-1",
                {"server2", "server3"},
                {"server1", "server2"},
        ...
        Command finalize = FinalizeJointConfigCommand{
            ...
            3};
        state = deliverConsensusCommandToQuorum(
            state, settings, leader, client, 326, 4, finalize,
            {replicas[1], replicas[2]});
        ...
        tests.check(old_leader != nullptr &&
                    ...
                    "removed leader should step down after finalization");
        tests.check(clientReplyResultInNetworkFor(
                        state, 326, 4, Result{StatementOkResult{}}),
                    "valid removal finalization should complete");
        for (const auto& replica : replicas) {
            const auto* node = state.nodeAs<ConsensusReplica>(replica);
            tests.check(catalogHasRowOn(
                            node, "__replica_groups",
