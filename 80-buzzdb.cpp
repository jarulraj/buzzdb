...
public:
    ...
    Field& operator=(const Field& other) {
        ...
        auto new_data = std::make_unique<char[]>(data_length);
        std::memcpy(new_data.get(), other.data.get(), data_length);
...
public:
    ...
    std::optional<size_t> addTupleAndReturnSlot(std::unique_ptr<Tuple> tuple) {
        return addRawRecordAndReturnSlot(tuple->serialize());
    }

    std::optional<size_t> addRawRecordAndReturnSlot(const std::string& bytes) {
    ...
    bool putTupleAtSlot(size_t slot_id, std::unique_ptr<Tuple> tuple) {
        return updateRawRecord(slot_id, tuple->serialize());
    }
    ...
    bool updateRawRecord(size_t slot_id, const std::string& bytes) {
        if (slot_id >= MAX_SLOTS || slots()[slot_id].empty) {
            return false;
        }

        return putSerializedTupleAtSlot(slot_id, bytes);
    }
...
public:
    ...
    void recover() {
        ...
        auto records = readTuplesForAnalysis(master_record);
...
private:
    ...
    std::vector<LogRecord> readTuplesForAnalysis(
...
class MultiversionTimestampOrderingPolicy {
private:
    struct TxnState {
        int timestamp = 0;
    ...
    void removeActiveTimestamp(int timestamp) {
        auto it = std::find(
...
public:
    ...
    void begin(int txn_id) {
        std::lock_guard<std::mutex> guard(latch);
        int timestamp = next_timestamp++;
        active_txns[txn_id] = {timestamp, false};
        txn_timestamps[txn_id] = timestamp;
        active_timestamps.push_back(timestamp);
        std::cout << "MVTO assigns timestamp " << timestamp
                  << " to txn " << txn_id << "." << std::endl;
    }

    ConcurrencyControlResult beforeCommit(int txn_id) const {
        std::lock_guard<std::mutex> guard(latch);
        if (active_txns.find(txn_id) == active_txns.end()) {
            return {
                false,
                "txn " + std::to_string(txn_id) + " has no MVTO state"
            };
        }
        return {};
    }

    void afterCommit(int txn_id) {
        std::lock_guard<std::mutex> guard(latch);
        auto txn_it = active_txns.find(txn_id);
        if (txn_it == active_txns.end()) {
            return;
        }
        removeActiveTimestamp(txn_it->second.timestamp);
        std::cout << "MVTO txn " << txn_id << " releases timestamp "
                  << txn_it->second.timestamp;
        if (txn_it->second.has_writes) {
            std::cout << " after publishing version(s)";
        }
        std::cout << "." << std::endl;
        active_txns.erase(txn_it);
    }
    ...
    int timestamp(int txn_id) const {
        std::lock_guard<std::mutex> guard(latch);
        auto active_it = active_txns.find(txn_id);
        if (active_it != active_txns.end()) {
            return active_it->second.timestamp;
        }
    ...
    int oldestActiveTimestamp() const {
        ...
        return *std::min_element(active_timestamps.begin(),
                                 active_timestamps.end());
    }
};
...
private:
    int next_txn_id = 1;
    MultiversionTimestampOrderingPolicy mvto_policy;
...
public:
    ...
    TxnPtr begin() {
        ...
        mvto_policy.begin(txn_id);
        return txn;
    }

    ConcurrencyControlResult prepareCommit(const TxnContext& txn) const {
        requireRunningTransaction(txn);
        return mvto_policy.beforeCommit(txn.id);
    }

    bool commit(TxnContext& txn) {
        auto result = prepareCommit(txn);
        if (!result.granted) {
            std::cout << result.detail << "; abort txn "
                      << txn.id << "." << std::endl;
            abort(txn);
            return false;
        }
        ...
        mvto_policy.afterCommit(txn.id);
        return true;
    }
    ...
    void abort(TxnContext& txn) {
        ...
        mvto_policy.abort(txn.id);
    }

    int mvtoTimestamp(const TxnContext& txn) const {
        return mvto_policy.timestamp(txn.id);
    }

    int oldestMVTOTimestamp() const {
        return mvto_policy.oldestActiveTimestamp();
    }

    void markMVTOWrite(const TxnContext& txn) {
        requireRunningTransaction(txn);
        mvto_policy.markWrite(txn.id);
    }
};
...
static constexpr size_t INVALID_SLOT_ID = std::numeric_limits<size_t>::max();

struct TupleId {
    PageID page_id = INVALID_PAGE_ID;
    size_t slot_id = INVALID_SLOT_ID;
...
int compareFields(const Field& left, const Field& right) {
    if (left.getType() != right.getType()) {
        return static_cast<int>(left.getType()) <
               static_cast<int>(right.getType()) ? -1 : 1;
    }
    ...
    switch (left.getType()) {
        case INT:
            ...
            return left.asInt() < right.asInt() ? -1 : 1;
        case FLOAT:
            if (left.asFloat() == right.asFloat()) {
                return 0;
            }
            return left.asFloat() < right.asFloat() ? -1 : 1;
        case STRING:
            if (left.asString() == right.asString()) {
                return 0;
            }
            return left.asString() < right.asString() ? -1 : 1;
    }
    throw std::runtime_error("Unsupported field type.");
}
...
struct TupleKey {
    TableId table_id = INVALID_TABLE_ID;
    ColumnId key_column_id = 0;
    Field key_value;

    TupleKey(TableId table_id,
             ColumnId key_column_id,
             const Field& key_value)
        : table_id(table_id),
    ...
    bool operator==(const TupleKey& other) const {
        return table_id == other.table_id &&
               key_column_id == other.key_column_id &&
    ...
    bool operator<(const TupleKey& other) const {
        ...
        if (key_column_id != other.key_column_id) {
            return key_column_id < other.key_column_id;
        }
        return compareFields(key_value, other.key_value) < 0;
    }

    std::string label() const {
        return "table=" + std::to_string(table_id) +
               " key_column=" + std::to_string(key_column_id) +
               " value=" +
               fieldToString(key_value);
    }
};

struct VersionedTupleHeader {
    int wts = 0;
    int rts = 0;
    int creator_txn = 0;
    bool committed = true;
    TupleId previous_version;
};
...
struct FieldAssignment {
    ...
    FieldAssignment(std::string name, int value)
        : name(std::move(name)), value(value) {}

    FieldAssignment(std::string name, float value)
        : name(std::move(name)), value(value) {}

    FieldAssignment(std::string name, const char* value)
        : name(std::move(name)), value(std::string(value)) {}

    FieldAssignment(std::string name, const std::string& value)
        : name(std::move(name)), value(value) {}

    FieldAssignment(std::string name, const Field& value)
        : name(std::move(name)), value(value) {}
};

using TupleUpdate = std::vector<FieldAssignment>;

struct ColumnAssignment {
    ColumnId column_id = 0;
    Field value;

    ColumnAssignment(ColumnId column_id, const Field& value)
        : column_id(column_id), value(value) {}
};

using TypedTupleUpdate = std::vector<ColumnAssignment>;

struct PendingTupleVersion {
    TupleId tuple_id;
    TupleKey key;
};

std::string typedTupleUpdateLabel(const TableSchema& schema,
                                  const TypedTupleUpdate& changes) {
    std::ostringstream output;
    bool first = true;
    for (const auto& change : changes) {
        ...
        if (change.column_id >= schema.columns.size()) {
            throw std::runtime_error("Typed tuple update has bad column id.");
        }
        output << schema.columns[change.column_id].name
               << "=" << fieldToString(change.value);
        first = false;
    }
    return output.str();
}

const Field& tupleFieldAt(const Tuple& tuple, size_t index) {
    if (index >= tuple.fields.size()) {
        throw std::runtime_error("Tuple is missing field at index " +
                                 std::to_string(index));
    }
    return *tuple.fields[index];
}

std::string tupleLabel(const TableSchema& schema, const Tuple& tuple) {
    if (schema.columns.size() != tuple.fields.size()) {
        throw std::runtime_error("Tuple field count does not match schema.");
    }
    ...
    for (size_t i = 0; i < schema.columns.size(); i++) {
        ...
        output << schema.columns[i].name << "="
               << fieldToString(tupleFieldAt(tuple, i));
    }
    return output.str();
}

std::unique_ptr<Tuple> mergeTupleByColumn(const TableSchema& schema,
                                          const Tuple* base,
                                          const TypedTupleUpdate& changes) {
    std::unique_ptr<Tuple> result = base ? base->clone()
                                         : std::make_unique<Tuple>();
    if (!base) {
        for (size_t i = 0; i < schema.columns.size(); i++) {
            auto change = std::find_if(
                changes.begin(),
                changes.end(),
                [&](const ColumnAssignment& assignment) {
                    return assignment.column_id == i;
                }
            );
            if (change == changes.end()) {
                throw std::runtime_error(
                    "INSERT-like version for table is missing column id: " +
                    std::to_string(i)
                );
            }
            result->addField(change->value.clone());
        }
        return result;
    }
    ...
    for (const auto& change : changes) {
        ...
        result->fields[change.column_id] = change.value.clone();
    }
    return result;
}

class VersionedTupleStore {
private:
    static constexpr size_t HEADER_FIELD_COUNT = 6;
    static constexpr size_t HEADER_BYTES = HEADER_FIELD_COUNT * sizeof(int32_t);

    struct StoredVersion {
        TupleId tuple_id;
        VersionedTupleHeader header;
    ...
    static void appendInt32(std::string& bytes, int32_t value) {
        const char* raw = reinterpret_cast<const char*>(&value);
        bytes.append(raw, sizeof(value));
    }

    static int32_t readInt32(const std::string& bytes, size_t& offset) {
        if (offset + sizeof(int32_t) > bytes.size()) {
            throw std::runtime_error("Corrupt version header.");
        }
        int32_t value = 0;
    ...
    static std::string serializeHeader(const VersionedTupleHeader& header) {
        std::string bytes;
        bytes.reserve(HEADER_BYTES);
        appendInt32(bytes, static_cast<int32_t>(header.wts));
        appendInt32(bytes, static_cast<int32_t>(header.rts));
        appendInt32(bytes, static_cast<int32_t>(header.creator_txn));
        appendInt32(bytes, static_cast<int32_t>(header.committed ? 1 : 0));
        appendInt32(
            bytes,
            header.previous_version.valid()
                ? static_cast<int32_t>(header.previous_version.page_id)
                : -1
        );
        appendInt32(
            bytes,
            header.previous_version.valid()
                ? static_cast<int32_t>(header.previous_version.slot_id)
                : -1
        );
        return bytes;
    }

    static VersionedTupleHeader deserializeHeader(const std::string& bytes) {
        if (bytes.size() < HEADER_BYTES) {
            throw std::runtime_error("Version record is missing system columns.");
        }

        size_t offset = 0;
        VersionedTupleHeader header;
        header.wts = readInt32(bytes, offset);
        header.rts = readInt32(bytes, offset);
        header.creator_txn = readInt32(bytes, offset);
        header.committed = readInt32(bytes, offset) != 0;
        int32_t previous_page = readInt32(bytes, offset);
        int32_t previous_slot = readInt32(bytes, offset);
        if (previous_page >= 0 && previous_slot >= 0) {
            header.previous_version = {
                static_cast<PageID>(previous_page),
                static_cast<size_t>(previous_slot)
            };
        }
        return header;
    }

    static std::string serializeRecord(const VersionedTupleHeader& header,
                                       const Tuple& tuple) {
    ...
    void flushPage(PageID page_id) {
        storage_manager.flush(page_id, pages[page_id]);
    }
...
public:
    VersionedTupleStore() {
        ...
        pages.push_back(storage_manager.load(0));
        pages[0]->reset();
        pages[0]->setTableId(FIRST_USER_TABLE_ID);
        flushPage(0);
    }

    TupleId insertVersion(const VersionedTupleHeader& header,
                           const Tuple& tuple) {
        auto bytes = serializeRecord(header, tuple);
        for (size_t page_index = 0; page_index < pages.size(); page_index++) {
            auto slot = pages[page_index]->addRawRecordAndReturnSlot(bytes);
            if (slot) {
                flushPage(static_cast<PageID>(page_index));
                return {
                    static_cast<PageID>(page_index),
                    *slot
                };
            }
        }
        ...
        auto page = storage_manager.load(page_id);
        page->reset();
        page->setTableId(FIRST_USER_TABLE_ID);
        pages.push_back(std::move(page));
        auto slot = pages.back()->addRawRecordAndReturnSlot(bytes);
        if (!slot) {
            throw std::runtime_error("Version record does not fit in an empty page.");
        }
        flushPage(page_id);
        return {
            page_id,
    ...
    StoredVersion readVersion(const TupleId& tuple_id) const {
        if (!tuple_id.valid() || tuple_id.page_id >= pages.size()) {
            throw std::runtime_error("Invalid version TupleId: " + tuple_id.label());
        }

        auto bytes = pages[tuple_id.page_id]->getRawRecord(tuple_id.slot_id);
        if (!bytes) {
            throw std::runtime_error("Missing version record: " + tuple_id.label());
        }
        auto header = deserializeHeader(*bytes);
        std::istringstream tuple_input(bytes->substr(HEADER_BYTES));
        auto tuple = Tuple::deserialize(tuple_input);
        return {tuple_id, header, std::move(tuple)};
    }

    void updateHeader(const TupleId& tuple_id,
                      const VersionedTupleHeader& header) {
        auto version = readVersion(tuple_id);
        auto bytes = serializeRecord(header, *version.tuple);
        if (!pages[tuple_id.page_id]->updateRawRecord(tuple_id.slot_id, bytes)) {
            throw std::runtime_error(
                "Updated version header no longer fits in " + tuple_id.label()
            );
        }
        flushPage(tuple_id.page_id);
    }

    void eraseVersion(const TupleId& tuple_id) {
        if (!tuple_id.valid() || tuple_id.page_id >= pages.size()) {
            return;
        }
        pages[tuple_id.page_id]->deleteTuple(tuple_id.slot_id);
        flushPage(tuple_id.page_id);
    }
};
...
private:
    ...
    VersionedTupleStore tuple_store;
    TableId next_table_id = FIRST_USER_TABLE_ID;
    std::map<TableId, TableDefinition> tables_by_id;
    std::map<std::string, TableId> table_ids_by_name;
    std::map<TupleKey, TupleId> version_heads;

    static ColumnId checkedColumnId(const TableSchema& schema,
                                    const std::string& column_name) {
        auto column_index = schema.getColumnIndex(column_name);
        if (column_index < 0 ||
            static_cast<size_t>(column_index) >
                std::numeric_limits<ColumnId>::max()) {
            throw std::runtime_error("Column id is out of range.");
        }
        return static_cast<ColumnId>(column_index);
    }

    const TableDefinition& tableDefinition(TableId table_id) const {
        auto it = tables_by_id.find(table_id);
        if (it == tables_by_id.end()) {
            throw std::runtime_error(
                "Unknown MVTO table id: " + std::to_string(table_id)
            );
        }
        return it->second;
    }

    static std::string versionName(const TupleId& tuple_id) {
        return tuple_id.label();
    }

    static std::string creatorLabel(int creator_txn) {
        return creator_txn == 0 ?
            std::string("bootstrap") :
            "txn " + std::to_string(creator_txn);
    }

    TupleKey keyFromTuple(const TableDefinition& table,
                          const Tuple& tuple) const {
        return TupleKey(
            table.table_id,
            table.key_column_id,
            tupleFieldAt(tuple, table.key_column_id)
        );
    }

    std::optional<TupleId> readableVersionTupleId(const TupleKey& key,
                                               int timestamp,
                                               int txn_id) const {
        auto head = version_heads.find(key);
        if (head == version_heads.end()) {
            return std::nullopt;
        }

        TupleId tuple_id = head->second;
        while (tuple_id.valid()) {
            auto version = tuple_store.readVersion(tuple_id);
            const auto& header = version.header;
            if (header.wts <= timestamp &&
                (header.committed || header.creator_txn == txn_id)) {
                return tuple_id;
            }
            tuple_id = header.previous_version;
        }
        return std::nullopt;
    }

    std::optional<TupleId> predecessorForWriteTupleId(
            const TupleKey& key,
            int timestamp) const {
        auto head = version_heads.find(key);
        if (head == version_heads.end()) {
        ...
        while (tuple_id.valid()) {
            auto version = tuple_store.readVersion(tuple_id);
            const auto& header = version.header;
            if (header.committed && header.wts < timestamp) {
                return tuple_id;
            }
            tuple_id = header.previous_version;
        }
...
public:
    TableId registerTable(const std::string& table_name,
                          const TableSchema& schema,
                          const std::string& key_column_name) {
        auto existing = table_ids_by_name.find(table_name);
        if (existing != table_ids_by_name.end()) {
            return existing->second;
        }

        TableId table_id = next_table_id++;
        ColumnId key_column_id = checkedColumnId(schema, key_column_name);
        ...
        table_ids_by_name[table_name] = table_id;
        std::cout << "Register table " << table_name
                  << " as table_id=" << table_id
                  << " with key column "
                  << schema.columns[key_column_id].name
                  << "." << std::endl;
        return table_id;
    }

    TableId tableId(const std::string& table_name) const {
        auto it = table_ids_by_name.find(table_name);
        if (it == table_ids_by_name.end()) {
    ...
    std::string keyLabel(const TupleKey& key) const {
        const auto& table = tableDefinition(key.table_id);
        std::ostringstream output;
        output << table.name << "."
               << table.schema.columns[key.key_column_id].name
               << "=" << fieldToString(key.key_value);
        return output.str();
    }

    void loadTuple(TableId table_id,
                   std::unique_ptr<Tuple> tuple) {
        const auto& table = tableDefinition(table_id);
        auto key = keyFromTuple(table, *tuple);
        VersionedTupleHeader header;
        auto tuple_id = tuple_store.insertVersion(header, *tuple);
        version_heads[key] = tuple_id;
    }

    TupleKey keyFor(TableId table_id, const Field& key_value) const {
        const auto& table = tableDefinition(table_id);
        return TupleKey(table_id, table.key_column_id, key_value);
    }

    TypedTupleUpdate resolveUpdate(TableId table_id,
                                   const TupleUpdate& changes) const {
        const auto& table = tableDefinition(table_id);
        TypedTupleUpdate typed_changes;
        for (const auto& change : changes) {
            ColumnId column_id = checkedColumnId(table.schema, change.name);
            if (table.schema.columns[column_id].type != change.value.getType()) {
                throw std::runtime_error(
                    "Update value type does not match column type."
                );
            }
            typed_changes.emplace_back(column_id, change.value);
        }
        return typed_changes;
    }

    std::optional<TupleId> readTupleVersion(int txn_id,
                                            const std::string& txn_label,
                                            int timestamp,
                                            const TupleKey& key) {
        auto tuple_id = readableVersionTupleId(key, timestamp, txn_id);
        if (!tuple_id) {
            std::cout << txn_label << " reads " << keyLabel(key)
                      << " at ts=" << timestamp
                      << " -> no visible version." << std::endl;
            return std::nullopt;
        }
        ...
        std::cout << txn_label << " reads " << keyLabel(key)
                  << " at ts=" << timestamp
                  << " -> " << tupleLabel(table.schema, *version.tuple)
                  << " from " << versionName(*tuple_id)
                  << " [_wts=" << version.header.wts
                  << ", _rts=" << version.header.rts
                  << ", _prev=" << version.header.previous_version.label()
                  << "]." << std::endl;

        if (timestamp > version.header.rts) {
            version.header.rts = timestamp;
            tuple_store.updateHeader(*tuple_id, version.header);
            std::cout << "  MVTO persists "
                      << versionName(*tuple_id)
                      << "._rts = " << timestamp << "." << std::endl;
        }
        return tuple_id;
    }

    std::optional<PendingTupleVersion> createVersion(
            int txn_id,
            const std::string& txn_label,
            int timestamp,
            const TupleKey& key,
        ...
        const auto& table = tableDefinition(key.table_id);
        auto typed_changes = resolveUpdate(key.table_id, changes);
        std::cout << txn_label << " requests WRITE " << keyLabel(key)
                  << " at ts=" << timestamp << " -> "
                  << typedTupleUpdateLabel(table.schema, typed_changes)
                  << "." << std::endl;

        auto predecessor = predecessorForWriteTupleId(key, timestamp);
        if (predecessor) {
            auto predecessor_version = tuple_store.readVersion(*predecessor);
            if (predecessor_version.header.rts > timestamp) {
                std::cout << "  MVTO rejects write because "
                          << versionName(*predecessor)
                          << "._rts=" << predecessor_version.header.rts
        ...
        std::unique_ptr<Tuple> base_tuple;
        if (predecessor) {
            base_tuple = tuple_store.readVersion(*predecessor).tuple->clone();
        }
        auto new_tuple = mergeTupleByColumn(
            table.schema,
            base_tuple ? base_tuple.get() : nullptr,
            typed_changes
        );

        VersionedTupleHeader header;
        header.wts = timestamp;
        header.rts = timestamp;
        header.creator_txn = txn_id;
        header.committed = false;
        header.previous_version = predecessor.value_or(TupleId{});
        auto tuple_id = tuple_store.insertVersion(header, *new_tuple);
        version_heads[key] = tuple_id;

        std::cout << "  MVTO accepts write; create active "
                  << versionName(tuple_id)
                  << " with persisted system columns "
                  << "[_wts=_rts=" << timestamp
                  << ", _prev=" << header.previous_version.label()
                  << "]." << std::endl;
        return PendingTupleVersion{tuple_id, key};
    }

    void publishWrites(const std::string& txn_label,
                       const std::vector<PendingTupleVersion>& writes) {
        std::cout << txn_label << " publishes "
                  << writes.size() << " physical version(s)." << std::endl;
        for (const auto& write : writes) {
            auto version = tuple_store.readVersion(write.tuple_id);
            version.header.committed = true;
            tuple_store.updateHeader(write.tuple_id, version.header);
            std::cout << "  commit " << versionName(write.tuple_id)
                      << " for " << keyLabel(write.key)
                      << " by setting _committed=1." << std::endl;
        }
    }

    void discardWrites(const std::string& txn_label,
                       const std::vector<PendingTupleVersion>& writes) {
        std::cout << txn_label
                  << " aborts; discard active physical version(s)."
                  << std::endl;
        for (const auto& write : writes) {
            auto version = tuple_store.readVersion(write.tuple_id);
            auto head = version_heads.find(write.key);
            if (head != version_heads.end() && head->second == write.tuple_id) {
                if (version.header.previous_version.valid()) {
                    head->second = version.header.previous_version;
                } else {
                    version_heads.erase(head);
                }
            }
            tuple_store.eraseVersion(write.tuple_id);
            std::cout << "  remove " << versionName(write.tuple_id)
                      << " before it becomes visible." << std::endl;
        }
    }

    void printVersionChain(const TupleKey& key) const {
        const auto& table = tableDefinition(key.table_id);
        std::cout << "Version chain for " << keyLabel(key) << ":"
                  << std::endl;
        ...
        while (tuple_id.valid()) {
            auto version = tuple_store.readVersion(tuple_id);
            std::cout << "  " << versionName(tuple_id)
                      << " [_wts=" << version.header.wts
                      << ", _rts=" << version.header.rts
                      << ", _prev=" << version.header.previous_version.label()
                      << "] "
                      << tupleLabel(table.schema, *version.tuple)
                      << " by " << creatorLabel(version.header.creator_txn)
                      << (version.header.committed ? " committed" : " active")
                      << std::endl;
            tuple_id = version.header.previous_version;
        }
    }

    void collectGarbage(const std::string& label, int oldest_timestamp) {
        std::cout << label << ": oldest_active_ts="
                  << oldest_timestamp << std::endl;

        size_t deleted = 0;
        for (auto& [key, head_tuple_id] : version_heads) {
            TupleId newer_tuple_id;
            TupleId current_tuple_id = head_tuple_id;
            while (current_tuple_id.valid()) {
                auto current = tuple_store.readVersion(current_tuple_id);
                bool reclaimable = false;
                if (newer_tuple_id.valid() && current.header.committed) {
                    auto newer = tuple_store.readVersion(newer_tuple_id);
                    reclaimable = newer.header.committed &&
                                  newer.header.wts <= oldest_timestamp;
                }

                std::cout << "  " << versionName(current_tuple_id)
                          << " for " << keyLabel(key)
                          << " [_wts=" << current.header.wts
                          << ", _rts=" << current.header.rts
                          << "] -> "
                          << (reclaimable ? "reclaimable" : "retain")
                          << std::endl;

                TupleId previous = current.header.previous_version;
                if (reclaimable) {
                    auto newer = tuple_store.readVersion(newer_tuple_id);
                    newer.header.previous_version = previous;
                    tuple_store.updateHeader(newer_tuple_id, newer.header);
                    tuple_store.eraseVersion(current_tuple_id);
                    deleted++;
                    current_tuple_id = previous;
                    continue;
                }

                newer_tuple_id = current_tuple_id;
                current_tuple_id = previous;
            }
        }

        std::cout << "  GC deleted " << deleted
                  << " obsolete physical version row(s)." << std::endl;
    }
};

class DatabaseImporter {
private:
    static FieldType parseFieldType(const std::string& type_name) {
        if (type_name == "INT") {
            return INT;
        }
        if (type_name == "FLOAT") {
    ...
    static TableSchema parseTableSchema(const std::vector<std::string>& tokens) {
        ...
        for (size_t i = 2; i < tokens.size(); i++) {
            ...
            if (separator == std::string::npos) {
                throw std::runtime_error(
                    "Bad column declaration: " + tokens[i]
                );
            }
    ...
    static std::unique_ptr<Tuple> makeTuple(
        ...
        if (schema.columns.size() != values.size()) {
            throw std::runtime_error("Fixture row has the wrong field count.");
        }
...
public:
    static void importTables(
        ...
        if (!inputFile) {
            throw std::runtime_error(
                "Unable to open fixture file: " + filename
            );
        }

        std::map<std::string, TableSchema> table_schemas;
        ...
        while (std::getline(inputFile, line)) {
            ...
            if (tokens[0] == "TABLE") {
                ...
                auto key_it = key_columns_by_table.find(table_name);
                if (key_it != key_columns_by_table.end()) {
                    imported_table_ids[table_name] =
                        version_manager.registerTable(
                            table_name,
                            schema,
                            key_it->second
                        );
            ...
            auto imported_table = imported_table_ids.find(table_name);
            if (imported_table == imported_table_ids.end()) {
                continue;
            }

            std::vector<std::string> values(tokens.begin() + 1, tokens.end());
            version_manager.loadTuple(
                imported_table->second,
                makeTuple(table_schemas.at(table_name), values)
            );
        }
    }
};
...
private:
    std::map<int, std::vector<PendingTupleVersion>> active_writes;
    ...
    void discardActiveWrites(int txn_id, const std::string& txn_label) {
        auto writes = active_writes.find(txn_id);
        if (writes == active_writes.end()) {
            return;
        }
        version_manager.discardWrites(txn_label, writes->second);
        active_writes.erase(writes);
    }
...
public:
    ...
    void trackPendingVersion(const TxnContext& txn,
                             const PendingTupleVersion& write) {
        requireRunningTransaction(txn);
        active_writes[txn.id].push_back(write);
        txn_manager.markMVTOWrite(txn);
    }

    bool commit(const TxnPtr& txn, const std::string& txn_label) {
        if (!txn) {
            throw std::runtime_error(
        ...
        auto ready = txn_manager.prepareCommit(*txn);
        if (!ready.granted) {
            std::cout << ready.detail << "; abort txn "
                      << txn->id << "." << std::endl;
            discardActiveWrites(txn->id, txn_label);
            txn_manager.abort(*txn);
            return false;
        }

        auto writes = active_writes.find(txn->id);
        if (writes != active_writes.end() && !writes->second.empty()) {
            version_manager.publishWrites(txn_label, writes->second);
            active_writes.erase(writes);
        }
    ...
    void abort(const TxnPtr& txn, const std::string& txn_label) {
    ...
    void collectGarbage(const std::string& label) {
        version_manager.collectGarbage(
            label,
            txn_manager.oldestMVTOTimestamp()
        );
    }
};
    ...
    StatementResult execute(const ParsedStatement& statement,
        ...
        switch (statement.kind) {
            case ParsedStatement::Kind::Select:
                return executeSelect(statement, txn, txn_label);
...
private:
    ...
    TupleUpdate bindAssignments(TableId table_id,
    ...
    TupleKey bindKeyPredicate(TableId table_id,
        ...
        if (where_terms.size() != 1) {
            throw std::runtime_error(
                "MVTO SQL statements require one equality predicate on the key column."
            );
        }
        ...
        if (column_id !=
            ...
            throw std::runtime_error(
                "MVTO SQL predicate must use the table key column."
            );
        }
    ...
    StatementResult executeSelect(const ParsedStatement& statement,
        ...
        auto tuple_id =
            transactional_storage_manager.version_manager.readTupleVersion(
                txn->id,
                txn_label,
                transactional_storage_manager.txn_manager.mvtoTimestamp(*txn),
                key
            );
        return {tuple_id ? 1U : 0U};
    }
    ...
    StatementResult executeUpdate(const ParsedStatement& statement,
        ...
        auto key = bindKeyPredicate(table_id, statement.where_terms);
        auto write = transactional_storage_manager.version_manager.createVersion(
            txn->id,
            txn_label,
            transactional_storage_manager.txn_manager.mvtoTimestamp(*txn),
            key,
            bindAssignments(table_id, statement.assignments)
        );
        if (!write) {
            transactional_storage_manager.abort(txn, txn_label);
            return {};
        }

        transactional_storage_manager.trackPendingVersion(*txn, *write);
        return {1};
    }
};
...
private:
    ...
    std::string txnLabel(const TxnPtr& txn) const {
        auto it = txn_labels.find(txn->id);
        if (it == txn_labels.end()) {
            return "txn " + std::to_string(txn->id);
        }
        return it->second;
    }
...
public:
    ...
    TxnPtr beginTransaction(const std::string& label,
        ...
        std::cout << label << " " << kind << std::endl;
        auto txn = transactional_storage_manager.txn_manager.begin();
        txn_labels[txn->id] = label;
        std::cout << label << " begins with timestamp "
                  << transactional_storage_manager.txn_manager.mvtoTimestamp(
                         *txn)
                  << "." << std::endl;
        return txn;
    }

    TxnPtr beginReadOnly(const std::string& label) {
        return beginTransaction(label, "READ ONLY");
    }

    TxnPtr beginUpdater(const std::string& label) {
        return beginTransaction(label, "UPDATER");
    }
    ...
    bool commit(const TxnPtr& txn) {
        requireRunningTransaction(txn);
        return transactional_storage_manager.commit(txn, txnLabel(txn));
    }

    void abort(const TxnPtr& txn) {
        transactional_storage_manager.abort(txn, txnLabel(txn));
    }

    void collectGarbage(const std::string& label) {
        transactional_storage_manager.collectGarbage(label);
    }

    void printTxnState(const std::string& label, const TxnPtr& txn) const {
        std::cout << label << " final state: "
                  << txnStateName(txn) << std::endl;
    }
...
void runMVTO() {
    ...
    BuzzDB db;
    db.importDatabaseDump(
        "booking.txt",
        {{"seats", "seat_no"}, {"holds", "id"}}
    );

    TableId seats_table = db.tableId("seats");
    TableId holds_table = db.tableId("holds");
    auto seat_1a = db.keyFor(
        seats_table,
        Field(std::string("1A"))
    );
    auto seat_1b = db.keyFor(
        seats_table,
        Field(std::string("1B"))
    );
    auto hold_1 = db.keyFor(
        holds_table,
        Field(1)
    );

    std::cout << db.concurrencyControlPolicyName() << std::endl;
    std::cout << "Each MVTO version is a physical "
              << "slotted-page tuple with additional system columns."
              << std::endl;
    std::cout << "Reads choose the newest committed version with wts <= txn "
              << "timestamp; writes create active versions unless they would "
              << "invalidate a later read." << std::endl;

    std::cout << "\nSchedule A: reader and writer coexist through versions"
              << std::endl;
    auto t1 = db.beginReadOnly("T1_old_reader");
    db.execute(t1, "SELECT {*} FROM seats WHERE seat_no = 1A");
    auto t2 = db.beginUpdater("T2_writer");
    db.execute(
        t2,
    ...
    db.execute(
        t2,
        "UPDATE holds SET id = 1, seat_id = 1, customer = garcia, status = open "
        "WHERE id = 1"
    );
    db.commit(t2);
    db.execute(t1, "SELECT {*} FROM seats WHERE seat_no = 1A");
    auto t3 = db.beginReadOnly("T3_new_reader");
    db.execute(t3, "SELECT {*} FROM seats WHERE seat_no = 1A");
    db.execute(t3, "SELECT {*} FROM holds WHERE id = 1");
    db.commit(t3);
    db.printTxnState("T1_old_reader", t1);
    db.printTxnState("T2_writer", t2);
    db.printTxnState("T3_new_reader", t3);

    std::cout << "\nSchedule B: older reader can still use an older seat version"
              << std::endl;
    auto t4 = db.beginUpdater("T4_writer");
    db.execute(
        t4,
        "UPDATE seats SET status = held, customer = patel "
        "WHERE seat_no = 1B"
    );
    db.commit(t4);
    db.execute(t1, "SELECT {*} FROM seats WHERE seat_no = 1B");
    db.printTxnState("T4_writer", t4);

    std::cout << "\nSchedule C: late older writer still aborts"
              << std::endl;
    auto t5 = db.beginUpdater("T5_old_writer");
    auto t6 = db.beginReadOnly("T6_later_reader");
    db.execute(t6, "SELECT {*} FROM seats WHERE seat_no = 1A");
    db.execute(
        t5,
        "UPDATE seats SET status = held, customer = patel "
        "WHERE seat_no = 1A"
    );
    if (t5->state == TxnContext::ABORTED) {
        std::cout << "T5_old_writer is aborted by MVTO." << std::endl;
    }
    db.commit(t6);
    db.printTxnState("T5_old_writer", t5);
    db.printTxnState("T6_later_reader", t6);

    std::cout << "\nSchedule D: aborted active version and version cleanup"
              << std::endl;
    auto t7 = db.beginUpdater("T7_aborter");
    db.execute(
        t7,
        "UPDATE seats SET status = held, customer = chen "
        "WHERE seat_no = 1B"
    );
    db.abort(t7);
    db.printVersionChain(seat_1a);
    db.printVersionChain(seat_1b);
    db.printVersionChain(hold_1);
    db.collectGarbage("GC while T1_old_reader is active");
    db.commit(t1);
    db.collectGarbage("GC after T1_old_reader commits");
    db.printVersionChain(seat_1a);
    db.printVersionChain(seat_1b);
    db.printVersionChain(hold_1);
    db.printTxnState("T1_old_reader", t1);
    db.printTxnState("T7_aborter", t7);
}
