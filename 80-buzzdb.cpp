...
struct TupleKey {
    ...
    TupleKey(std::string table_name,
             std::string key_column,
    ...
    bool operator<(const TupleKey& other) const {
        ...
        if (key_column != other.key_column) {
            return key_column < other.key_column;
        }
...
std::string tupleUpdateLabel(const TupleUpdate& changes) {
    std::ostringstream output;
    bool first = true;
    for (const auto& change : changes) {
        ...
        output << change.name << "=" << fieldToString(change.value);
        first = false;
    }
    return output.str();
}
...
std::unique_ptr<Tuple> mergeTuple(const TableSchema& schema,
                                  const Tuple* base,
        ...
        for (const auto& column : schema.columns) {
            auto change = std::find_if(
                ...
                [&](const FieldAssignment& assignment) {
                    return assignment.name == column.name;
                }
            );
            if (change == changes.end()) {
                throw std::runtime_error(
                    "INSERT-like version for table is missing column: " +
                    column.name
                );
            }
    ...
    for (const auto& change : changes) {
        size_t index = static_cast<size_t>(schema.getColumnIndex(change.name));
        result->fields[index] = change.value.clone();
    }
    return result;
}
...
private:
    ...
    const TableDefinition& tableDefinition(const std::string& table_name) const {
        auto it = tables.find(table_name);
        if (it == tables.end()) {
            throw std::runtime_error("Unknown MVTO table: " + table_name);
        }
        return it->second;
    }
    ...
    TupleKey keyFromTuple(const TableDefinition& table,
        ...
        size_t key_index = static_cast<size_t>(
            table.schema.getColumnIndex(table.key_column)
        );
        return TupleKey(table.name, table.key_column, tupleFieldAt(tuple, key_index));
    }
...
public:
    void registerTable(const std::string& table_name,
        ...
        schema.getColumnIndex(key_column);
        tables[table_name] = {table_name, schema, key_column};
    }

    void loadTuple(const std::string& table_name,
                   std::unique_ptr<Tuple> tuple) {
        const auto& table = tableDefinition(table_name);
    ...
    TupleKey keyFor(const std::string& table_name,
                    const std::string& key_column,
                    const std::string& value) const {
        const auto& table = tableDefinition(table_name);
        size_t key_index = static_cast<size_t>(
            table.schema.getColumnIndex(key_column)
        );
        auto field = parseFieldValue(table.schema.columns[key_index].type, value);
        return TupleKey(table_name, key_column, *field);
    }
    ...
    std::optional<TupleId> readTuple(const TxnPtr& txn,
        ...
        if (!tuple_id) {
            std::cout << txnLabel(txn) << " reads " << key.label()
                      << " at ts=" << timestamp
        ...
        auto version = tuple_store.readVersion(*tuple_id);
        const auto& table = tableDefinition(key.table_name);
        std::cout << txnLabel(txn) << " reads " << key.label()
                  << " at ts=" << timestamp
    ...
    std::optional<PendingTupleVersion> writeTuple(
            ...
            const TupleUpdate& changes) {
        int timestamp = txn_manager.mvtoTimestamp(*txn);
        std::cout << txnLabel(txn) << " requests WRITE " << key.label()
                  << " at ts=" << timestamp << " -> "
                  << tupleUpdateLabel(changes) << "." << std::endl;
    ...
    void printVersionChain(const TupleKey& key) const {
        const auto& table = tableDefinition(key.table_name);
        std::cout << "Version chain for " << key.label() << ":" << std::endl;
...
private:
    static bool importsTable(const std::string& table_name) {
        return table_name == "seats" || table_name == "holds";
    }

    static std::string keyColumnFor(const std::string& table_name) {
        return table_name == "seats" ? "seat_no" : "id";
    }
...
public:
    static void importBookingDemo(VersionManager& version_manager,
        ...
        while (std::getline(inputFile, line)) {
            ...
            if (tokens[0] == "TABLE") {
                ...
                if (importsTable(table_name)) {
                    version_manager.registerTable(
                        table_name,
                        schema,
                        keyColumnFor(table_name)
                    );
                }
                continue;
            }
...
void runMVTO() {
    ...
    DatabaseImporter::importBookingDemo(version_manager, "booking.txt");

    auto seat_1a = version_manager.keyFor("seats", "seat_no", "1A");
    auto seat_1b = version_manager.keyFor("seats", "seat_no", "1B");
    auto hold_1 = version_manager.keyFor("holds", "id", "1");
