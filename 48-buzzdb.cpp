...
const std::string CATALOG_MAGIC = "BUZZDB_CATALOG";
...
// Page 0 persists catalog; table heaps follow next_page links from first_page.
class Catalog {
private:
...
public:
    explicit Catalog(BufferManager& manager) : buffer_manager(manager) {
        load();
    }

    void load() {
        auto& page = buffer_manager.getPage(CATALOG_PAGE_ID);
        std::string catalog_text = readPageText(page);
        std::cout << catalog_text;
        auto first_line_end = catalog_text.find('\n');
        std::string first_line = first_line_end == std::string::npos
            ? catalog_text
            : catalog_text.substr(0, first_line_end);
        if (first_line != CATALOG_MAGIC) {
...
private:
    ...
    void loadFromText(const std::string& catalog_text) {
        tables_by_name.clear();

        std::istringstream input(catalog_text);
        std::string line;
        if (!std::getline(input, line)) {
            throw std::runtime_error("Catalog page is empty.");
        }

        if (line != CATALOG_MAGIC) {
            throw std::runtime_error(database_filename + " is not a valid BuzzDB file.");
        }
        ...
        while (std::getline(input, line)) {
            ...
            if (tokens.size() < 5 || tokens[0] != "TABLE") {
                throw std::runtime_error("Bad catalog table record.");
            }

            TableId table_id = static_cast<TableId>(std::stoi(tokens[1]));
            std::string table_name = tokens[2];
            PageID first_page = static_cast<PageID>(std::stoi(tokens[3]));
            size_t column_count = static_cast<size_t>(std::stoul(tokens[4]));
            if (tokens.size() != 5 + column_count) {
                throw std::runtime_error("Bad catalog schema record.");
            }

            TableSchema schema;
            for (size_t i = 0; i < column_count; i++) {
                const auto& column_token = tokens[5 + i];
                auto separator = column_token.find(':');
                if (separator == std::string::npos) {
                    throw std::runtime_error("Bad catalog column record.");
                }
                auto column_name = column_token.substr(0, separator);
                auto column_type = static_cast<FieldType>(
                    std::stoi(column_token.substr(separator + 1))
                );
                schema.columns.push_back({column_name, column_type});
            }

            TableMetadata metadata{
                table_id, table_name, std::move(schema), first_page
            };
            tables_by_name.insert_or_assign(table_name, std::move(metadata));
        }
    }
...
public:
    ...
    PageID nextPage(PageID page_id) {
        return getPage(page_id)->getNextPage();
    }
    ...
    PageID allocatePageAfter(PageID previous_page_id) {
        ...
        if (previous_page_id != INVALID_PAGE_ID) {
            ...
            previous_page->setNextPage(page_id);
            buffer_manager.flushPage(previous_page_id);
        } else {
