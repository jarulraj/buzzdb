...
private:
    ...
    std::unique_ptr<Field> lookup_key;
    std::unique_ptr<IPredicate> residual_predicate;
...
public:
    IndexScanOperator(TableHeap& table_heap,
                      const HashIndex& index,
                      std::unique_ptr<Field> lookup_key,
                      std::unique_ptr<IPredicate> residual_predicate = nullptr)
        : table_heap(table_heap),
          index(index),
          lookup_key(std::move(lookup_key)),
          residual_predicate(std::move(residual_predicate)) {}

    void open() override {
        matching_records = index.lookup(*lookup_key);
        record_index = 0;
        has_next = false;
    ...
    bool next() override {
        has_next = false;
        current_output.clear();
        while (matching_records && record_index < matching_records->size()) {
            const auto& record = (*matching_records)[record_index++];
            auto tuple = table_heap.getTuple(record.page_id, record.slot_id);
            if (!tuple) {
                continue;
            }
            if (residual_predicate &&
                !residual_predicate->check(tuple->fields)) {
                continue;
            }
            current_output = std::move(tuple->fields);
            has_next = true;
            return true;
        }
        return false;
...
struct QueryComponents {
    ...
    std::vector<UdfPredicateClause> udf_predicates;
...
QueryComponents cloneQueryComponents(const QueryComponents& source) {
    QueryComponents copy;
    copy.tableName = source.tableName;
    copy.selectAttributes = source.selectAttributes;
    copy.aggregateOperation = source.aggregateOperation;
    copy.aggregateFunction = source.aggregateFunction;
    copy.aggregateAttributeIndex = source.aggregateAttributeIndex;
    copy.groupBy = source.groupBy;
    copy.groupByAttributeIndex = source.groupByAttributeIndex;
    copy.whereCondition = source.whereCondition;
    copy.whereAttributeIndex = source.whereAttributeIndex;
    copy.lowerBound = source.lowerBound;
    copy.upperBound = source.upperBound;
    copy.equalityCondition = source.equalityCondition;
    copy.equalityAttributeIndex = source.equalityAttributeIndex;
    copy.equalityColumnName = source.equalityColumnName;
    copy.equalityValueText = source.equalityValueText;
    if (source.equalityValue) {
        copy.equalityValue = source.equalityValue->clone();
    }

    copy.base_table_ref_id = source.base_table_ref_id;
...
QueryComponents parseJoinQuery(const std::string& query, Catalog& catalog) {
    ...
    if (std::regex_match(rest, matches, whereRegex)) {
        for (const auto& predicate : splitOnAnd(matches[1])) {
            ...
            if (std::regex_match(predicate, matches, udfPredicateRegex)) {
                components.udf_predicates.push_back({
                    matches[1],
                    resolveColumn(matches[2], matches[3]),
                    matches[4]
                });
                continue;
            }
...
struct ScalarUdfDefinition {
    ...
    std::string lookup_key_column;
    std::string return_column;
    std::string default_alias;
};

class ScalarUdfCatalog {
public:
    ScalarUdfDefinition lookup(const std::string& function_name) const {
        if (function_name == "info_type_name") {
            return {
                "info_type",
                "id",
                "info",
        ...
        throw std::runtime_error("Unsupported scalar UDF: " + function_name);
    }
};
...
public:
    explicit QueryRewriter(Catalog& catalog) : catalog(catalog) {}

    QueryComponents inlineScalarUdfs(QueryComponents components) {
        for (const auto& predicate : components.udf_predicates) {
            auto udf = udf_catalog.lookup(predicate.function_name);
            const auto& lookup_table = catalog.getTable(udf.lookup_table);

            std::string alias = freshAlias(components, udf.default_alias);
            TableRefId lookup_ref_id = nextTableRefId(components);
            components.table_refs.push_back({
                lookup_ref_id,
            ...
            size_t lookup_key_column = static_cast<size_t>(
                lookup_table.schema.getColumnIndex(udf.lookup_key_column)
            );
            size_t return_column = static_cast<size_t>(
                lookup_table.schema.getColumnIndex(udf.return_column)
            );

            components.joins.push_back({
                lookup_ref_id,
                predicate.argument,
                {lookup_ref_id, lookup_key_column}
            });
            components.filters.push_back({
                {lookup_ref_id, return_column},
                FieldParser::parseValue(
                    lookup_table.schema.columns[return_column].type,
                    predicate.value_text
                )
            });
        }
        components.udf_predicates.clear();
        return components;
    }

private:
    static TableRefId nextTableRefId(const QueryComponents& components) {
        TableRefId next_id = 1;
        for (const auto& table_ref : components.table_refs) {
            next_id = std::max<TableRefId>(next_id, table_ref.id + 1);
        }
        return next_id;
    }

    static std::string freshAlias(const QueryComponents& components,
                                  const std::string& base_alias) {
        auto aliasExists = [&](const std::string& alias) {
            return std::any_of(
                components.table_refs.begin(),
                components.table_refs.end(),
                [&](const TableRef& table_ref) {
                    return table_ref.alias == alias;
                }
            );
        };

        if (!aliasExists(base_alias)) {
            return base_alias;
        }
        for (size_t suffix = 1; ; suffix++) {
            std::string alias = base_alias + std::to_string(suffix);
...
struct OpaqueUdfExecutionStats {
...
struct OpaqueUdfPredicateBinding {
    std::string function_name;
...
public:
    OpaqueScalarUdfEvaluator(Catalog& catalog,
                             PageManager& page_manager,
                             OpaqueUdfExecutionStats& stats)
        : catalog(catalog), page_manager(page_manager), stats(stats) {}
...
class OpaqueScalarUdfPredicate : public IPredicate {
private:
    const OpaqueScalarUdfEvaluator& evaluator;
    std::vector<OpaqueUdfPredicateBinding> bindings;
    OpaqueUdfExecutionStats& stats;

public:
    OpaqueScalarUdfPredicate(
            const OpaqueScalarUdfEvaluator& evaluator,
            std::vector<OpaqueUdfPredicateBinding> bindings,
            OpaqueUdfExecutionStats& stats)
        : evaluator(evaluator),
    ...
    bool check(const std::vector<std::unique_ptr<Field>>& tupleFields) const override {
        ...
        for (const auto& binding : bindings) {
            if (binding.argument_offset >= tupleFields.size()) {
                throw std::runtime_error(
                    "Opaque scalar UDF argument offset out of range."
                );
            }
            if (!evaluator.matches(binding,
                                   *tupleFields[binding.argument_offset])) {
                return false;
            }
        }
        stats.passed_rows++;
...
struct PhysicalPlanNode {
    ...
    bool has_index_lookup = false;
    ColumnRef index_lookup_column;
    ColumnRef index_scan_column;
    std::shared_ptr<Field> index_scan_value;
    std::optional<ColumnRef> delivered_order;
    double cost = 0.0;
    ...
    bool isIndexScan() const {
        return op == PhysicalOpKind::IndexScan;
    }

    bool isBaseAccess() const {
        return isTableScan() || isIndexScan();
    }
...
struct MemoRuleStats {
    ...
    size_t index_scan_implementations = 0;
...
QueryResult executeJoinQuery(const QueryComponents& components,
                             ...
                             OpaqueUdfExecutionStats* opaque_udf_stats = nullptr) {
    ...
    std::vector<std::unique_ptr<IndexScanOperator>> index_scans;
    ...
    OpaqueUdfExecutionStats local_opaque_udf_stats;
    OpaqueUdfExecutionStats& udf_stats =
        opaque_udf_stats ? *opaque_udf_stats : local_opaque_udf_stats;
    std::optional<OpaqueScalarUdfEvaluator> opaque_udf_evaluator;
    if (!components.udf_predicates.empty()) {
        opaque_udf_evaluator.emplace(catalog, page_manager, udf_stats);
    }

    auto makeOpaqueUdfPredicateForTable =
            [&](TableRefId table_ref_id) -> std::unique_ptr<IPredicate> {
        if (!opaque_udf_evaluator) {
            return nullptr;
        }

        std::vector<OpaqueUdfPredicateBinding> bindings;
        for (const auto& predicate : components.udf_predicates) {
            if (predicate.argument.table_ref_id != table_ref_id) {
                continue;
            }
            bindings.push_back({
                predicate.function_name,
                predicate.argument.column_index,
                predicate.value_text
            });
        }
        if (bindings.empty()) {
        ...
        return std::make_unique<OpaqueScalarUdfPredicate>(
            *opaque_udf_evaluator,
            std::move(bindings),
            udf_stats
        );
    };

    auto makeBaseAccessPredicate =
            [&](TableRefId table_ref_id) -> std::unique_ptr<IPredicate> {
        auto combined = std::make_unique<ComplexPredicate>(
            ComplexPredicate::LogicOperator::AND
        );
        bool has_predicate = false;

        auto table_predicate = makeTableFilterPredicate(components, table_ref_id);
        if (table_predicate) {
            combined->addPredicate(std::move(table_predicate));
            has_predicate = true;
        }

        auto udf_predicate = makeOpaqueUdfPredicateForTable(table_ref_id);
        if (udf_predicate) {
            combined->addPredicate(std::move(udf_predicate));
            has_predicate = true;
        }
    ...
    auto addScan = [&](TableRefId table_ref_id) -> Operator& {
        ...
        auto predicate = makeBaseAccessPredicate(table_ref_id);
        if (predicate) {
    ...
    auto addIndexScan =
            [&](TableRefId table_ref_id,
                const ColumnRef& index_column,
                const Field& lookup_key) -> Operator& {
        if (!index_manager) {
            throw std::runtime_error("Index scan requires an index manager.");
        }
        const auto& table_ref = tableRefForId(components, table_ref_id);
        IndexDescriptor descriptor{
            table_ref.table_id,
        ...
        TableHeap& table_heap = addTableHeap(table_ref_id);
        auto predicate = makeBaseAccessPredicate(table_ref_id);
        index_scans.push_back(std::make_unique<IndexScanOperator>(
            table_heap,
            index_manager->index(descriptor),
            lookup_key.clone(),
            std::move(predicate)
        ));
        return *index_scans.back();
    };
    ...
    auto addIndexJoinOperator =
        ...
        auto inner_predicate =
            makeBaseAccessPredicate(indexed_table_ref_id);
        size_t outer_attr_index =
    ...
    std::function<BuiltPlanOperator(const std::shared_ptr<PhysicalPlanNode>&)>
        ...
        if (node->isTableScan() || node->isIndexScan()) {
            Operator& scan = node->isIndexScan()
                ? addIndexScan(
                      node->table_ref_id,
                      node->index_scan_column,
                      *node->index_scan_value
                  )
                : addScan(node->table_ref_id);
            return {
                &scan,
                {node->table_ref_id},
...
private:
    ...
    struct IndexedEqualityFilter {
        ColumnRef column;
        const Field* value = nullptr;
    };
    ...
    static double indexScanCost(double estimated_rows) {
        return tupleIndexProbeCost(1.0) +
               tupleFetchByRecordIdCost(estimated_rows) +
               tupleMaterializationCost(estimated_rows);
    }
    ...
    std::optional<IndexedEqualityFilter> indexedEqualityFilterForTable(
            const QueryComponents& components,
            TableRefId table_ref_id,
            const IndexManager* index_manager) {
        if (!index_manager) {
    ...
    std::shared_ptr<PhysicalPlanNode> makeIndexScanPlanNode(
            TableRefId table_ref_id,
            const RelationStats& relation,
            const IndexedEqualityFilter& filter) {
        auto node = std::make_shared<PhysicalPlanNode>();
        node->op = PhysicalOpKind::IndexScan;
        node->table_ref_id = table_ref_id;
        node->table_refs.insert(table_ref_id);
        node->relation = relation;
        node->index_scan_column = filter.column;
        node->index_scan_value = std::shared_ptr<Field>(filter.value->clone());
        node->cost = indexScanCost(relation.rows);
        return node;
    }
    ...
    std::optional<JoinTreeStepEstimate> estimateJoinTreeStepForKind(
        ...
        auto considerIndexJoin =
                [&](const PhysicalPlanNode* indexed_plan,
                    ...
                    double outer_rows,
                    double outer_cost) {
            if (!indexed_plan || !indexed_plan->isBaseAccess()) {
                return;
            }
            if (!indexDescriptorForColumn(
                    components,
    ...
    void appendPhysicalPlanTree(
        ...
        if (node->isIndexScan()) {
            output << tableRefForId(components, node->table_ref_id).alias
                   << " = IndexScan on "
                   << columnRefLabel(components, catalog, node->index_scan_column)
                   << " = " << fieldToString(*node->index_scan_value)
                   << ", est rows=" << formatEstimate(node->relation.rows)
                   << ", cost=" << formatEstimate(node->cost)
                   << "\n";
            return;
        ...
        for (GroupId group_id = 1; group_id <= group_count; group_id++) {
            ...
            for (const auto& expression : expressions) {
                if (expression.op == MemoOpKind::LogicalScan) {
                    TableRefId table_ref_id = INVALID_TABLE_REF_ID;
                    if (memo.groupFor(group_id).table_refs.size() == 1) {
                        table_ref_id = *memo.groupFor(group_id).table_refs.begin();
                    }
                    ...
                    if (table_ref_id != INVALID_TABLE_REF_ID &&
                        indexedEqualityFilterForTable(
                            components,
                            table_ref_id,
                            index_manager) &&
                        memo.addExpressionToGroup(
                            group_id,
                            MemoOpKind::IndexScan,
                            {},
                            expression.label)) {
                        stats.scan_implementations++;
                        stats.index_scan_implementations++;
                    }
                    continue;
                }
    ...
    void applyCascadesExpression(
        ...
        if (expression.op == MemoOpKind::IndexScan) {
            if (group.table_refs.size() != 1) {
                return;
            }
            TableRefId table_ref_id = *group.table_refs.begin();
            auto filter = indexedEqualityFilterForTable(
                components,
                table_ref_id,
                index_manager
            );
            ...
            auto plan = makeIndexScanPlanNode(
                table_ref_id,
                base_relations.at(table_ref_id),
                *filter
            );
...
public:
    explicit QueryProcessor(
        ...
        : transactional_storage_manager(transactional_storage_manager),
          ...
          query_rewriter(transactional_storage_manager.catalog),
    ...
    void execute(const ParsedStatement& statement,
        ...
        if (statement.kind == ParsedStatement::Kind::Select &&
            ...
            auto rewritten_query = rewriteScalarUdfs(statement.query);
            auto stats = query_optimizer.analyzeQueryTables(rewritten_query);
    ...
    QueryComponents inlineScalarUdfs(QueryComponents components) {
        return query_rewriter.inlineScalarUdfs(std::move(components));
    }
    ...
    MemoOptimizationResult optimizeWithCascadesQueue(
            const QueryComponents& components) {
        auto rewritten_query = rewriteScalarUdfs(components);
        auto stats = query_optimizer.analyzeQueryTables(rewritten_query);
    ...
    MemoOptimizationResult optimizeWithCascadesQueueWithoutInlining(
            const QueryComponents& components) {
...
private:
    QueryComponents rewriteScalarUdfs(const QueryComponents& components) {
        return query_rewriter.inlineScalarUdfs(cloneQueryComponents(components));
    }
...
public:
    ...
    QueryComponents inlineScalarUdfs(QueryComponents components) {
        return query_processor.inlineScalarUdfs(std::move(components));
    }
    ...
    MemoOptimizationResult optimizeWithCascadesQueueWithoutInlining(
            const QueryComponents& components) {
        return query_processor.optimizeWithCascadesQueueWithoutInlining(components);
    }
...
const std::string imdb_udf_query =
    "PROJECT {mc.company_id}, {miidx.info}, {mi.info}, {t.title} "
    ...
    "WHERE info_type_name({mi.info_type_id}) = release_dates "
...
void printMemoRuleStats(const std::string& label,
    ...
    std::cout << "  " << label
              ...
              << " (index=" << stats.index_scan_implementations << ")"
...
void runImdbScalarUdfInlining() {
    BuzzDB db;
    ensureImdbDatasetLoaded(db);

    auto original = db.parseSelectStatement(imdb_udf_query);
    auto opaque_query = cloneQueryComponents(original);

    auto indexes = imdbIndexSpecs();
    db.buildIndexes(indexes);

    std::cout << "\nOriginal IMDB query:" << std::endl;
    std::cout << "  tables before inlining=" << original.table_refs.size()
              << ", scalar UDF predicates="
              << original.udf_predicates.size() << std::endl;
    std::cout << "  hidden lookup: info_type_name(mi.info_type_id)"
              << " reads info_type.id -> info_type.info"
              << std::endl;

    auto opaque_search = db.optimizeWithCascadesQueueWithoutInlining(
        opaque_query
    );

    std::cout << "\nOpaque scalar UDF plan:" << std::endl;
    std::cout << "  release_dates lookup is hidden inside the function body"
              << std::endl;
    printMemoRuleStats("without UDF inlining", opaque_search.rule_stats);
    std::cout << "  cost=" << formatEstimate(opaque_search.estimated_cost)
              << ", final_est=" << formatEstimate(opaque_search.final_estimate)
              << ", index_joins=" << opaque_search.index_joins
              << ", index_scans=" << opaque_search.index_scans
              << ", sort_enforcers=" << opaque_search.rule_stats.sort_enforcers
              << std::endl;
    OpaqueUdfExecutionStats opaque_udf_stats;
    auto opaque_executed = executeTimedPlan(
        db,
        opaque_search.components,
        opaque_search.plan_root,
        0,
        &opaque_udf_stats
    );

    std::cout << "\nExecuted opaque UDF plan:" << std::endl;
    std::cout << "  rows=" << opaque_executed.result.row_count
              << ", elapsed=" << formatEstimate(opaque_executed.elapsed_seconds)
              << " seconds" << std::endl;
    std::cout << "  UDF candidate rows=" << opaque_udf_stats.candidate_rows
              << ", function calls=" << opaque_udf_stats.function_calls
              << ", rows after UDF=" << opaque_udf_stats.passed_rows
              << ", cached lookup rows=" << opaque_udf_stats.cache_entries
              << std::endl;

    auto inlined = db.inlineScalarUdfs(cloneQueryComponents(original));

    std::cout << "\nFroid-style rewrite:" << std::endl;
    std::cout << "  added JOIN info_type AS it_udf"
              << " ON mi.info_type_id = it_udf.id" << std::endl;
    std::cout << "  added filter it_udf.info = release_dates" << std::endl;
    std::cout << "  tables after inlining=" << inlined.table_refs.size()
              << ", scalar UDF predicates="
              << inlined.udf_predicates.size() << std::endl;

    auto search = db.optimizeWithCascadesQueue(inlined);
    ...
    std::cout << "\nExecuted inlined plan:" << std::endl;
    std::cout << "  rows=" << executed.result.row_count
              << ", elapsed=" << formatEstimate(executed.elapsed_seconds)
              << " seconds" << std::endl;
    std::cout << "  UDF function calls=0" << std::endl;
    std::cout << "  row-count check: opaque="
              << opaque_executed.result.row_count
              << ", inlined=" << executed.result.row_count
              << (opaque_executed.result.row_count == executed.result.row_count
                      ? " (match)"
                      : " (DIFFER)")
              << std::endl;

    std::cout << "\nTakeaway: the scalar UDF is no longer a black-box"
              << " per-row function call. Inlining exposes info_type as"
              << " a normal table, so Cascades can cost it, reorder it, and"
              << " use the movie_info.info_type_id index instead of scanning"
              << " movie_info and calling the function for every row.\n";
}

int main() {
    try {

    std::cout << "IMDB scalar UDF inlining\n";

    runImdbScalarUdfInlining();
