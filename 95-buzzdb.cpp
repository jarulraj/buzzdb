...
PlannedQuery chooseBushyDPPlan(const QueryComponents& components,
                               const StatisticsCatalog& stats,
                               bool ordered_output_required = false) {
    auto table_names = writtenJoinOrder(components);
    if (table_names.size() <= 1 || table_names.size() >= 63) {
...
PlannedQuery chooseJoinOrderPlan(const QueryComponents& components,
    ...
    if (algorithm == JoinOrderAlgorithm::BushyDP) {
        return chooseBushyDPPlan(
            components,
            stats,
            ordered_output_required
...
public:
    ...
    void printStatsAndEstimates(const std::string& query,
        ...
        auto default_algorithm = JoinOrderAlgorithm::BushyDP;
...
struct JoinOrderRun {
    ...
    size_t dpCandidatesConsidered = 0;
    size_t dpCrossProductsPruned = 0;
    std::vector<AnnealingCheckpointRun> annealingCheckpoints;
...
int main(int argc, char* argv[]) {
    ...
    std::vector<JoinOrderAlgorithm> algorithms = {
        ...
        JoinOrderAlgorithm::SimulatedAnnealing
    };
    ...
    for (auto algorithm : algorithms) {
        ...
        std::vector<AnnealingCheckpointRun> checkpoint_runs;
        for (const auto& checkpoint : plan.annealingCheckpoints) {
            db.clearBufferPool();
            auto checkpoint_start = std::chrono::steady_clock::now();
            auto checkpoint_rows = db.executePlanSnapshot(
                checkpoint,
                query_txn,
                false
            );
            auto checkpoint_end = std::chrono::steady_clock::now();
            checkpoint_runs.push_back({
                checkpoint.transformations,
                elapsedMs(checkpoint_start, checkpoint_end),
                checkpoint.estimatedCost,
                checkpoint_rows.size(),
                checkpoint.order
            });
        }
        db.clearBufferPool();
        ...
        runs.push_back({
            ...
            std::move(checkpoint_runs),
            std::move(rows)
        });
    }

    for (const auto& run : runs) {
        std::cout << "  " << joinOrderAlgorithmName(run.algorithm)
                  << ": " << run.elapsed_ms << " ms"
        ...
        if (run.dpStatesKept > 0) {
            ...
            if (run.algorithm == JoinOrderAlgorithm::ConnectedSubgraphDP) {
                std::cout << ", connected pairs: "
        ...
        if (!run.annealingCheckpoints.empty()) {
            std::cout << "    runtime checkpoints:" << std::endl;
            for (const auto& checkpoint : run.annealingCheckpoints) {
                std::cout << "      after " << checkpoint.transformations
                          << " transformations: " << checkpoint.elapsed_ms
                          << " ms, estimated cost="
                          << formatEstimate(checkpoint.estimated_cost)
                          << ", rows=" << checkpoint.output_rows
                          << std::endl;
                std::cout << "        plan: " << checkpoint.order
                          << std::endl;
            }
        }
    }
    ...
    if (!runs.empty()) {
        printSampleRows(runs.back().sample_rows, 5);
    }
    db.commit(query_txn);
    std::cout << "\nTiming:" << std::endl;
    if (seed_database) {
