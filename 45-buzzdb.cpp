...
public:
    ...
    void insert(int key, int value) {
        executeStatementsAndQueries({
            "INSERT (" + std::to_string(key) + ", " + std::to_string(value) + ")"
        }, false);
    }
    ...
    void executeStatementsAndQueries(const std::vector<std::string>& statements,
        ...
        for (const auto& statement : statements) {
            ...
            std::regex insertRegex("^\\s*INSERT\\s*\\(?\\s*(-?\\d+)\\s*,?\\s*(-?\\d+)\\s*\\)?\\s*;?\\s*$",
                                   std::regex_constants::icase);
            if (std::regex_match(statement, matches, insertRegex)) {
                const int insertKey = std::stoi(matches[1]);
                const int insertValue = std::stoi(matches[2]);
            ...
            std::regex updateRegex("^\\s*UPDATE\\s+SET\\s+\\{(\\d+)\\}\\s*=\\s*(-?\\d+)\\s+WHERE\\s+\\{(\\d+)\\}\\s*=\\s*(-?\\d+)\\s*;?\\s*$",
                                   std::regex_constants::icase);
            if (std::regex_match(statement, matches, updateRegex)) {
                const size_t setColumn = static_cast<size_t>(std::stoi(matches[1]) - 1);
                const int setValue = std::stoi(matches[2]);
                const size_t whereColumn = static_cast<size_t>(std::stoi(matches[3]) - 1);
                const int whereValue = std::stoi(matches[4]);
            ...
            std::regex deleteRegex("^\\s*DELETE\\s+WHERE\\s+\\{(\\d+)\\}\\s*=\\s*(-?\\d+)\\s*;?\\s*$",
                                   std::regex_constants::icase);
            if (std::regex_match(statement, matches, deleteRegex)) {
                const size_t whereColumn = static_cast<size_t>(std::stoi(matches[1]) - 1);
                const int whereValue = std::stoi(matches[2]);
            ...
            if (std::regex_match(statement, matches, selectRegex)) {
                ...
                auto components = parseQuery(queryText);
...
int main() {
    ...
    db.executeStatementsAndQueries({
        "UPDATE SET {2} = 7 WHERE {1} = 3",
        "DELETE WHERE {1} = 4",
        "INSERT (6, 6)",
        "SELECT SUM{2} GROUP BY {1} WHERE {1} > 2 and {1} < 6"
    });
