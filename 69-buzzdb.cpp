...
constexpr uint16_t BUZZDB_VERSION = 69;
...
void RecoveryManager::recover() {
    ...
    auto collectWalRecord = [&](const std::string& record) {
        ...
        if (!(input >> prev_lsn)) {
            throw std::runtime_error(
                "buzzdb.log was written by an older WAL format; remove it before running v69."
            );
        }
        if (txn_id > 0) {
...
public:
    explicit ScheduleAnalyzer(std::vector<ScheduleOperation> schedule)
        : schedule(std::move(schedule)) {}

    void printAnalysis() const {
        Analysis analysis = analyze();

        std::cout << "  Schedule analysis:" << std::endl;
        std::cout << "    Serialization graph edges:" << std::endl;
        if (analysis.conflict_edges.empty()) {
            std::cout << "      none" << std::endl;
        } else {
            for (const auto& edge : analysis.conflict_edges) {
                std::cout << "      T" << edge.from_txn
                          << " -> T" << edge.to_txn
                          << " because " << edge.reason << std::endl;
            }
        }
        ...
        printResult("Strict", analysis.strict);
    }

private:
    std::vector<ScheduleOperation> schedule;
    ...
    Analysis analyze() const {
        ...
        for (size_t i = 0; i < schedule.size(); i++) {
            graph.addTransaction(schedule[i].txn_id);
            for (size_t j = i + 1; j < schedule.size(); j++) {
                graph.addTransaction(schedule[j].txn_id);
                ...
                graph.addEdge(
                    schedule[i].txn_id,
                    schedule[j].txn_id,
                    operationLabel(schedule[i]) + " precedes " + operationLabel(schedule[j])
                );
            }
        }
    ...
    static bool conflicts(const ScheduleOperation& left,
    ...
    void analyzeRecoveryProperties(Analysis& analysis) const {
        ...
        for (size_t i = 0; i < schedule.size(); i++) {
            const auto& operation = schedule[i];
            ...
            if (writer != last_writer.end() &&
                ...
                !finishedBefore(end_index, writer->second, i)) {
    ...
    static bool finishedBefore(const std::map<int, size_t>& end_index,
                               int txn_id,
    ...
    static std::string operationLabel(const ScheduleOperation& operation) {
...
public:
    ...
    void executeStatement(const std::string& statement,
        ...
        if (recovery_manager.isActive()) {
            throw std::runtime_error("INSERT inside an undo/redo WAL update transaction is not supported in v69.");
        }
...
public:
    ...
    void run() {
        std::cout << "\nThreaded seat booking with lock upgrades" << std::endl;
        std::cout << "  T1 and T2 run in separate threads and both try seat 1B." << std::endl;
        std::cout << "  Shared-read phase: both can check availability with compatible S locks." << std::endl;
        std::cout << "  Upgrade phase: booking requires converting S to X on the same row." << std::endl;
        std::cout << "  The lock manager records waits-for edges and names S-to-X upgrades." << std::endl;
        std::cout << "  DB calls are serialized here because the storage layer is not thread-safe yet." << std::endl;

        runReleaseBeforeUpgradeContrast();
        resetBookingRows(db);
        ...
        printFinalState();
        std::cout << "  Observed history analysis:" << std::endl;
        ScheduleAnalyzer(observedHistory()).printAnalysis();
    }
...
private:
    ...
    void runReleaseBeforeUpgradeContrast() {
    ...
    void bookSeat(int txn_id,
        ...
        waitForBothReads();
        if (txn_id == 1) {
            log("upgrade benefit: both transactions completed the availability read before either took X");
        }
        if (txn_id == 2) {
        ...
        auto old_hold = writeItem(hold_item, customer);
        record({txn_id, ScheduleOpType::WRITE, hold_item_name, customer});
        log("T" + std::to_string(txn_id) +
            " UPDATE holds SET customer=" + customer +
            " WHERE id=" + hold_item.key_value + ": " +
            old_hold.value + " -> " + customer +
    ...
    bool requestLock(int txn_id,
                     const std::string& mode,
                     const std::string& resource,
                     std::chrono::milliseconds timeout) {
        bool upgrade = mode == "X" &&
                       lock_manager.hasSharedLock(txn_id, resource);
        std::string request_label = upgrade ?
            "S->X lock upgrade on " + resource :
            mode + " lock on " + resource;

        log("T" + std::to_string(txn_id) + " requests " + request_label);
        ...
        log("T" + std::to_string(txn_id) + " " +
            request_label + " timed out; " + result.reason);
        return false;
    }
    ...
    void releaseAndLog(int txn_id) {
        if (lock_manager.releaseAll(txn_id)) {
            log("T" + std::to_string(txn_id) + " RELEASE locks");
        }
    }
    ...
    ItemRead readItem(const ScheduleItemRef& item) {
        std::lock_guard<std::mutex> guard(db_latch);
        return readItemNoLock(item);
    }

    ItemRead writeItem(const ScheduleItemRef& item, const std::string& value) {
        std::lock_guard<std::mutex> guard(db_latch);
        auto old_value = readItemNoLock(item);
        executeQuiet(db, updateStatement(item, value));
        return old_value;
    }

    ItemRead readItemNoLock(const ScheduleItemRef& item) {
        auto rows = db.executeQuery(projectStatement(item), nullptr, false);
        for (const auto& row : rows) {
            if (row.fields.size() >= 2 &&
                fieldToString(*row.fields[0]) == item.key_value) {
                if (!row.tuple_id) {
                    throw std::runtime_error("Query did not return a TupleId.");
                }
    ...
    void printFinalState() {
        ...
        std::cout << "    holds[id=1].customer=" << hold_1_customer.value
                  << " at " << tupleIdLabel(hold_1_customer.tuple_id) << std::endl;
        std::cout << "    holds[id=2].customer=" << hold_2_customer.value
                  << " at " << tupleIdLabel(hold_2_customer.tuple_id) << std::endl;
    }

    void record(ScheduleOperation operation) {
        std::lock_guard<std::mutex> guard(history_latch);
        history.push_back(std::move(operation));
    }

    std::vector<ScheduleOperation> observedHistory() {
        std::lock_guard<std::mutex> guard(history_latch);
        return history;
    }
    ...
    static std::string resourceFor(const ScheduleItemRef& item) {
        return item.table + "." + item.key_column + "=" + item.key_value;
    }
