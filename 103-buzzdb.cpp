...
int main(int argc, char* argv[]) {
    ...
    std::cout << "\nQuery execution with Lookahead Information Passing" << std::endl;
    std::cout << "  Workload: JOB 9-table join query" << std::endl;
    std::cout << "  LIP builds Bloom filters from filtered join inputs"
              << " and probes them during the same execution." << std::endl;
    ...
    std::cout << "\nSame plan execution timing:" << std::endl;
    std::cout << "  Buffer pool is cleared before each timed execution."
              << std::endl;

    db.clearBufferPool();
    auto no_lip_start = std::chrono::steady_clock::now();
    auto no_lip_rows = db.executePlanSnapshot(
        snapshot,
        nullptr,
        false,
        false
    );
    auto no_lip_end = std::chrono::steady_clock::now();

    db.clearBufferPool();
    LipExecutionContext lip_context;
    auto lip_start = std::chrono::steady_clock::now();
    auto lip_rows = db.executePlanSnapshot(
        snapshot,
        nullptr,
        false,
        true,
    ...
    auto lip_end = std::chrono::steady_clock::now();

    printLipSummary(lip_context);
    std::cout << "  Without LIP: "
              << elapsedMs(no_lip_start, no_lip_end)
              << " ms, output rows " << no_lip_rows.size() << std::endl;
    std::cout << "  With LIP: "
              << elapsedMs(lip_start, lip_end)
              << " ms, output rows " << lip_rows.size() << std::endl;

    std::cout << "\nSelected physical plan:" << std::endl;
    ...
    std::cout << "  output rows: " << lip_rows.size() << std::endl;
    if (plan.planRoot) {
